...
public abstract class QueryVodOrder extends PersonQueryPage
{
    static transient final Logger logger = Logger.getLogger(QueryVodOrder.class);
    ...
    public void queryBaseRecord(IRequestCycle cycle) throws Exception
    {
        ...
        String bookName = condParams.getString("BOOK_NAME", ""); // 书名
        String chapterName = condParams.getString("CHAPTER_NAME", ""); // 章节名称
        String startDate = condParams.getString("START_DATE", ""); // 订购起始时间
        String endDate = condParams.getString("END_DATE", ""); // 订购结束时间
        String chargeType = condParams.getString("CHARGE_TYPE", ""); // 计费方式
        String contentType = condParams.getString("CONTENT_TYPE", ""); // 内容类型
        String isPresent = condParams.getString("IS_PRESENT", ""); // 是否赠送
        String orderWay = condParams.getString("ORDER_WAY", ""); // 订购途径

        StringBuilder operateConditions = new StringBuilder();

        operateConditions.append(serialNum);
        operateConditions.append("|");
        operateConditions.append(bookName);
        operateConditions.append("|");
        operateConditions.append(startDate);
        operateConditions.append("|");
        operateConditions.append(endDate);
        operateConditions.append("|");
        operateConditions.append(chargeType);
        operateConditions.append("|");
        operateConditions.append(contentType);
        operateConditions.append("|");
        operateConditions.append(isPresent);
        operateConditions.append("|");
        operateConditions.append(orderWay);
        operateConditions.append("|");
        operateConditions.append(chapterName);

        queryData.put("KIND_ID", "BIP2C092_T2002092_0_0");
        queryData.put("CALLERNO", serialNum);
        queryData.put("SVCTYPEID", "0103030305");// 服务请求分类编码
        queryData.put("CONTACTCHANNEL", "08");// 受理渠道
        queryData.put("SERVICETYPEID", "42");// 业务类别
        queryData.put("OPERATETYPEID", "01006");// 操作类型
