...
public abstract class GroupImsSpecialOpen extends GroupBasePage
{
    ...
    public void getGroupInfoByGrpSn(IRequestCycle cycle) throws Throwable
    {
        ...
        {
            CSViewException.apperr(CrmCommException.CRM_COMM_103, "该集团产品编码不是集团多媒体桌面电话产品，无法在此界面操作！");
        }
        String userid = userInfo.getString("USER_ID");
        queryInfoByUserId(userid,productId);
    }
    ...
    public void onSubmitBaseTrade(IRequestCycle cycle) throws Throwable
    {
        ...
        if(!"2222".equals(productId))
        {
        	CSViewException.apperr(CrmCommException.CRM_COMM_103, "请输入集团多媒体桌面电话产品编码查询信息后再提交！");
        }
        ...
        IDataset dataset = CSViewCall.call(this, "SS.ImsDesktopTelMebBatSVC.crtOpenBat", svcData);
      
        setAjax(dataset);

    }

    private void queryInfoByUserId(String userid,String proudctId) throws Throwable
    {
        ...
        else
        {
        	if (IDataUtil.isNotEmpty(svcStateInfos))
            {
                for (int i = 0; i < svcStateInfos.size(); i++)
                {
                    ...
                    {
                    	CSViewException.apperr(GrpException.CRM_GRP_713, "该集团多媒体桌面电话状态不是欠费停机状态,业务不能继续!");
                    }
                }
            }
        }
