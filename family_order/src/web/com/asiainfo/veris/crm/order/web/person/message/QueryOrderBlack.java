...
public abstract class QueryOrderBlack extends PersonBasePage{
    ...
    public void queryBaseRecord(IRequestCycle cycle) throws Exception
    {
        ...
        String serialNum = condParams.getString("SERIAL_NUMBER", ""); // 手机号码
        String queryBlackType = condParams.getString("BLACK_TYPE", ""); // 操作类型
        String startTime = condParams.getString("START_TIME", ""); // 开始时间
        String endTime = condParams.getString("END_TIME", "");// 结束时间
        StringBuilder operateConditions = new StringBuilder();

        operateConditions.append(serialNum);
        operateConditions.append("|");
        operateConditions.append(queryBlackType);//操作类型
        operateConditions.append("|");
        operateConditions.append(startTime);
        operateConditions.append("|");
