...
public abstract class VpmnManagerDispatchList extends CSBasePage
{
    ...
    public void qryVpmnManagerImp(IRequestCycle cycle) throws Exception
    {
        ...
        inparam.put("IMPORT_TYPE", "VPMNDIS");
        ...
        setCondition(param);
        // 记录客户经理操作日志
        insOperLog("VPMN客户经理批量分配批次信息查询", "QRY", "输入参数为:" + inparam, getVisit().getStaffId(), getVisit().getDepartId(), getVisit().getCityCode(), getVisit().getRemoteAddr());
    }
    ...
    public void doThisVpmnManagerInfo(IRequestCycle cycle) throws Exception
    {
        ...
        IDataset dataset = CSViewCall.call(this, "SS.VpmnCustManagerMgrSVC.doThisVpmnManagerInfo", param);
        // 记录客户经理操作日志
        if ((Boolean) dataset.get(0))
        {
            insOperLog("VPMN客户经理批量分配处理导入标志为【" + importId + "】数据成功", "INS", "输入参数为:" + importId, getVisit().getStaffId(), getVisit().getDepartId(), getVisit().getCityCode(), getVisit().getRemoteAddr());
        }
        else
        {
            insOperLog("VPMN客户经理批量分配处理导入标志为【" + importId + "】数据失败", "UPD", "输入参数为:" + importId, getVisit().getStaffId(), getVisit().getDepartId(), getVisit().getCityCode(), getVisit().getRemoteAddr());
        }
    }
