...
public abstract class QueryBooksInfo extends PersonQueryPage
{
    static transient final Logger logger = Logger.getLogger(QueryBooksInfo.class);
    ...
    public void queryBaseRecord(IRequestCycle cycle) throws Exception
    {
        ...
        String serialNum = condParams.getString("BOOK_NAME", "");// 书项名称
        String paraCode = condParams.getString("PARA_CODE", "");// MCP名称
        String roleCode = condParams.getString("ROLE_CODE_B", "");// 作家笔名

        StringBuilder operateConditions = new StringBuilder();

        operateConditions.append(serialNum);// 手机号码
        operateConditions.append("|");
        operateConditions.append(paraCode);// MCP名称
        operateConditions.append("|");
        operateConditions.append(roleCode);// 作家笔名

        queryData.put("KIND_ID", "BIP2C092_T2002092_0_0");
        queryData.put("CALLERNO", serialNum);
        queryData.put("SVCTYPEID", "0103030302");// 服务请求分类编码
