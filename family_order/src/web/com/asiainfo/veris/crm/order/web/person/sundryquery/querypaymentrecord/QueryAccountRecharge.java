...
public abstract class QueryAccountRecharge extends PersonQueryPage
{
    static final transient Logger logger = Logger.getLogger(QueryAccountRecharge.class);
    ...
    public void queryBaseRecord(IRequestCycle cycle) throws Exception
    {
		...
		String orderNum = condParams.getString("ORDER_NUM", "");// 订单号
		String userType = condParams.getString("USER_TYPE", "");// 用户类型
		String accountType = condParams.getString("ACCOUNT_TYPE", "");// 账户类型
		String sequenceType = condParams.getString("SEQUENCE_TYPE", "");// 排序类型
		String tradeMode = condParams.getString("TRADE_MODE", "");// 交易类型
		String tradeStartDate = condParams.getString("TRADE_START_DATE", "");// 交易开始时间
		String tradeEndDate = condParams.getString("TRADE_END_DATE", "");// 交易结束时间
        
        StringBuilder operateConditions = new StringBuilder();
        
        operateConditions.append(serialNum);
        operateConditions.append("|");
        operateConditions.append(orderNum);
        operateConditions.append("|");
        operateConditions.append(userType);
        operateConditions.append("|");
        operateConditions.append(accountType);
        operateConditions.append("|");
        operateConditions.append(sequenceType);
        operateConditions.append("|");
        operateConditions.append(tradeMode);
        operateConditions.append("|");
        operateConditions.append(tradeStartDate + " 00:00:00");
		operateConditions.append("|");
		operateConditions.append(tradeEndDate + " 23:59:59");
        ...
        queryData.put("OPERATETYPEID", "01008");// 操作类型
