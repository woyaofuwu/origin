package com.asiainfo.veris.crm.order.web.person.sundryquery.querycloudrecord;
...
public abstract class QueryBlackList extends PersonQueryPage
{
    static transient final Logger logger = Logger.getLogger(QueryBlackList.class);
    ...
    public void queryBaseRecord(IRequestCycle cycle) throws Exception
    {
        ...
        String type = condParams.getString("BLACK_TYPE", "");
        String startdate = condParams.getString("PARA_CODE3", "");
        String enddate = condParams.getString("PARA_CODE4", "");

        StringBuilder operateConditions = new StringBuilder();

        operateConditions.append(serialNum);// 手机号码
        operateConditions.append("|");
        operateConditions.append(type);// 查询类型
        operateConditions.append("|");
        operateConditions.append(startdate);// 时间范围开始
        operateConditions.append("|");
        operateConditions.append(enddate);// 时间范围结束

        queryData.put("KIND_ID", "BIP2C092_T2002092_0_0");
        queryData.put("CALLERNO", serialNum);
        queryData.put("SVCTYPEID", "01010547");// 服务请求分类编码
        queryData.put("CONTACTCHANNEL", "08");// 受理渠道
        queryData.put("SERVICETYPEID", "165");// 业务类别
