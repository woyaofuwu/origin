...
public abstract class QueryBasicInfo extends PersonQueryPage
{
    static transient final Logger logger = Logger.getLogger(QueryBasicInfo.class);
    ...
    public void queryBaseRecord(IRequestCycle cycle) throws Exception
    {
        ...
        String serialNum = condParams.getString("BUSINESS_NAME", ""); // 业务名称
        String busiCode = condParams.getString("BUSINESS_CODE", ""); // 业务编码
        String busiType = condParams.getString("BUSINESS_TYPE", ""); // 业务类型

        StringBuilder operateConditions = new StringBuilder();

        operateConditions.append(serialNum);
        operateConditions.append("|");
        operateConditions.append(busiCode);
        operateConditions.append("|");
        operateConditions.append(busiType);

        if (StringUtils.isBlank(serialNum))
        {
            serialNum = "11111222223";
        }

        queryData.put("KIND_ID", "BIP2C092_T2002092_0_0");
        queryData.put("CALLERNO", serialNum);
        queryData.put("SVCTYPEID", "0103030205");// 服务请求分类编码
