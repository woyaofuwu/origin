package com.asiainfo.veris.crm.iorder.web.frame.icsview.common.component.cssubmit;
...
public abstract class CSSubmit extends CSBizTempComponent
{
	...
	private IData getElecPrintInfo(IData data) throws Exception
	{
		...
		//System.out.println("CSSubmitxxxxxxxxxxxxxxxxxxxxxxxgetElecPrintInfo80 "+data);
		...
		//System.out.println("CSSubmitxxxxxxxxxxxxxxxxxxxxxxxgetElecPrintInfo84 "+printDataSet);
		String serialNumber = "";
		if (IDataUtil.isNotEmpty(printDataSet))
		{
			returnData = printDataSet.getData(0);
			serialNumber = returnData.getString("SERIAL_NUMBER","").trim();
		...
		//System.out.println("CSSubmitxxxxxxxxxxxxxxxxxx98 "+returnData);		
		//REQ201902190015  关于吉祥号码开户全量纳入稽核的接口改造需求
		returnData.put("BEAUTIFUL_TAG", "0");//1 吉祥;0 普通		
		
		if(!serialNumber.equals("")){
			IData inData = new DataMap();
			inData.put("SERIAL_NUMBER", serialNumber);
			inData.put(Route.USER_EPARCHY_CODE,  getVisit().getStaffEparchyCode());// 用户归属地州
	        inData.put("TRADE_EPARCHY_CODE", getVisit().getStaffEparchyCode());// 用户交易地州
	        inData.put("TRADE_CITY_CODE", getVisit().getCityCode());// 受理业务区
	        inData.put("TRADE_DEPART_ID", getVisit().getDepartId());// 受理部门
	        inData.put("TRADE_STAFF_ID", getVisit().getStaffId());// 受理员工
	        inData.put(Route.ROUTE_EPARCHY_CODE, getVisit().getStaffEparchyCode());
			//System.out.println("CSSubmitxxxxxxxxxxxxxxxxxx113 "+inData);		

			ServiceResponse response = (ServiceResponse) BizServiceFactory.call("RCF.resource.INumberIntfOperateSV.getPhoneInfoByNum", inData, null);
			//System.out.println("CSSubmitxxxxxxxxxxxxxxxxxx116 "+response);		

			IDataset resds = response.getData();;
//			IDataset resds = ResCall.getMphonecodeInfo(serialNumber);
			//System.out.println("CSSubmitxxxxxxxxxxxxxxxxxx119 "+resds);		

			if(resds!=null&&resds.size()>0){
				if(resds.getData(0).getString("BEAUTIFUL_TAG","0").trim().equals("1")){
					returnData.put("BEAUTIFUL_TAG", "1");//1 吉祥;0 普通				
				}
			}
		} 
		//REQ201902190015  关于吉祥号码开户全量纳入稽核的接口改造需求
		
		//System.out.println("CSSubmitxxxxxxxxxxxxxxxxxx130 "+returnData);		
		return returnData;
	}
	...
	{
		//System.out.println("CSSubmitxxxxxxxxxxxxx129 "+data);
		data.put(Route.ROUTE_EPARCHY_CODE, data.getString("EPARCHY_CODE", getTradeEparchyCode()));
	...
	public void renderComponent(StringBuilder infoParamsBuilder, IMarkupWriter writer, IRequestCycle cycle) throws Exception
	{
		...
		{
			getPage().addResAfterBodyBegin("scripts/iorder/icsserv/component/cssubmit/CSSubmit.js");
			getPage().addResAfterBodyBegin("scripts/iorder/icsserv/component/print/PrintMgr.js");
			getPage().addResAfterBodyBegin("scripts/iorder/icsserv/component/phone/MBOP.js");
			getPage().addResAfterBodyBegin("scripts/iorder/icsserv/component/phone/common.js");
		...
		} else
		{
			...
			}else if("PSPT_FRONTBACK_HEADPIC".equals(action)){
				qryPsptFrontBack(data);
			}else if("FOREGIFT_TRADE".equals(action)){
				qryForegiftTrade(data);
            }
            else if ("QRY_WELFARE".equals(action))
            {
                // 权益弹窗信息查询
                qryWelfareRelOrder(data);
            }
     ...
     * @throws Exception
     */
	public void qryPsptFrontBack(IData data)throws java.lang.Exception{
		IData info = new DataMap();
		data.put(Route.ROUTE_EPARCHY_CODE, data.getString("EPARCHY_CODE", getTradeEparchyCode()));
		IDataset infos = CSViewCall.call(this, "CS.PrintNoteSVC.qryPsptFrontBack", data);
		if (null != infos && infos.size() > 0)
		{
			info = infos.getData(0);
		}
	 ...
	 * @throws Exception
	 */
	public void qryForegiftTrade(IData data)throws java.lang.Exception{
		IData info = new DataMap();
		data.put(Route.ROUTE_EPARCHY_CODE, data.getString("EPARCHY_CODE", getTradeEparchyCode()));
		IDataset infos = CSViewCall.call(this, "CS.PrintNoteSVC.qryForegiftTrade", data);
		if (null != infos && infos.size() > 0)
		{
			info = infos.getData(0);
		}
    ...
    /**
     * @Description: 权益受理关联订单查询
     * @Param: [data]
     * @return: void
     * @Author: zhenggang
     * @Date: 2020/7/8 10:55
     */
    public void qryWelfareRelOrder(IData data) throws Exception
    {
        IData info = new DataMap();
        String eparchyCode = data.getString("EPARCHY_CODE");
        data.put(Route.ROUTE_EPARCHY_CODE, eparchyCode);
        IDataset infos = CSViewCall.call(this, "SS.WelfareOfferSVC.qryWelfareRelOrder", data);
        if (null != infos && infos.size() > 0)
        {
            info = infos.getData(0);
        }
