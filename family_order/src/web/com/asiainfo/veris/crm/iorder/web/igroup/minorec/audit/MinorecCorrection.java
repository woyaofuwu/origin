...
public abstract class MinorecCorrection extends CSBasePage
{
    ...
    * @date 2019年11月22日下午3:04:42
    ...
    public void initial(IRequestCycle cycle) throws Exception
    {
        ...
        {
            // 获取节点信息
            preNodeId = eweNodeInfo.getString("NODE_ID");
        }
        catch (Exception e)
        {
            errorInfo = "根据流程节点ID【" + busiformNodeId + "】未查询到数据！";
        ...
        IData auditInfos = CSViewCall.callone(this, "SS.QryAuditInfoSVC.qryAuditInfoByIbsysid", auditInfo);
        if (IDataUtil.isNotEmpty(auditInfos))
        {
            setAuditInfo(auditInfos);
        }

        // 查询产品数据 TF_B_EOP_QUICKORDER_DATA SS.QuickOrderDataSVC.getNewQuickorderData
...
//                params.put("PARAM_CODE","'CC_PHONE_FEE', 'CC_WDS_FEE', 'CC_PHONE_OUTFEE','CC_COMSHOW_FEE','CC_CRBT'");
    ...
    * @date 2019年11月19日11:07:18
    ...
    public void submits(IRequestCycle cycle) throws Exception
    {
        ...
        if ("onceCorrection".equals(data.getString("NODE_ID")))
        {
            // 一次整改
