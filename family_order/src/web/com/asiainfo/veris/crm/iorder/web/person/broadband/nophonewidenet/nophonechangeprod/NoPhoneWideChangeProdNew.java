...
public abstract class NoPhoneWideChangeProdNew extends PersonBasePage
{  
    ...
    public void qryUserWideInfos(IRequestCycle cycle) throws Exception
    {
      ...
      //初始化宽带类型
        ...
        if (IDataUtil.isNotEmpty(result))
        {
            ...
            pdata1.put("IS_ONESN_MANYWIDE", result.getData(0).getString("IS_ONESN_MANYWIDE",""));
...
/*        String serialNumber = data.getString("SERIAL_NUMBER_A","");
        ...
        System.out.println("===========NoPhoneWideChangeProdNew==========SERIAL_NUMBER:"+serialNumber);
		IDataOutput output = this.call1("PB.AddressManageSvc.queryAddressForGIGABIT", data);
        if(output!=null&&output.getData().size()>0)
        {
        	System.out.println("===========NoPhoneWideChangeProdNew==========widenetInfos:"+ output.getData());
        	pdata1.put("BANDWIDTH", output.getData().getData(0).getString("BANDWIDTH",""));
        }*/
...
//        String seril_num2 = data.getString("SERIAL_NUMBER_A","");
        ...
        if (IDataUtil.isNotEmpty(result2))
        {
        	...
        	discntCode = result2.getData(0).getString("DISCNT_CODE","");
        }
