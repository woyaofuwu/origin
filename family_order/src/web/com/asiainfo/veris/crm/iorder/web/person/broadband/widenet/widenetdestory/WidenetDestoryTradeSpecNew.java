 ...
 * 用户立即销户
 */
public abstract class WidenetDestoryTradeSpecNew extends PersonBasePage
{
    ...
    public void loadChildInfo(IRequestCycle cycle) throws Exception
    {
      ...
      //查询营销活动信息，提示
		...
		// REQ202003180001 “共同战疫宽带助力”活动开发需求 start
		//非宽带类营销活动查询
		IDataset otherActives= CSViewCall.call(this, "SS.DestroyUserNowSVC.checkWideOtherSaleActive", data);
		if(IDataUtil.isNotEmpty(otherActives)){
			double otherActiveFee=0.0d;
			String otherActiveName="";
			for (int i = 0; i < otherActives.size(); i++) {
				IData otherActive = otherActives.getData(i);
				otherActiveFee+=Double.parseDouble(new BigDecimal(otherActive.getString("OTHER_ACTIVE_FEE")).toString());
				otherActiveName+=otherActive.getString("OTHER_ACTIVE_NAME")+" ";
			}
			info.put("OTHER_ACTIVE_FEE_TAG", "1");
			info.put("OTHER_ACTIVE_NAME", otherActiveName);
			info.put("OTHER_ACTIVE_FEE", otherActiveFee);
		}else{
			info.put("OTHER_ACTIVE_FEE_TAG", "0");
		}
		// REQ202003180001 “共同战疫宽带助力”活动开发需求 end
		...
		// REQ202003180001 “共同战疫宽带助力”活动开发需求 start
		param.put("OTHER_ACTIVE_FEE_TAG", info.getString("OTHER_ACTIVE_FEE_TAG"));
		param.put("OTHER_ACTIVE_NAME", info.getString("OTHER_ACTIVE_NAME",""));
		param.put("OTHER_ACTIVE_FEE", info.getString("OTHER_ACTIVE_FEE",""));
		// REQ202003180001 “共同战疫宽带助力”活动开发需求 end
