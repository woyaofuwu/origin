...
package com.asiainfo.veris.crm.iorder.web.merch.component.marketingActivity.marketactivitylist;
...
public abstract class MarketActivityList extends CSBizTempComponent
{
    ...
    {
        getPage().addResAfterBodyBegin("scripts/merch/marketingActivity/marketactivitylist/MarketActivityList.js");
        ...
        if (isAjax && StringUtils.isBlank(specTag))
        {
            return;
        }

        String pageSize = getPage().getParameter("PAGE_SIZE", "30");
        String current = getPage().getParameter("CURRENT", "1");
        Pagination page = new Pagination(Integer.parseInt(pageSize), Integer.parseInt(current));
        ...
        {
            renderComponentAction(productId, newImei, searchContent,isIphone6,iphone6Imei,page);
        }
     ...
     * @throws Exception
     */
    private void renderComponentAction(String productId, String newImei, String searchContent,String isIphone6,String iphone6Imei, Pagination page) throws Exception
    {
        ...
        if (StringUtils.isNotBlank(productId) || StringUtils.isNotBlank(newImei) || StringUtils.isNotBlank(searchContent)|| StringUtils.isNotBlank(isIphone6)|| StringUtils.isNotBlank(iphone6Imei))
        {
            ...
            saleactives = CSViewCall.callPage(this, querySaleActivesSvc, data, page).getData();
            //如果是IPHONE6的 则进行处理 caolin 20141016
        ...
        for (int i = saleactives.size() - 1; i >= 0; i--)
        {
            ...
            saleactive.put("PACKAGE_TITLE", saleactive.getString("PACKAGE_ID") + "|" + saleactive.getString("PACKAGE_NAME"));
            String packageDesc=saleactive.getString("DESCRIPTION");//搜索出来的营销包没有描述信息，需要再查一次放在页面
            if(StringUtils.isBlank(packageDesc)){           	
                IData PMoffer = UpcViewCall.queryOfferByOfferId(this, "K", saleactive.getString("PACKAGE_ID"));
                 packageDesc = PMoffer.getString("DESCRIPTION");

            }
            saleactive.put("PACKAGE_DESC",packageDesc );
