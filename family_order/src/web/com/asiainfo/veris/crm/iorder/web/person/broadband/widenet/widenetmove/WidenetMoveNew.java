package com.asiainfo.veris.crm.iorder.web.person.broadband.widenet.widenetmove;
...
public abstract class WidenetMoveNew extends PersonBasePage
{
    ...
    public void loadChildInfo(IRequestCycle cycle) throws Exception
    {
        ...
        //查询原设备ID
        IData pbParam = new DataMap();
        String deviceId = "";
        pbParam.put("SERIAL_NUMBER", serialNumber);
        IDataset userDeviceIdInfos = CSViewCall.call(this, "PB.AddressManageSvc.queryDeviceByAccount", pbParam);
        if(IDataUtil.isNotEmpty(userDeviceIdInfos)){
        	deviceId = userDeviceIdInfos.getData(0).getString("DEVICE_ID");
        }
        ...
        wideInfo.put("PRODUCT_MODE", wideTypeCode);
        wideInfo.put("DEVICE_ID", deviceId);
    ...
    public void initProductChg(IRequestCycle cycle) throws Exception
    {
    	...
    	result.put("IS_IMS_DIFF_CITY", "0");
        if(IDataUtil.isNotEmpty(dataset)){
        	String areaCode = data.getString("AREA_CODE", "");
        	String rsrvStr4 = dataset.getData(0).getString("RSRV_STR4", ""); 
        	IData imsInfo = null;
        	if(!"".equals(areaCode) && !"".equals(rsrvStr4) && !areaCode.equals(rsrvStr4))
        	{
        		imsInfo = CSViewCall.callone(this,"SS.ChangeSvcStateSVC.getIMSInfoBySerialNumber", data);
        		if (IDataUtil.isNotEmpty(imsInfo)) {
        	    	result.put("IS_IMS_DIFF_CITY", "1");
        	    	resus.add(result);
                    setAjax(resus);
                    return ;
        		}
        ...
        if((isFtth&&isOldFtth)||(isFtth&&isUsedFtth)){
            ...
            if(newAreaCode.equals(oldAreaCode)){
            	...
            	/*REQ201804170012 宽带移机功能优化  by xuzh5 2018-5-24 17:19:00
            ...
            }/*夸区域中兴华为光猫互用 by xuzh5 2018-7-27 11:39:58
        ...
        if(isFtth){
            IData data1 = getData();       
            data1.put("PARAM_CODE", "600");
            data1.put("PARA_CODE1", "FTTH");
            IDataset dataset1 = CSViewCall.call(this, "SS.MergeWideUserCreateSVC.getSaleActiveByParaCode1", data);
            //System.out.println("===========initProductChg========="+dataset1);
            setSaleActiveList2(dataset1);   
        }
        //BUS201907310012关于开发家庭终端调测费的需求
        ...
        boolean isStaffPriv = StaffPrivUtil.isPriv(staffId, "MIGRATE_ACTIVITY", "1");
        if(isStaffPriv){
        	modelInfo.put("IS_MIGRATE_ACTIVITY","1");
        }else{
        	modelInfo.put("IS_MIGRATE_ACTIVITY","0");
        }
        //REQ201912030008宽带客户FTTH迁移营销活动需求2—BOSS侧
    ...
    public void getSaleActiveDesc(IRequestCycle cycle) throws Exception
    {
        ...
        for(int i=0;i<results.size();i++){
        	...
        	if((newProductId.equals(result.getString("PARA_CODE1")))&&(packageId.equals(result.getString("PARA_CODE5")))){
                ...
                //danglt
                IDataset saleActiveCycleList = CSViewCall.call(this, "SS.WidenetMoveSVC.getSaleActiveCycle", data);
                if (IDataUtil.isNotEmpty(saleActiveCycleList)) {
                    idata.put("NEW_SALE_CYCLE", saleActiveCycleList.first().getString("PARA_CODE4"));
                }
    ...
    public void getSaleActiveDesc2(IRequestCycle cycle) throws Exception
    {
        IData data = getData();
        String saleActiveFee = "0";
        String newProductId = data.getString("NEW_PRODUCT_ID");
        ...
        for(int i=0;i<results.size();i++){
        	IData result = results.getData(i);
        	if(("FTTH".equals(result.getString("PARA_CODE1")))&&(packageId.equals(result.getString("PARA_CODE5")))){
        		resus.add(result);
        		idata.put("ACTIVE_DESC", result.getString("PARA_CODE24"));
                idata.put("PRODUCT_ID", result.getString("PARA_CODE4"));
                idata.put("CAPM_TYPE", result.getString("PARA_CODE6"));
                ...
                if (IDataUtil.isNotEmpty(activeFee)){
                    ...
                    if(depositFee>0){
                    	...
                    	String hintInfo = "";
                    	if(depositFee>Integer.parseInt(cash)){
                    		hintInfo = "您选择办理宽带调测费活动，需要冻结预存"+depositFee/100+"元，您当前余额为"+(Integer.parseInt(cash))/100+"，请提前缴纳预存！";
                    	}else{
                    		hintInfo = "您选择办理宽带调测费活动，需要冻结预存"+depositFee/100+"元";
                    	}
                        idata.put("HINT_INFO", hintInfo);
                    }
                    saleActiveFee = "" + Integer.parseInt(activeFee.getData(0).getString("SALE_ACTIVE_FEE", "0"))/100;
        ...
        setSaleActiveFee2(saleActiveFee);
        IDataset dataset = new DatasetList();
        dataset.add(idata);
        setAjax(dataset);
    }
     ...
     * @throws Exception
     */
    public void chooseMigrateActivity(IRequestCycle cycle) throws Exception{
        IData data = getData();
        IData params=new DataMap();
        params.put("DEVICE_CODE", data.getString("DEVICE_ID"));
        params.put("REMOVE_TAG", "0");
        IDataset dataset = CSViewCall.call(this,"SS.GardenDeviceInfoSVC.qryGardenDeviceInfo", params);
    ...
    public void checkSPAMDiscntRequirement(IRequestCycle cycle) throws Exception
    {
        IData data = getData();
        if ("".equals(data.getString("SERIAL_NUMBER", "")))
        {
        ...
        IDataset dataset = CSViewCall.call(this, "SS.WidenetMoveSVC.checkSPAMDiscntRequirement", data);
    ...
    public abstract void setSaleActiveList2(IDataset saleActiveList);
    public abstract void setSaleActiveFee2(String saleActiveFee);
