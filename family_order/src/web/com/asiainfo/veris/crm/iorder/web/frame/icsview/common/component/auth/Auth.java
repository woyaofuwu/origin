...
package com.asiainfo.veris.crm.iorder.web.frame.icsview.common.component.auth;
...
//        }
        ...
        //add by zhangxing3 for 海南移动重保-互联网暴露资产漏洞
        String resultCode = authData.getString("RESULT_CODE","");
        if("330".equals(data.getString("TRADE_TYPE_CODE","")) && !"".equals(resultCode))
        {
        	String systime = SysDateMgr.getSysDateYYYYMMDDHHMMSS();
        	authData.put("CHECKPASSTAG",desObj.setEncPwd(resultCode+systime));
        	authData.put("CHECKPASSKEY",systime);
...
//        }
       //根据TRADE_TYPE_CODE进行拦截配置 0 不拦截  默认   1拦截，
        ...
        IData userInfo = infos.getData(0).getData("USER_INFO");
        IData custInfo = infos.getData(0).getData("CUST_INFO");
        String productName = UpcViewCall.queryOfferNameByOfferId(this, "P", userInfo.getString("PRODUCT_ID"));
        String codesetName = StaticUtil.getStaticValue("USER_STATE_CODESET",userInfo.getString("USER_STATE_CODESET"));
        IData allStopIData = new DataMap();
        allStopIData.put("USER_ID",userInfo.getString("USER_ID"));
        if("5".equals(userInfo.getString("USER_STATE_CODESET"))) {
            IDataset allStop  = CSViewCall.call(this, "SS.GetUser360ViewSVC.qryUserIfAllStop", data);
            if(DataUtils.isNotEmpty(allStop)) {
                codesetName = "非实名制全停";
            }
        }
		userInfo.put("PRODUCT_NAME", productName);
		userInfo.put("USER_STATE_CODESET_NAME", codesetName);
		String userId=userInfo.getString("USER_ID");
        String custId=userInfo.getString("CUST_ID");
    ...
    public void renderComponent(StringBuilder infoParamsBuilder, IMarkupWriter writer, IRequestCycle cycle) throws Exception
    {
        ...
        if (null == action || "".equals(action))
        {
             ...
             * 组件初始化载入认证的脚本
             */
            getPage().addResAfterBodyBegin("scripts/iorder/icsserv/component/auth/Auth.js");
            getPage().addResAfterBodyBegin("scripts/iorder/icsserv/component/person/CommLib.js");
            getPage().addResAfterBodyBegin("scripts/iorder/icsserv/common/tradecheck/TradeCheck.js");
            getPage().addResAfterBodyBegin("scripts/iorder/icsserv/component/fee/FeeMgr.js");
            getPage().addResAfterBodyBegin("scripts/iorder/icsserv/common/developstaff/DevelopStaff.js");
        }
        else
        {
