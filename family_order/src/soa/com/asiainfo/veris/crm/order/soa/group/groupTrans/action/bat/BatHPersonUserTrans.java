...
public class BatHPersonUserTrans implements ITrans{
	
	private static final Logger log = Logger.getLogger(BatHPersonUserTrans.class);
    ...
    protected void checkRequestDataSub(IData batData) throws Exception
    {
        ...
        if(batchOperType.equals("HPERSONUSERREG_SPE")){
        ...
        if (IDataUtil.isEmpty(userGrpInfo)){// 网外号码（异网号码未开户）
    	...
    	//排除内网和校园用户，异网和校园用户是（H+电话号码）,只能异网号码导入
    	...
    	if(batData.get("SERIAL_NUMBER").toString().trim().startsWith("H")){
    		...
    		//非异网成员不能导入
    		CSAppException.apperr(GrpException.CRM_GRP_916);
    	}
        ...
        if(!StringUtils.isBlank(batData.getString("DATA2")))
        {
            ...
            //checkStudentInfoforHP(httpData);
        }
    ...
    public void checkStudentInfoforHP(IData httpData)throws Exception
    {
        ...
        log.info("test_guonj_BatNewXxtUserTrans1_start");
        log.info("test_guonj_BatNewXxtUserTrans1_param"+httpStr);
        IDataset result = GroupBatTransUtil.checkStuInfoForXXTPlat(httpStr);
        log.info("test_guonj_BatNewXxtUserTrans1_end");
        log.info("test_guonj_BatNewXxtUserTrans1_result="+httpData.getString("STU_NAME")+httpData.getString("MOB_NUM")+"."+result);
...
//        }
    ...
    protected void builderSvcData(IData batData) throws Exception
    {
        ...
        String discnt3 = batData.getString("DATA7");
        String discnt4="";
        if(("HPERSONUSERREG_SPE").equals(svcData.getString("BATCH_OPER_TYPE")))
        {
    ...
    protected void setSVC(IData batData) throws Exception
    {
        ...
        System.out.println("转换后的svcData："+svcData.toString());
    }
}
