...
public class ChangeBBossMerchPUserTrade extends ChangeUserElement
{
    ...
    public void actTradePrdParams() throws Exception
    {
        ...
        for (int i = 0; i < params.size(); i++)
        {
            ...
            else if (GroupBaseConst.PRODUCT_ATTR_STATUS_DESC.ATTR_DEL.getValue().equals(state))
            {
                ...
                if (!isExistAddAttrInfo(paramCode, params) && StringUtils.isEmpty(attrGroup) && "P".equals(instParam.getString("INST_TYPE")))
                {
    ...
    public void actTradeSub() throws Exception
    {
        ...
        if (GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE.getValue().equals(merchpOperType) || // 产品暂停
                GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PREDESTORY.getValue().equals(merchpOperType) || // 产品预取消
                GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_CONTINUE.getValue().equals(merchpOperType) || // 产品恢复
                GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_CANCLEPREDESTORY.getValue().equals(merchpOperType)|| //预取消恢复
                GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE_MEBFLUX.getValue().equals(merchpOperType)||    //暂停
                GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_CONTINUE_MEBFLUX.getValue().equals(merchpOperType))// 恢复
        {
            // 修改用户服务状态表
    ...
    public void infoRegDataEntireMerchP(String merchpOperType) throws Exception
    {
        ...
        // 1-2 根据用户编号获取产品信息
        IDataset products = UserGrpMerchpInfoQry.qryMerchpInfoByUserIdMerchSpecProductSpecStatus(userId, null, null, null, null);
        ...
        // 1-5 修改BBOSS侧产品表数据
        this.addTradeGrpMerchp(IDataUtil.idToIds(grpMerchPInfo));
    }
    ...
    protected void infoRegDataMerchPDct(IDataset productDcts, String productOrderId) throws Exception
    {
        ...
        for (int i = 0; i < productDcts.size(); i++)
        {
            ...
            String merchpSpecCode = GrpCommonBean.productToMerch(productId, 0);// 产品规格编号
            String productDiscntCode = GrpCommonBean.productToMerch(productDct.getString("DISCNT_CODE"), 1);// BBOSS产品优惠编码
