...
package com.asiainfo.veris.crm.order.soa.person.busi.changeuserinfo.order.trade;
 ...
 * @author liutt
 */
public class ModifyUserInfoTrade extends BaseTrade implements ITrade
{
    ...
    public void createBusiTradeData(BusiTradeData btd) throws Exception
    {
        ...
        // 如果担保客户信息不为空且不存在，则需要新增tf_f_customer 和tf_f_cust_person
    ...
    {
        ModifyUserInfoReqData reqData = (ModifyUserInfoReqData) btd.getRD();
    ...
    private IData getAssureCustInfo(BusiTradeData btd) throws Exception
    {
        ...
        if (StringUtils.isNotBlank(reqData.getAssurePsptTypeCode()) && StringUtils.isNotBlank(reqData.getAssurePsptId()) && StringUtils.isNotBlank(reqData.getAssureName()))
        {
            ...
            {
                for (int i = 0, size = modifyDataset.size(); i < size; i++)
                {
                    IData data = modifyDataset.getData(i);
                    if (data.getString("CUST_NAME").equals(reqData.getAssureName()))
                    {
                        seqCustId = data.getString("CUST_ID");
                        break;
                    }
                    else
                    {
