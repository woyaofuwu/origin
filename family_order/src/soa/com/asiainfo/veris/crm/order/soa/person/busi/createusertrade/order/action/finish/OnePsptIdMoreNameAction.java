 ...
 * @author: liquan
 */
public class OnePsptIdMoreNameAction implements ITradeFinishAction {
    ...
    public void executeAction(IData mainTrade) throws Exception
    {
        ...
        if (action) {
            ...
            /* 
             * 1.检查tf_f_cust_person，看开户人，在给定品牌下是否有一证多名的数据
             *   如存在，存量开户人：下发短信，变更实名制，走半停全停流程，发送客户资料变更工单，将设置为非实名制
             * 
             * 2.检查 TF_F_USER_PSPT，看使用人、经办人、责任人，在给定品牌下是否有一证多名的数据
            ...
            if (DataSetUtils.isNotBlank(custpersonDs)) {
                ...
                if (PsptType.equals("0") || PsptType.equals("1") || PsptType.equals("2")|| PsptType.equals("3")) {//本地、外地身份证、户口
                    ...
                    if (custName.trim().length() > 0 && psptId.trim().length() > 0) {
                        insertCustomer(mainTrade, custName, psptId);
                ...
                if (usePsptType.equals("0") || usePsptType.equals("1") || usePsptType.equals("2")|| usePsptType.equals("3")) {//本地、外地身份证、户口
                    ...
                    if (useName.trim().length() > 0 && usePsptid.trim().length() > 0) {
                        insertCustomer(mainTrade, useName, usePsptid);
            ...
            if (DataSetUtils.isNotBlank(customerDs)) {
                ...
                if (agentPsptType.equals("0") || agentPsptType.equals("1") || agentPsptType.equals("2")|| agentPsptType.equals("3")) {//本地、外地身份证、户口
                    ...
                    if (agentName.trim().length() > 0 && agentPsptid.trim().length() > 0) {
                        insertCustomer(mainTrade, agentName, agentPsptid);
            ...
            //责任人
            IDataset personOtherDs = CustPersonInfoQry.qryPerInfoByPsptId_2(tradeId);
            if (DataSetUtils.isNotBlank(personOtherDs)) {
                IData personOtherData = personOtherDs.getData(0);
                String zrrPsptType = personOtherData.getString("RSRV_STR3", "");
                if (zrrPsptType.equals("0") || zrrPsptType.equals("1") || zrrPsptType.equals("2") || zrrPsptType.equals("3")) {//本地、外地身份证、户口
                    String zrrName = personOtherData.getString("RSRV_STR2", "");
                    String zrrPsptid = personOtherData.getString("RSRV_STR4", "");
                    if (zrrName.trim().length() > 0 && zrrPsptid.trim().length() > 0) {
                        insertCustomer(mainTrade, zrrName, zrrPsptid);
                       // updateUseAgentZrr(mainTrade, zrrName, zrrPsptid);
                    }
                }
            }
        }
    ...
    private void insertCustomer(IData mainTrade, String custName, String psptId) throws Exception
    {
        IDataset ds = CustPersonInfoQry.qryPerInfoByPsptId_1(psptId, custName);
        if (DataSetUtils.isNotBlank(ds)) {
            ...
            for (int i = 0; i < ds.size(); i++) {
                ...
                deal.put("DEAL_COND", "{\"SERIAL_NUMBER\":\""+data.getString("SERIAL_NUMBER")+"\"}");
