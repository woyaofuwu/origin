...
public class OpenOneKeyBean extends CSBizBean
{
    ...
    public IDataset crtBat(IData inParam) throws Exception
    {
        ...
        mebBatData.put("BATCH_OPER_TYPE", "OPENGROUPKDMEM");
        mebBatData.put("BATCH_TASK_NAME", "一键恢复集团下所有的商务宽带成员");
        ...
        if (IDataUtil.isNotEmpty(relaList))
        {
            for (int i = 0, row = relaList.size(); i < row; i++)
            {
                ...
                if (IDataUtil.isNotEmpty(userinfos))
                {
                    ...
                    if(!StringUtils.equals("1", userStateCode) && !StringUtils.equals("01", userStateCode))
                    {
                        continue;
                    }
        ...
        grpBatData.put("BATCH_OPER_TYPE", "OPENGROUPKDUSER");
        grpBatData.put("BATCH_TASK_NAME", "恢复集团商务宽带用户");
