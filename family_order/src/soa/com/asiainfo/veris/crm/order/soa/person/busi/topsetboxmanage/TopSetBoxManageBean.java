...
public class TopSetBoxManageBean extends CSBizBean{
	 private static Logger logger = Logger.getLogger(TopSetBoxManageBean.class);
	   ...
	   public void checkThreeMonthNotReturnTopSetBoxUser(IData inParam) throws Exception{
		   IData dealParam = new DataMap(inParam.getString("DEAL_COND"));
			...
			try{		
				...
				params.put("REMARK", "魔百和业务取消3个月未归还机顶盒，押金进行沉淀！");
				...
				errParam.put("CALL_INTEFACE", "SS.TopSetBoxManageSVC.checkThreeMonthNotReturnTopSetBoxUser错误。");
	   ...
	   public void checkThreeYearsTopSetBoxUser(IData inParam) throws Exception
	   {
			...
			if(fee > 0){
				...
				try{
					...
					errParam.put("USER_ID", userId);
					errParam.put("INST_ID", instId);
					errParam.put("RESULT_INFO", "SS.TopSetBoxManageSVC.checkThreeYearsTopSetBoxUser错误。");
					this.updateUserResCallFalse(errParam);
   ...
   /**
	...
	public static void updateUserResTopSetBoxStatus(IData inParams) throws Exception
	{
    	...
    	StringBuilder sql = new StringBuilder(1000);
    	sql.append(" update TF_F_USER_RES t ");
    	sql.append(" set t.rsrv_num2 = '0',t.end_date=sysdate,t.UPDATE_TIME=sysdate,t.remark='魔百和业务取消满3个月未归还机顶盒押金沉淀'");
    	sql.append(" where  t.rsrv_tag1 = 'J' "); 
    	sql.append(" and t.res_type_code= '4' ");
    	sql.append(" and sysdate between t.start_date and t.end_date ");
    	sql.append(" and t.user_id=:USER_ID ");
	...
	public static void updateUserResDeposit(IData inParams) throws Exception
	{
    	...
    	StringBuilder sql = new StringBuilder(1000);
    	sql.append(" update TF_F_USER_RES t ");
    	sql.append(" set t.rsrv_num2 = '0',t.UPDATE_TIME=sysdate,t.remark='魔百和机顶盒申领满三年，退还押金'");
    	sql.append(" where  t.rsrv_tag1 = 'J' "); 
    	sql.append(" and t.res_type_code= '4' ");
    	sql.append(" and sysdate between t.start_date and t.end_date ");
    	sql.append(" and t.user_id=:USER_ID ");
	 ...
	 * zhangxing3
	 * */
	 public IDataset stopTopsetboxService(IData input) throws Exception
	{
         IData dealParam = new DataMap(input.getString("DEAL_COND"));
         dealParam.put("IN_MODE_CODE", "0");
         dealParam.put("ROUTE_EPARCHY_CODE", "0898");
         dealParam.put("TRADE_TYPE_CODE", "3900");
         dealParam.put("DJKD_TAG", "1");
    	 return CSAppCall.call("SS.StopTopSetBoxRegSVC.tradeReg", dealParam);           
	}
}
