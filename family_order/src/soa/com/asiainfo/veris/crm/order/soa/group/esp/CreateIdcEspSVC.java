...
public class CreateIdcEspSVC extends GroupOrderService {
	public IDataset createUserIdcOrder(IData map) throws Exception
    {
...
//        		createSerialNumber(groupId,productId,user.getString("EPARCHY_CODE"));
      ...
      //  inparam.put("PRODUCT_PARAM_INFO", IDataUtil.idToIds(productParamInfo));
        ...
        for (int w=0,wsize=workformAttrList.size();w<wsize;w++){
        	...
        	if("IDC_Price".equals(workformAttr.getString("ATTR_CODE"))){
    			param.put(IdcDiscntConst.discntElementId_price,workformAttr.getString("ATTR_VALUE"));
			}else if("IDC_Discount".equals(workformAttr.getString("ATTR_CODE"))){
				param.put(IdcDiscntConst.discntElementId_discount,workformAttr.getString("ATTR_VALUE"));
			}else if(IDataUtil.isNotEmpty(workformAttr)&&workformAttr.getString("ATTR_CODE", "").indexOf("IDC_")>=0){
       ...
       // param.putAll(DatalineEspUtil.getCommInfo(ibsysId));
        ...
        if("7041".equals(productId)){
            ...
            for (int w=0,wsize=workformAttr7041List.size();w<wsize;w++){
            	...
            	if(IDataUtil.isNotEmpty(workformAttr7041)&&workformAttr7041.getString("ATTR_CODE", "").indexOf("IDC_")>=0){
//                    param.put(workformAttr7041.getString("ATTR_CODE"), workformAttr7041.getString("ATTR_VALUE"));
                    ...
                    if(recordNum.matches("^[0-9]*$")){
                    	...
                    	if(recordNumInt==0){ 
                    		if("IDC_Price".equals(workformAttr7041.getString("ATTR_CODE"))){
                    			param.put(IdcDiscntConst.discntElementId_price,workformAttr7041.getString("ATTR_VALUE"));
        					}else if("IDC_Discount".equals(workformAttr7041.getString("ATTR_CODE"))){
        						param.put(IdcDiscntConst.discntElementId_discount,workformAttr7041.getString("ATTR_VALUE"));
        					}else{
                        		param.put(workformAttr7041.getString("ATTR_CODE"), workformAttr7041.getString("ATTR_VALUE"));
        					}
                    		continue;
                		...
                		if(applyConfirmList.size()<recordNumInt){
                			for(int size=applyConfirmList.size();size<recordNumInt;size++){
                				applyConfirmList.add(new DataMap());
                			}
                		}
       ...
       //必选信息填入 begin 
        /*String price=param.getString(IdcDiscntConst.discntElementId_price,"0");
        String discount=param.getString(IdcDiscntConst.discntElementId_discount,"100");
...
//        IDataset productTypeList=StaticUtil.getStaticListByParent("IDC_PRODUCTTYPE",productId);
        String price=param.getString(IdcDiscntConst.discntElementId_price,"0");
        String discount=param.getString(IdcDiscntConst.discntElementId_discount,"100");
...
//        	if(IdcDiscntConst.discntElementId.equals(selectedElement.getString("ELEMENT_ID", ""))){
        		if(selectedElement.get("ATTR_PARAM")!=null){
                    ...
                    for (int p=0,psize=selectedElementsParams.size();p<psize;p++){
                    	IData selectedElementsParam=selectedElementsParams.getData(p);
//                    	 for (int q=0,qsize=productTypeList.size();q<qsize;q++){
//                         	IData productTypeListData=productTypeList.getData(q);
                        	if(param.getString("IDC_ProductType").equals(selectedElementsParam.getString("ATTR_CODE", ""))){//费用
                        		selectedElementsParam.put("ATTR_VALUE", price);
                        	}
//                    	 }
                    	if(IdcDiscntConst.discntElementId_discount.equals(selectedElementsParam.getString("ATTR_CODE", ""))){//折扣
                    		selectedElementsParam.put("ATTR_VALUE", discount);
                    	}
                    }
        		}
//        	}
        ...
        result =CSAppCall.call("SS.CreateGroupUserIdcSVC.crtOrder", inparam);
