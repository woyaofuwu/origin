...
public class ChangeJKDTPastOfferTrade extends ChangeUserElement {
    ...
    public void actTradeSub() throws Exception {
        ...
        if (GroupBaseConst.MERCH_STATUS.MERCH_PASTE.getValue().equals(merchOperType) || // 商品品暂停
                GroupBaseConst.MERCH_STATUS.MERCH_CONTINUE.getValue().equals(merchOperType))// 商品品恢复
        {
            // 修改用户服务状态表
            this.processSvcState(merchOperType);
        }
    ...
    public void infoRegDataEntireMerch(String merchOperType) throws Exception {
        ...
        merchInfo.put("PAY_MODE", "2");
        merchInfo.put("RSRV_TAG1", "2");//暂停下账期生效
        ...
        IData epData = GrpCommonBean.actEcrecepProcedure(reqData.getUca().getUser().getUserId(), SeqMgr.getInstId(), reqData.getUca().getUser().getSerialNumber(), merchInfo.getString("MERCH_ORDER_ID"), merchInfo.getString("MERCH_OFFER_ID", ""), merchOperType, "N", "N");
   ...
   protected void processSvcState(String merchpOperType) throws Exception {
       ...
       if (IDataUtil.isNotEmpty(reqData.getGOOD_INFO())) {
           isCredit = reqData.getGOOD_INFO().getBoolean("IS_CREDIT");// 信控暂停标志
       }
       // 2 获取用户服务表数据
       ...
       for (int i = 0; i < userSvcInfo.size(); i++) {
           ...
           for (int j = 0; j < userSvcStateInfo.size(); j++) {
               // 根据不同的产品操作类型来确定新的状态值
               String state = "0";
               if (GroupBaseConst.MERCH_STATUS.MERCH_PASTE.getValue().equals(merchpOperType)) {// 产品暂停
                   if (isCredit) {
                       state = "7";//信控发起的要改成7 高额停机 否则缴费后计费不发缴费信控开机
                   } else {
                       state = "1";//前台主动暂停的，设置为申请停机
                   }
               }

               if (GroupBaseConst.MERCH_STATUS.MERCH_CONTINUE.getValue().equals(merchpOperType)) {// 商品品恢复
                   state = "0";//正常
               }
               ...
               userSvcState.put("END_DATE", SysDateMgr.getLastSecond(getAcceptTime()));// 获取系统当前日期-1S
    ...
    {
        // 产品控制信息
        BizCtrlInfo ctrlInfo = BizCtrlBean.qryProductCtrlInfo(productId, BizCtrlType.ChangeUserPast);

        String validMethod = ctrlInfo.getAttrStr1Value("Validate");
        if(StringUtils.isEmpty(validMethod)){
        	if("CrtUs".equals(ctrlType)){
        		validMethod ="chkGrpUserOpen";
        	}
        	if("ChgUs".equals(ctrlType)){
        		validMethod ="chkGrpUserChg";
        	}
        	if("DstUs".equals(ctrlType)){
        		validMethod ="chkGrpUserDestory";
        	}
        	if("CrtMb".equals(ctrlType)){
        		validMethod ="chkGrpMebOrder";
        	}
        	if("ChgMb".equals(ctrlType)){
        		validMethod ="chkGrpMebChg";
        	}
        	if("DstMb".equals(ctrlType)){
        		validMethod ="chkGrpMebDestory";
        	}
        	
        }
        
        reqData.setValidateMethod(validMethod);
