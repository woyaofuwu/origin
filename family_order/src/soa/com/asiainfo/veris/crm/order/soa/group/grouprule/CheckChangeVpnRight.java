...
public class CheckChangeVpnRight extends BreBase implements IBREScript
{
    ...
    private static Logger logger = Logger.getLogger(CheckChangeVpnRight.class);

    public boolean run(IData databus, BreRuleParam ruleParam) throws Exception
    {

        if (logger.isDebugEnabled())
            logger.debug(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 进入 CheckChangeVpnRight() >>>>>>>>>>>>>>>>>>");
        ...
        {
            IDataset right = StaffInfoQry.queryGrpRightByIdCode(staff_id, "MODIFY_HAINMOBILE_MEB_RIGHT", null);
        ...
        {
            IDataset rightInfo1 = StaffInfoQry.queryGrpRightByIdCode(null, "MODIFY_VPN_MEMBER", strEcSN);
        ...
        bResult = true;
        if (logger.isDebugEnabled())
            logger.debug(" <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 退出 CheckChangeVpnRight() " + bResult + "<<<<<<<<<<<<<<<<<<<");
        return bResult;
