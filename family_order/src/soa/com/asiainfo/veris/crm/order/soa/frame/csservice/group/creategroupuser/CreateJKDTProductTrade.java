...
public class CreateJKDTProductTrade extends CreateGroupUser{
    ...
    private void actTradeFeeDefer() throws Exception
    {
        ...
        for (int i = 0; i < productParamInfoList.size(); i++)
        {
            ...
            onceFeeData.put("FEE_TYPE_CODE", feeTypeCode);// 费用类型编码：可以为营业费用项、押金或存折类型的编码
...
//            CSAppException.apperr(CrmCommException.CRM_COMM_579,"一次性费用配置缺失");
    ...
    {
        IDataset onceInfo = StaticUtil.getStaticList("BBOSS_ONCEFEE_NONEED");
    ...
    public void actTradePrdAndPrdParams() throws Exception
    {
        ...
        // 添加主产品信息
        productIdset.put(reqData.getUca().getProductId(), CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
                ...
                if (GroupBaseConst.PRODUCT_MODE.USER_MAIN_PRODUCT.getValue().equals(productMode))
                {
                    ...
                    if (productParam != null && productParam.size() > 0)
                    {
                        ...
                        for (int i = 0; i < productParam.size(); i++)
                        {
                            ...
                            map.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
    ...
    public void actTradeSub() throws Exception
    {
        ...
        super.addTradeEcrecepProduct(reqData.getMerchp());
    ...
    public void infoRegDataEntireMerchP() throws Exception
    {
        ...
        merchPInfo.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
        ...
        IData epData = GrpCommonBean.actEcrecepProcedure(reqData.getUca().getUser().getUserId(), SeqMgr.getInstId(), reqData.getUca().getUser().getSerialNumber(), merchPInfo.getString("PRODUCT_ORDER_ID"), merchPInfo.getString("PRODUCT_OFFER_ID", ""), "1", "A", "A");
    ...
    protected void infoRegDataOther() throws Exception
    {
        ...
        //集客大厅--企业彩铃  同步业务模式给计费 daidl
        IData modeInfo = new DataMap();
        modeInfo.put("USER_ID", reqData.getUca().getUser().getUserId());
        modeInfo.put("RSRV_VALUE_CODE", "BBOSS_BIZ_MODE");
        modeInfo.put("RSRV_VALUE", reqData.getBBOSS_PRODUCT_INFO().getString("BIZ_MODE", ""));
        modeInfo.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
        modeInfo.put("START_DATE", getAcceptTime());
        modeInfo.put("END_DATE", SysDateMgr.getTheLastTime());
        modeInfo.put("INST_ID", SeqMgr.getInstId());
        this.addTradeOther(modeInfo);
    }
    ...
    protected void infoRegDataOtherToBilling() throws Exception
    {
        ...
        if (IDataUtil.isNotEmpty(isToBilling))
        {
            ...
            otherInfo.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
...
//        }
    ...
    protected void setTradeBase() throws Exception
    {
        ...
        if(specProductId.equals("910401") && "6".equals(CSBizBean.getVisit().getInModeCode())){
        	//如果是企业视频彩铃产品，则执行时间不带时间戳
         data.put("EXEC_TIME",nowDate.format(new Date()));
