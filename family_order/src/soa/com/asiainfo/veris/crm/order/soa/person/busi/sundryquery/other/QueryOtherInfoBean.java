...
public class QueryOtherInfoBean extends CSBizBean
{
	...
	public IData queryMoreNum(IData input) throws Exception {
		...
		String custName = decrypt(IDataUtil.chkParam(data, "custCertName"),result);
		String psptId = decrypt(IDataUtil.chkParam(data, "custCertNo"),result);
...
//			callResult = getTestData(0);
				...
				if (openNum > 0)
				{
				        ...
				        if(IDataUtil.isNotEmpty(returnDataset))
				        {
				        	...
				        	for(int i = 0; i < returnDataset.size(); i++)
				        	{
				        		...
				        		resinfo.put("custName", encrypt(returnDataset.getData(i).getString("CUSTOMER_NAME"), result));
	 ...
	 */
	private String decrypt(String encryptString,IData result) throws Exception {
		String pubKey = BizEnv.getEnvString("zhongyizaixian.http.pubkey");
		String resStr = "";
		if(StringUtils.isBlank(pubKey)){
			result.put("returnCode", "2999");
	 ...
	 * @date :2017-12-11下午03:32:55
	 */
	private String encrypt(String encryptString,IData result) throws Exception {
		String pubKey = BizEnv.getEnvString("zhongyizaixian.http.pubkey");
		String resStr = "";
		if(StringUtils.isBlank(pubKey)){
			result.put("returnCode", "2999");
