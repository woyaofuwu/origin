...
public class CheckPSJBInfoSyn extends BreBase implements IBREScript
{
	private static Logger logger = Logger.getLogger(CheckPSJBInfoSyn.class);
    ...
    public boolean run(IData databus, BreRuleParam rule) throws Exception{
    	...
    	if (logger.isDebugEnabled())
            logger.debug(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 进入 CheckPSJBInfoSyn() >>>>>>>>>>>>>>>>>>");
        ...
        ibossParam.put("KIND_ID", "BIPSJB01_TX100001_0_0");
        IDataset callResult = IBossCall.dealInvokeUrl("BIPSJB01_TX100001_0_0", "IBOSS7", ibossParam);
