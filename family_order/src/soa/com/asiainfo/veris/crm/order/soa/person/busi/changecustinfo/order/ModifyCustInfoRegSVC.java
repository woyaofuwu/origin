...
package com.asiainfo.veris.crm.order.soa.person.busi.changecustinfo.order;
...
public class ModifyCustInfoRegSVC extends OrderService
{
    // 未激活用户实名制时进行 首话单激活
    private void firstCall(String userId, String serialNumber, BusiTradeData btd) throws Exception
    {
        ...
        params.put("TRADE_TYPE_CODE", btd.getTradeTypeCode());//REQ202003200037【实名制】关于手机号码实名登记激活方式优化
    ...
    {
        return this.input.getString("TRADE_TYPE_CODE", "60");
    }
    ...
    private void openMobile(String tradeId, String serialNumber) throws Exception
    {
        ...
        params.put("REMARK", "登记实名制完工流水" + tradeId);
    ...
    public void otherTradeDeal(IData idata, BusiTradeData btd) throws Exception
    {
        ...
        ModifyCustInfoReqData custInfoRD = (ModifyCustInfoReqData) btd.getRD();
        // 客户本次办理了实名制
        if (StringUtils.equals("1", custInfoRD.getIsRealName()))
        {
                    ...
                    if (psptType.equals("0") || psptType.equals("1") || psptType.equals("3") || psptType.equals("A")) {//本地外地户口护照军人
                        ...
                        if (IDataUtil.isNotEmpty(globalLimitResult)) {
                            ...
                            if (IDataUtil.isNotEmpty(nationalOpenLimit)) {
								...
								if (untrustresult > 0) {
									CSAppException.apperr(CrmCommException.CRM_COMM_103, "开户人有不良信息，不满足开户条件，禁止开户");
								}
            ...
            if("AT".equals(userStateCodeset_now)||"4".equals(userStateCodeset)){
            	stopUserOpenMobile(tradeId, serialNumber);
            }else{
	            if ((StringUtils.equals("1", custInfoRD.getUca().getUser().getRsrvNum1())&&!"MOSP".equals(custInfoRD.getUca().getBrandCode())))
	            {//一卡多号虚拟副号不做 
	                this.openMobile(tradeId, serialNumber);
	            }
            }
            
            if (StringUtils.equals("60", input.getString("TRADE_TYPE_CODE")))
            {// 一卡多号虚拟副号不做激活  REQ201606270002 非实名用户关停改造需求 chenxy3 20160629 非实名制局方停机不再激活
                ...
                {
//                	String psptTypeCode = custInfoRD.getUca().getCustomer().getPsptTypeCode();
...
//                	}
                ...
                //信用购机增加=================================
                if("1".equals(idata.getString("CREDIT_PAY_TAG"))){
                	 IData saleactiveData = new DataMap();
                     saleactiveData.put("SERIAL_NUMBER",idata.getString("SERIAL_NUMBER"));
                     saleactiveData.put("PRODUCT_ID",idata.getString("CREDIT_PRODUCT_ID"));
                     saleactiveData.put("PACKAGE_ID", idata.getString("CREDIT_PACKAGE_ID"));
                     CSAppCall.call("SS.SaleActiveRegSVC.tradeReg4Intf", saleactiveData);
                }
            }
        }
