...
public class ChangeJKDTBaseParamProductTrade extends ChangeUserElement
{
	 private static final Logger logger = LoggerFactory.getLogger(ChangeJKDTBaseParamProductTrade.class);
    ...
    public void actTradePrdParams() throws Exception
    {
        ...
        //账期生效规则修改 --add by huangzl3
        IData bbossProductInfoData = reqData.getBBOSS_PRODUCT_INFO();
        String poRatePolicyEffRule = "1";//套餐生效规则
        ...
        logger.debug(">>>>>>>>>>>>>>>>>>PRODUCT_PARAM DATA>>>>>>>>>"+params);
        ...
        if(!"6".equals(CSBizBean.getVisit().getInModeCode())){
        	 ...
        	 attrMap.put("STATE", "ADD");
             attrMap.put("ATTR_CODE", "9005");
             attrMap.put("ATTR_VALUE", "变更");
             attrMap.put("ATTR_GROUP", "");
             attrMap.put("ATTR_NAME", "变更原因");
             params.add(attrMap);
        }
        	
        
             logger.info(">>>>>>>>>>>>>>>>>>PRODUCT_PARAM DATA>>>>>>>>>"+params);
        ...
        for (int i = 0; i < params.size(); i++)
        {
            ...
            if (GroupBaseConst.PRODUCT_ATTR_STATUS_DESC.ATTR_ADD.getValue().equals(state))
            {
                ...
                addAttrTradeInfo(poRatePolicyEffRule,paramCode, paramName, paraValue, attrGroup, modifyTag, regProductParamInfos);
                continue;
            }
            else if (GroupBaseConst.PRODUCT_ATTR_STATUS_DESC.ATTR_DEL.getValue().equals(state))
            {
                ...
                //生失效规则 --add  by huangzl3
                if("2".equals(poRatePolicyEffRule)){ 
                	//下账期生效，本月最后一天失效                	
                	instParam.put("END_DATE", SysDateMgr.getLastDateThisMonth());               
                }else if("4".equals(poRatePolicyEffRule)){	//今天23：59：59失效
                	instParam.put("END_DATE", SysDateMgr.getSysDate()+" 23:59:59");               	 
                }else {	
                ...
                {
                    addAttrTradeInfo(poRatePolicyEffRule, paramCode, paramName, "", attrGroup, "F", regProductParamInfos);
                }
            }
        }
     ...
     * @date 2013-10-24
     */
    protected void addAttrTradeInfo(String poRatePolicyEffRule, String paramCode, String paramName, String paramValue, String attrGroup, String modifyTag, IDataset regProductParamInfos) throws Exception
    {
        // 1- 定义参数对象
        ...
        //修改生效规则 -- add by huangzl3
        if("2".equals(poRatePolicyEffRule)){ 
        	//下账期生效，下月第一天生效
        	newParam.put("START_DATE", SysDateMgr.getFirstDayOfNextMonth());
        }else if("4".equals(poRatePolicyEffRule)){
        	//下一天生效
        	newParam.put("START_DATE", SysDateMgr.getTomorrowDate());
        }else {
    ...
    public void infoRegDataEntireMerchP(String merchpOperType) throws Exception
    {
        ...
        if (merchpOperType.equals(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_CONTINUE.getValue()) ||
                merchpOperType.equals(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_CONTINUE_MEBFLUX.getValue())	)// 产品恢复
        {
            grpMerchPInfo.put("STATUS", "A");
        }
        else if (merchpOperType.equals(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_CANCLEPREDESTORY.getValue()))// 预取消恢复
        {
            grpMerchPInfo.put("STATUS", "A");
        }
        else if (merchpOperType.equals(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE.getValue()) ||
                merchpOperType.equals(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE_MEBFLUX.getValue()))// 产品暂停
        {
            grpMerchPInfo.put("STATUS", "N");
        }
        else if (merchpOperType.equals(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PREDESTORY.getValue())) // 产品预取消
        {
            grpMerchPInfo.put("STATUS", "D"); // D 预销
        }
    ...
    protected void infoRegDataMerchPDct(IDataset productDcts, String productOrderId) throws Exception
    {
        ...
        for (int i = 0; i < productDcts.size(); i++)
        {
            ...
            IData bbossProductInfoData = reqData.getBBOSS_PRODUCT_INFO();
            String jkpoRatePolicyEffRule = "1";//套餐生效规则
            if(IDataUtil.isNotEmpty(bbossProductInfoData)){
            	jkpoRatePolicyEffRule = bbossProductInfoData.getString("PO_RATE_POLICY_EFF_RULE","1");
            }  
            logger.debug("=======jk==========reqData.getBBOSS_PRODUCT_INFO========="+bbossProductInfoData);
            logger.debug("======jk======  jkpoRatePolicyEffRule  //======="+jkpoRatePolicyEffRule);
            ...
            if (productDct.getString("MODIFY_TAG").equals("0") && !"".equals(productDiscntCode))
            {
                ...
                if("2".equals(jkpoRatePolicyEffRule)){ 
                	//下账期生效，下月第一天生效
                	productDct.put("START_DATE", SysDateMgr.getFirstDayOfNextMonth());
                }else if("4".equals(jkpoRatePolicyEffRule)){
                	//下一天生效
                	productDct.put("START_DATE", SysDateMgr.getTomorrowDate());
                	logger.debug("============  getTomorrowDate()   ======="+SysDateMgr.getTomorrowDate());
                }else {
                	//poRatePolicyEffRule为1，立即生效
            ...
            {
            	 IDataset datas = UserGrpMerchDiscntInfoQry.getJKDTUserProductDiscnt(userId, merchSpecCode, merchpSpecCode,"1", productDiscntCode, null);
                 if (datas != null && datas.size() > 0)// 一般来说，查询出的数据只有一条
                 {
                     IData data = datas.getData(0);
                     data.put("MODIFY_TAG", "1");
                     
                     if("2".equals(jkpoRatePolicyEffRule)){ //下账期生效，本月最后一天失效
                    	 data.put("END_DATE", SysDateMgr.getLastDateThisMonth());
                     }else if("4".equals(jkpoRatePolicyEffRule)){	//今天23：59：59失效
...
//        }
    ...
    protected void makReqData(IData map) throws Exception
    {
        ...
        modifyDiscntForPayMod(map);
        modifyElementParamForPayMod(map);
    ...
    /**
     * @description 根据套餐生效规则来处理资费生失效时间
     * @author daidl
     * @date 2019-03-30
     */
    protected void modifyDiscntForPayMod(IData map) throws Exception
    {
    	String newPayMode =map.getString("PO_RATE_POLICY_EFF_RULE");//套餐生效规则
    	IDataset disDataset = reqData.cd.getDiscnt();
    	if(logger.isDebugEnabled()){
    		logger.debug(">>>>>>>>>>>>>>>>>>>disDataset DATA>>>>>>>>>"+disDataset);
    		logger.debug(">>>>>>>>>>>>>>>>>>>mapt DATA>>>>>>>>>"+map);
    	}
        ...
        if (IDataUtil.isNotEmpty(disDataset))
        {
            for (int i = disDataset.size() - 1; i >= 0; i--)
            {
                ...
                if (TRADE_MODIFY_TAG.Add.getValue().equals(disData.getString("MODIFY_TAG")))
                {
                    if("4".equals(newPayMode) || newPayMode=="4"){
                    	
                    	reqData.cd.getDiscnt().getData(i).put("START_DTAE", SysDateMgr.getTomorrowDate());
                    }
                    else if("2".equals(newPayMode))
            		{
                    	reqData.cd.getDiscnt().getData(i).put("START_DTAE", SysDateMgr.getFirstDayOfNextMonth());
            		}
            		else
            		{
            			reqData.cd.getDiscnt().getData(i).put("START_DTAE", SysDateMgr.getSysTime());
            		}
                }
                else if(TRADE_MODIFY_TAG.DEL.getValue().equals(disData.getString("MODIFY_TAG"))){
                	
                	if("4".equals(newPayMode) || newPayMode=="4"){
                    	
                		reqData.cd.getDiscnt().getData(i).put("END_DATE", SysDateMgr.getSysDate()+" 23:59:59");
                    }
                	else if("2".equals(newPayMode))
            		{
                		reqData.cd.getDiscnt().getData(i).put("END_DATE", SysDateMgr.getLastDateThisMonth());
            		}
            		else
            		{
            			reqData.cd.getDiscnt().getData(i).put("END_DATE", SysDateMgr.getSysTime());
            		}
                }
            }
        }
    ...
    /**
     * @description 根据套餐生效规则来处理trade_attr表的资费生失效时间
     * @author daidl
     * @date 2019-03-30
     */
    protected void modifyElementParamForPayMod(IData map) throws Exception
    {
    	String newPayMode = map.getString("PO_RATE_POLICY_EFF_RULE","");
    	IDataset elmentParam = reqData.cd.getElementParam();
    	if(logger.isDebugEnabled()){
    		logger.debug(">>>>>>>>>>>>>>>>>>>elmentParam DATA>>>>>>>>>"+elmentParam);
    		logger.debug(">>>>>>>ddd>>>>>>>>>>>>map DATA>>>>>>>>>"+map);
    	}
    	
        if (IDataUtil.isNotEmpty(elmentParam))
        {
            for (int i = elmentParam.size() - 1; i >= 0; i--)
            {
                IData disData = elmentParam.getData(i);

                if (TRADE_MODIFY_TAG.Add.getValue().equals(disData.getString("MODIFY_TAG")))
                {
                    if("4".equals(newPayMode) || newPayMode=="4"){
                    	
                    	reqData.cd.getElementParam().getData(i).put("START_DTAE", SysDateMgr.getTomorrowDate());
                    }
                    else if("2".equals(newPayMode))
            		{
                    	reqData.cd.getElementParam().getData(i).put("START_DTAE", SysDateMgr.getFirstDayOfNextMonth());
            		}
            		else
            		{
            			reqData.cd.getElementParam().getData(i).put("START_DTAE", SysDateMgr.getSysTime());
            		}
                }
                else if(TRADE_MODIFY_TAG.DEL.getValue().equals(disData.getString("MODIFY_TAG"))){
                	...
                	if("4".equals(newPayMode) || newPayMode=="4"){
                		...
                		reqData.cd.getElementParam().getData(i).put("END_DATE", SysDateMgr.getSysDate()+" 23:59:59");
                    }
                	else if("2".equals(newPayMode))
            		{
                		reqData.cd.getElementParam().getData(i).put("END_DATE", SysDateMgr.getLastDateThisMonth());
            		}
            		else
            		{
            			reqData.cd.getElementParam().getData(i).put("END_DATE", SysDateMgr.getSysTime());
            		}
                }
            }
        }
    ...
    protected void init() throws Exception
    {
    	...
    	String merchpOperType = reqData.getMerchpOperType();//产品操作类别
    	
    	String ctrlType = BizCtrlType.ChangeUserDis;
        
    	if(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE.getValue().equals(merchpOperType)){
    		ctrlType = BizCtrlType.ChangeUserPast;
    	}else if(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_CONTINUE.getValue().equals(merchpOperType)){
    		ctrlType = BizCtrlType.ChangeUserContinue;
    	}else if(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_MODIFY_DISCNT.getValue().equals(merchpOperType)){
    		ctrlType = BizCtrlType.ChangeUserChangeDiscnt;
    	}else if(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PREDESTORY.getValue().equals(merchpOperType)){
    		ctrlType = BizCtrlType.ChangeUserPreDel;
    	}else if(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_CANCLEPREDESTORY.getValue().equals(merchpOperType)){
    		ctrlType = BizCtrlType.ChangeUserCanclePreDel;
    	}else if(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE_MEBFLUX.getValue().equals(merchpOperType)){
    		ctrlType = BizCtrlType.ChangeUserPastMebFlux;
    	}else if(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_CONTINUE_MEBFLUX.getValue().equals(merchpOperType)){
    		ctrlType = BizCtrlType.ChangeUserContinueMebFlux;
    	}else if(GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_MODIFY_PARAM.getValue().equals(merchpOperType)){
