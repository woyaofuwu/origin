...
public class HuntGrpAddBean extends MemberBean
{
    boolean ifCentrex = false; // 是否融合网

    protected HuntGrpAddBeanReqData reqData = null;
    ...
    {
        return new HuntGrpAddBeanReqData();
    }
    ...
    public void infoRegDataRelation() throws Exception
    {
        ...
        rela.put("RELATION_TYPE_CODE", "XH"); // 寻呼组业务
        rela.put("USER_ID_A", reqData.getGrpUca().getUserId()); // 集团user_id
        rela.put("SERIAL_NUMBER_A", reqData.getGrpUca().getSerialNumber()); // 集团sn
        rela.put("USER_ID_B", reqData.getUca().getUserId()); // 寻呼组user_id
        rela.put("SERIAL_NUMBER_B", reqData.getUca().getSerialNumber()); // 寻呼组sn
        ...
        rela.put("RSRV_STR1", reqData.getTEAM_TYPE()); // 寻呼组类型
    ...
    {
        IData userData = new DataMap(); // 寻呼组用户信息(虚拟用户)

        userData.put("USER_ID", reqData.getUca().getUserId()); // 寻呼组user_id
        userData.put("SERIAL_NUMBER", reqData.getUca().getSerialNumber()); // 寻呼组sn

        userData.put("CUST_ID", reqData.getUca().getUser().getCustId()); // 寻呼组客户
        userData.put("USECUST_ID", "-1");

        userData.put("IN_DATE", SysDateMgr.getSysTime());
        ...
        userData.put("RSRV_STR1", reqData.getTEAM_TYPE()); // 寻呼组类型
        userData.put("RSRV_STR4", "0"); // 组类型- 0：寻呼组 1：代答组 2:闭合用户群组
    ...
    protected void initReqData() throws Exception
    {
        ...
        reqData = (HuntGrpAddBeanReqData) getBaseReqData();
    }
    ...
    protected void makReqData(IData map) throws Exception
    {
        ...
        reqData.setTEAM_TYPE(map.getString("RSRV_STR1")); // 寻呼组类型 0：优先组 ；1：轮选组
    }
    ...
    protected void regTrade() throws Exception
    {
        ...
        data.put("RSRV_STR1", reqData.getTEAM_TYPE()); // 寻呼组类型：优先组、轮选组
