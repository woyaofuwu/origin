...
public class CustManagerInfoQry
{
    ...
    public static IDataset qryVpmnStaffInfo(IData param, Pagination pg) throws Exception
    { 
    	
    	//return Dao.qryByCodeParser("TD_M_STAFF", "SEL_VPMN_STAFF", param, pg, Route.CONN_CRM_CG);
    	String area_code = (String)param.get("AREA_CODE");
        String dept_id = (String)param.get("DEPART_ID");
        String need_stafftag = param.getString("NEED_STAFFTAG", "false");
        String staff_tags = param.getString("STAFF_TAGS", "1");
        ...
        if ((dept_id == null) || ("".equals(dept_id))) {
          if ((!CheckPriv.checkPermission(staffId, "ORG_LISTALL", "F")) && (!CheckPriv.checkPermission(staffId, "ORG_LIST_AREA", "F")) && (!CheckPriv.checkPermission(staffId, "ORG_LIST_AREA_20", "F")) && (!CheckPriv.checkPermission(staffId, "ORG_LIST_AREA_30", "F")) && (CheckPriv.checkPermission(staffId, "ORG_LIST_AREA_40", "F")))
          {
            IData root_area = getAreaGrantByDeptId(String.valueOf(CSBizBean.getVisit().getDepartId()), staffId);
            if ((root_area != null) && ("".equals(root_area.getString("RSVALUE3", "")))) {
              param.put("_AREA_FRAME_1", StaticUtil.getStaticValue(CSBizBean.getVisit(), "TD_M_AREA", "AREA_CODE", "AREA_FRAME", root_area.getString("RSVALUE3")));
              parser.addSQL(" and exists (select comarea.AREA_CODE from TD_M_AREA comarea where comarea.AREA_CODE = :_AREA_FRAME_1)");
            }
          } else {
            parser.addSQL(OrgFactory.getAreaSqlByOrg(CSBizBean.getVisit(), param, area_code, "dept.AREA_CODE"));
          }
        }
        else parser.addSQL(OrgFactory.getDepartSqlByFrame(CSBizBean.getVisit(), param, dept_id, "dept.DEPART_FRAME"));

        if (("true".equals(need_stafftag)) && (!"".equals(staff_tags))) {
          String[] tags = staff_tags.split("\\|");
          String rsrv_tag1 = "";
          for (int i = 0; i < tags.length; i++) {
            rsrv_tag1 = new StringBuilder().append(rsrv_tag1).append("'").append(tags[i]).append("'").toString();
            if (i + 1 < tags.length) {
              rsrv_tag1 = new StringBuilder().append(rsrv_tag1).append(",").toString();
            }
          }
          parser.addSQL(new StringBuilder().append(" and staff.RSRV_TAG1 in (").append(rsrv_tag1).append(")").toString());
        }
    ...
    throws Exception
  {
     ...
     * @throws Exception
     */
    public static IData getGroupKeyMemByGrpdMemSn( String memSn) throws Exception{
        IData param = new DataMap();
        param.put("MEM_SERIAL_NUMBER", memSn);
        StringBuilder sql = new StringBuilder();
        sql.append(SqlUtil.trimSql("SELECT A.*												"));
