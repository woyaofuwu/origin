...
public class TradeFinishCallEOSAction implements ITradeFinishAction
{
    ...
    public void executeAction(IData mainTrade) throws Exception
    {
        ...
        // bboss的商品工单上登记TRADE_EXT,专线产品上不登记TRADE_EXT,
        // 专线需要等最后一个专线完工时才调用esop的send接口，因此需要通过产品专线的最后一笔单子触发商品的sendesop

        String skipEosTag = mainTrade.getString("RSRV_STR1", ""); // 商品的RSRV_STR1上打了个跳过标记，不调用ESOP的send接口
        if (skipEosTag.equals("SKIPSENDESOP"))
            return;

        if (skipEosTag.equals("SENDEOS"))
        {// 专线的RSRV_STR1上SENDEOS表明是最后一笔专线（需要在归档的时候在专线的台账上RSRV_STR1=SENDEOS RSRV_STR2=商品的tradeid）
            tradeId = mainTrade.getString("RSRV_STR2", "");
            if (tradeId.equals(""))
                return;
        }
        ...
        if("2990".equals(tradeTypeCode) || "2991".equals(tradeTypeCode) || "3010".equals(tradeTypeCode)
        		|| "3011".equals(tradeTypeCode) || "3080".equals(tradeTypeCode) || "3081".equals(tradeTypeCode)){
        	return;
        }
