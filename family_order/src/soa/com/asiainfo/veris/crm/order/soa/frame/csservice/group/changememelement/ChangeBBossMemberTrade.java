...
public class ChangeBBossMemberTrade extends ChangeMemElement
{
    /*
    ...
    public void infoRegDataEntireMerchMeb(IData productInfo) throws Exception
    {
        ...
        IDataset merchMebs = UserGrpMerchMebInfoQry.getSEL_BY_USERID_USERIDA(reqData.getUca().getUserId(), productInfo.getString("USER_ID"), reqData.getUca().getUserEparchyCode());

        if (merchMebs != null && merchMebs.size() > 0)
        {
            IData merchMeb = merchMebs.getData(0);
            productOfferId=merchMeb.getString("PRODUCT_OFFER_ID");
            if (mebOperCode.equals(GroupBaseConst.BBOSS_MEB_STATUS.MEB_CANCLE.getValue()))
            {
                merchMeb.put("MODIFY_TAG", TRADE_MODIFY_TAG.DEL.getValue());
                merchMeb.put("STATUS", "Z");
                merchMeb.put("END_DATE", getAcceptTime());
            }
            else if (mebOperCode.equals(GroupBaseConst.BBOSS_MEB_STATUS.MEB_CONTINUE.getValue()))
            {
                merchMeb.put("MODIFY_TAG", TRADE_MODIFY_TAG.MODI.getValue());
                merchMeb.put("STATUS", "A");
                // 成员暂停与恢复，更改BB关系表与服务状态表状态
                modifyStateForRelaTabs(mebOperCode);
            }
            else if (mebOperCode.equals(GroupBaseConst.BBOSS_MEB_STATUS.MEB_PASTE.getValue()))
            {
                merchMeb.put("MODIFY_TAG", TRADE_MODIFY_TAG.MODI.getValue());
                merchMeb.put("STATUS", "N");
                // 成员暂停与恢复，更改BB关系表与服务状态表状态
                modifyStateForRelaTabs(mebOperCode);
            }
            ...
            this.addTradeGrpMerchMeb(merchMeb);
