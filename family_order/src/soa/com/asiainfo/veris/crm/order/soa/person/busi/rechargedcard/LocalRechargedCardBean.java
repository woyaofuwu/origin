...
public class LocalRechargedCardBean extends CSBizBean
{

    private IData callService(IData data) throws Exception
    {
        IData result = new DataMap();
        String url = "http://10.200.179.177:20000/service/SMPLVC";
        ...
        String paramCode3 = "V280"; // CardType
        ...
        try
        {
            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "URL");
            if (StringUtils.isNotBlank(paramValue))
                url = paramValue;

            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "SWITCH_CODE");
            if (StringUtils.isNotBlank(paramValue))
                switchCode = paramValue;

            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "SRV_CODE");
            if (StringUtils.isNotBlank(paramValue))
                srvCode = paramValue;

            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "CARDPIN_PARAM_CODE");
            if (StringUtils.isNotBlank(paramValue))
                paramCode0 = paramValue;

            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "MSISDN_PARAM_CODE");
            if (StringUtils.isNotBlank(paramValue))
                paramCode1 = paramValue;

            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "SERIALNUMBER_PARAM_CODE");
            if (StringUtils.isNotBlank(paramValue))
                paramCode2 = paramValue;

            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "CARDTYPE_PARAM_CODE");
            if (StringUtils.isNotBlank(paramValue))
                paramCode3 = paramValue;

            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "SMS_FOR_OWN");
            if (StringUtils.isNotBlank(paramValue))
                sms4own = paramValue;

            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "SMS_FOR_OTHER1");
            if (StringUtils.isNotBlank(paramValue))
                sms4other1 = paramValue;

            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "SMS_FOR_OTHER2");
            if (StringUtils.isNotBlank(paramValue))
                sms4other2 = paramValue;
            
            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "CHECK_TYPE");
            if (StringUtils.isNotBlank(paramValue))
                {checktype = paramValue;}
            
            paramValue = StaticUtil.getStaticValue("LOCAL_CARD_SRV", "TIME_OUT");
            if (StringUtils.isNotBlank(paramValue))
        ...
        varNode3.setMStrValue(data.getString("CARDTYPE"));
    ...
    private void recordCallLog(IData data, IData sopQueryResult, IData result) throws Exception
    {
        try
        {
            ...
            param.put("CARD_TYPE", data.getString("CARDTYPE", ""));
            ...
            sql.append("INSERT INTO LOCALCARD_CALL_LOG(ORDER_ID, SERIAL_NUMBER, USER_ID, SERIAL_NUMBER_B, USER_ID_B, ");
            sql.append("CARD_TYPE, CARD_PIN, ACCEPT_DATE, ACCEPT_MONTH, SEND_INFO, OPERATION_CODE, USER_AMOUNT, ");
            sql.append("CARD_PIN_AMOUNT, ACTIVEDAYS, SEQUENCE, ERROR_DESCRIPTION, X_RESULTCODE, X_RESULTINFO, SMS_NOTICE_ID, IN_MODE_CODE) ");
            sql.append("VALUES (:ORDER_ID, :SERIAL_NUMBER, :USER_ID, :SERIAL_NUMBER_B, :USER_ID_B, :CARD_TYPE, :CARD_PIN, ");
            sql.append("TO_DATE(:ACCEPT_DATE,'yyyyMMddHH24miss'), :ACCEPT_MONTH, :SEND_INFO, :OPERATION_CODE, :USER_AMOUNT, ");
            sql.append(":CARD_PIN_AMOUNT, :ACTIVEDAYS, :SEQUENCE, :ERROR_DESCRIPTION, :X_RESULTCODE, :X_RESULTINFO, :SMS_NOTICE_ID, :IN_MODE_CODE)");
