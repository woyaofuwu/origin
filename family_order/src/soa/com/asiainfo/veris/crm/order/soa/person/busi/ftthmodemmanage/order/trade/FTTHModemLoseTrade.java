 ...
 *
 */
public class FTTHModemLoseTrade extends BaseTrade implements ITrade
{

	@Override
	public void createBusiTradeData(BusiTradeData btd) throws Exception {
		// TODO Auto-generated method stub
		FTTHModemLoseReqData rd = (FTTHModemLoseReqData) btd.getRD();
			...
			if("0".equals(apply_type)){
				...
				if(Integer.parseInt(deposit) > 0){//有押金才做押金处理
	                   String sysdate=SysDateMgr.getSysDate(SysDateMgr.PATTERN_STAND_YYYYMMDD);
	                    ...
	                    if(newmodemStartDate.compareTo(sysdate)>0){//用户申领光猫小于三年丢失，押金沉淀处理
						//3、获取默认账户  （acct_id)
			    		...
			    		if(!"".equals(result) && "0".equals(result)){
			    			...
			    			CSAppException.appError("61312", "调用接口AM_CRM_TransFeeOutADSL资金转存现金错误:"+inAcct.getString("RESULT_INFO"));
			    		}
					}
				}
				else
				{
					//add by zhangxing3 for 没有押金，也需要处理other表
					createOtherTradeInfo(btd,input.getData(0));
					//add by zhangxing3 for 没有押金，也需要处理other表
				}
			}else{
