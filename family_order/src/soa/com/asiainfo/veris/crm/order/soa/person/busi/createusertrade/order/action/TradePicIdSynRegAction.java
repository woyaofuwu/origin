...
public class TradePicIdSynRegAction implements ITradeAction{

	private static Logger logger = Logger.getLogger(TradePicIdSynRegAction.class);
	...
	private IData getPicId (List<OtherTradeData> others, String tradeTypeCode) throws Exception
	{
		...
		if(others != null && others.size() > 0)
        {
            ...
            {
                if("PIC_ID".equals(other.getRsrvValueCode())){
                	picid = other.getRsrvStr1();
                	agentpicid = other.getRsrvStr3();
                	formpicid = other.getRsrvStr1();
                	picTag = "1";
                }
            }
        }
	...
	public void printLog(String name ,String value)
	{
		if(logger.isDebugEnabled()){
			logger.debug("<<<<<<<<<<<<<<<<<<<<<<<<TradePicIdSynRegAction "+name+value+"<<<<<<<<<<<<<<<<<<<<<<<<");
		}
