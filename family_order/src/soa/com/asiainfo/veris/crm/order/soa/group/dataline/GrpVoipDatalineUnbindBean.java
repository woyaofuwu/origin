...
public class GrpVoipDatalineUnbindBean extends GroupBean
{
    //VOIP专线（专网专线）解绑
    
    protected GrpVoipDatalineReqData reqData = null;
    ...
    public void infoRegUserOther() throws Exception{
        ...
        if(IDataUtil.isNotEmpty(dataLineInfos)){
            ...
            for (int i = 0; i < dataLineInfos.size(); i++){
                ...
                IDataset codeOtherInfos = TradeOtherInfoQry.queryUserOtherInfosByIdRsrvCode(tradeId,userId,"VOIP");
                if(IDataUtil.isNotEmpty(codeOtherInfos)){
                    for (int j = 0; j < codeOtherInfos.size(); j++)
                    {
                        IData codeOtherInfo = codeOtherInfos.getData(j);
                        if(IDataUtil.isNotEmpty(codeOtherInfo)){
                            codeOtherInfo.put("UPDATE_TIME", getAcceptTime());
                            codeOtherInfo.put("END_DATE", SysDateMgr.getSysTime());
                            codeOtherInfo.put("MODIFY_TAG", TRADE_MODIFY_TAG.MODI.getValue());
                            newOtherData.add(codeOtherInfo);
                        }
                    }
                }
    ...
    {
        return new GrpVoipDatalineReqData();
    }
    ...
    protected void initReqData() throws Exception
    {
        ...
        reqData = (GrpVoipDatalineReqData) getBaseReqData();
    }
