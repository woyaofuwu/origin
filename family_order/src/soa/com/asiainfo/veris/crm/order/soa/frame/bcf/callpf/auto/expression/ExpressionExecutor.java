...
import com.asiainfo.veris.crm.order.soa.frame.bcf.callpf.auto.expression.format.ExpressionParser;
...
public class ExpressionExecutor
{
     ...
     * @throws Exception
     */
    private void addToken(String tokenString, List<ExpressionToken> tokens) throws Exception
    {
        ...
        {
            java.util.Date d = SysDateMgr.string2Date(tokenString.substring(1, tokenString.length() - 1), SysDateMgr.PATTERN_STAND);

            token = ExpressionToken.createConstantToken(BaseDataMeta.DataType.DATATYPE_DATE, d);
            tokens.add(token);
     ...
     * @throws Exception
     */
    public List<ExpressionToken> analyze(String expression) throws Exception
    {
                        ...
                        else if (ExpressionToken.ETokenType.ETOKEN_TYPE_OPERATOR == onTopOp.getTokenType())
                        {
                            ...
                            else if (expToken.getOperator().getPiority() == onTopOp.getOperator().getPiority())
                            {
                                ...
                                else if (Operator.COLON == expToken.getOperator())
                                {
                                    ...
                                    else if (Operator.SELECT == onTopOp.getOperator())
                                    { // ?: , : -->弹出?: ,执行校验
                                        // 执行操作符校验
     ...
     * @throws Exception
     */
    public List<ExpressionToken> stringToTokens(String tokenExpression) throws Exception
    {
