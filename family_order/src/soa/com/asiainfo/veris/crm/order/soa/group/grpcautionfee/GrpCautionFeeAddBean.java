...
public class GrpCautionFeeAddBean extends GroupBean
{
	...
	private void createGrpCautionFee() throws Exception
	{
		...
		if(IDataUtil.isNotEmpty(feeInfos))//原来该用户有保证金记录,则叠加加上
		{
		...
		else 
		{
		    ...
		    parser.addSQL("select TF_F_USER_GRP_CAUTIONFEE$SEQ.nextval as REL_ID from dual");
		    IDataset dataset = Dao.qryByParse(parser,  null, Route.CONN_CRM_CG);
		    String relId = dataset.getData(0).getString("REL_ID");
		    infoData.put("REL_ID", relId);
			...
			Dao.insert("TF_F_USER_GRP_CAUTIONFEE", infoData);
		}
