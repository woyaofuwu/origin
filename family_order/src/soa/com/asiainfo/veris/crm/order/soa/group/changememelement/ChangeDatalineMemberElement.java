...
public class ChangeDatalineMemberElement extends ChangeUserElement {
    protected ChangeInternetUserElementReqData reqData = null;

    protected String userIdB = "";

    public ChangeDatalineMemberElement() {
    ...
    private void actTradeDatalineAttr() throws Exception {
        ...
        if (DataUtils.isNotEmpty(attrDataList)) {
            for (Object attrInf : attrDataList) {
                ...
                if ("PRODUCT_ID".equals(attrCode) && "7012".equals(attrValue)) {
                    attrInfo.put("ATTR_VALUE", "97012");
                }
            }
        }
        ...
        if (internet != null && internet.size() > 0 && null != userOther && userOther.size() > 0 && flag.equals("2")) {
            ...
            for (int i = 0; i < userOther.size(); i++) {
                ...
                if (numberCode.equals(lineNumberCode)) {
                    ...
                    // 集团客户所在市县
                    newVispUser.put("RSRV_STR11", groupCity);

                    String aCity = UAreaInfoQry.getAreaNameByAreaCode(internet.getString("pam_NOTIN_A_CITY"));
                    // A端所在市县
                    newVispUser.put("RSRV_STR12", aCity);

                    String zCity = UAreaInfoQry.getAreaNameByAreaCode(internet.getString("pam_NOTIN_Z_CITY"));
                    // Z端所在市县
                    newVispUser.put("RSRV_STR13", zCity);

                    // 集团客户所在市县
                    newVispUser.put("RSRV_STR14", internet.getString("pam_NOTIN_GROUP_CITY"));
                    // A端所在市县
                    newVispUser.put("RSRV_STR15", internet.getString("pam_NOTIN_A_CITY"));
                    // Z端所在市县

                    // // 集团所在市县分成比例
                    // newVispUser.put("RSRV_STR17", internet.getString("pam_NOTIN_GROUP_PERCENT"));
                    // // A端所在市县分成比例
                    // newVispUser.put("RSRV_STR18", internet.getString("pam_NOTIN_A_PERCENT"));
                    // // Z端所在市县分成比例
                    // newVispUser.put("RSRV_STR19", internet.getString("pam_NOTIN_Z_PERCENT"));
    ...
    private void actTradeUserAttr() throws Exception {
        ...
        if ("97012".equals(productId)) {
            productId = "7012";
            discntCode = DataLineDiscntConst.getElementIdToProductId("7012",userIdB);
        } else if ("97011".equals(productId)) {
            productId = "7011";
            discntCode = DataLineDiscntConst.getElementIdToProductId("7011",userIdB);
        }else if (DataLineDiscntConst.dataline1ProductIdMember.equals(productId)) {
            productId = DataLineDiscntConst.dataline1ProductId;
            discntCode = DataLineDiscntConst.getElementIdToProductId(productId,userIdB);
        }else if (DataLineDiscntConst.dataline2ProductIdMember.equals(productId)) {
            productId = DataLineDiscntConst.dataline2ProductId;
            discntCode = DataLineDiscntConst.getElementIdToProductId(productId,userIdB);
        }
