...
package com.asiainfo.veris.crm.order.soa.person.busi.changepayrelation.order.trade;
...
public class ModifyPayRelationInfoTrade extends BaseTrade implements ITrade
{
    ...
    public void createTradePayrelation(BusiTradeData btd) throws Exception
    {
        ...
        {// 将原账户下所有用户转移
            ...
            IDataset dataset = PayRelaInfoQry.qryAcctValidUserInfos(oldAcctId, "1", "1");
        ...
        else
        {
            ...
            /* 手机用户普通付费关系变更导致宽带付费关系连带变更 */
            String KDserialNumber = "KD_" + btd.getRD().getUca().getSerialNumber();
            IData kdUsers = UcaInfoQry.qryUserInfoBySn(KDserialNumber);
            if (IDataUtil.isNotEmpty(kdUsers))
            {
                String userId = kdUsers.getString("USER_ID");
                dealPayRelationTrade(btd, userId, "手机付费变更连带宽带变更");
            }
        }
    }
