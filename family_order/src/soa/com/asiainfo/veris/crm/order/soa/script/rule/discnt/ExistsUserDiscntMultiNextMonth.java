 ...
 * 
 */
public class ExistsUserDiscntMultiNextMonth extends BreBase implements IBREScript
{

    private static Logger logger = Logger.getLogger(ExistsUserDiscntMultiNextMonth.class);
            ...
            else
            {
                for (int i = userAllDiscnt.size() - 1; i >= 0; i--)
                {
                    ...
                    for (int j = commparaList.size() - 1; j >= 0; j--)
                    {
                        ...
                        if (userDiscnt.getString("DISCNT_CODE").equals(commpara.getString("PARA_CODE1")) && userDiscnt.getString("END_DATE").compareTo(SysDateMgr.getFirstDayOfNextMonth()) > 0)
                        {
                            // 只要找到一条就够了
                            resultFlag = true;
                            break;
