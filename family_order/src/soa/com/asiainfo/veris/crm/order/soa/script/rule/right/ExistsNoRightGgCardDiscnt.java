...
public class ExistsNoRightGgCardDiscnt extends BreBase implements IBREScript
{

    private static Logger logger = Logger.getLogger(ExistsNoRightGgCardDiscnt.class);
        ...
        if (!"SUPERUSR".equals(strStaffId))
        {
            for (Iterator iter = listTradeDiscnt.iterator(); iter.hasNext();)
            {
                ...
                if ("0".equals(tradeDiscnt.getString("MODIFY_TAG")))
                {
                    ...
                    {
                        listCommpara = BreQryForCommparaOrTag.getCommpara("CSM", 1080, strRsrvStr1, strTradeEparchyCode);
                    }
                    ...
                    for (Iterator iterator = listCommpara.iterator(); iterator.hasNext();)
                    {
                        ...
                        if (commparam.getString("RSRV_STR5").equals(tradeDiscnt.getString("DISCNT_CODE")))
                        {
                            bExists = true;
                            break;
                        }
