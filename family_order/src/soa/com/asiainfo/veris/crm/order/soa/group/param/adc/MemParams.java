...
public class MemParams
{
    ...
    /**
     * 2014-07-28 liaolc 根据集团服务ID,获取集团服务对应成员服务。
     * 
     * @param userIdA
     * @param serviceId
     * @return
     * @throws Exception
     */
    public static String getmebServIdByGrpServId(String serviceId) throws Exception
    {
        IDataset tempLists = AttrBizInfoQry.getBizAttrByDynamic(serviceId, "S", "Obver", serviceId, null);
        if (IDataUtil.isEmpty(tempLists))
        {
            CSAppException.apperr(ParamException.CRM_PARAM_513, serviceId);
        }
        String mebServId = tempLists.getData(0).getString("ATTR_VALUE");
        if (StringUtils.isBlank(mebServId))
        {
            CSAppException.apperr(ParamException.CRM_PARAM_513, serviceId);
        }
        return mebServId;
     ...
     * @throws Exception
     */
    public  IDataset getmebServIdByGrpServId(IData serviceId) throws Exception
    {
        String strserviceId = serviceId.getString("SERVICE_ID");
        IDataset tempLists = AttrBizInfoQry.getBizAttrByDynamic(strserviceId, "S", "Obver", strserviceId, null);
        if (IDataUtil.isEmpty(tempLists))
        {
            CSAppException.apperr(ParamException.CRM_PARAM_513, strserviceId);
        }
        String mebServId = tempLists.getData(0).getString("ATTR_VALUE");
        if (StringUtils.isBlank(mebServId))
        {
    ...
    /**
     * 2014-08-11 liaolc 根据集团服务ID,获取集团服务对应成员服务 或 根据集团定制的成员服务ID获取集团订购的服务ID。
     * 
     * @param serviceId
     * @return
     * @throws Exception
     */
    public static String getServIdByGrpORmebServId(String serviceId) throws Exception
    {
        String retServId = "";
        IDataset tempLists = AttrBizInfoQry.getBizAttrByDynamic(serviceId, "S", "Obver", serviceId, null);
        if (IDataUtil.isNotEmpty(tempLists))
        {
            retServId = tempLists.getData(0).getString("ATTR_VALUE");
        }
        return retServId;
    }
