 ...
 * @date: 2019-4-25
 */
public class CancelSaleActiveForDiscntFinishAction implements ITradeFinishAction{
	
	private static transient Logger logger = Logger.getLogger(CancelSaleActiveForDiscntFinishAction.class);
	...
	public void executeAction(IData mainTrade) throws Exception {
    	...
    	logger.debug("进入CancelSaleActiveForDiscntFinishAction"+DiscntTradesAdd+";"+DiscntTradesDel);
    	if (IDataUtil.isNotEmpty(productTradesAdd)&&IDataUtil.isNotEmpty(productTradesDel))
        {
    		IDataset commparaInfos9935 = CommparaInfoQry.queryComparaByAttrAndCode1("CSM","9935",null,null);
			System.out.println("CancelSaleActiveForDiscntFinishAction标志"+commparaInfos9935);
    		if(IDataUtil.isNotEmpty(commparaInfos9935)){
        			for (int j = 0; j < commparaInfos9935.size(); j++) {
        				 String productId = commparaInfos9935.getData(j).getString("PARAM_CODE");
        				 String products = commparaInfos9935.getData(j).getString("PARA_CODE20");//档次范围的产品
        				 String products2 = commparaInfos9935.getData(j).getString("PARA_CODE21");//档次范围的产品扩展
        				 String paraCode6 = commparaInfos9935.getData(j).getString("PARA_CODE6","3");//终止时间 0:当前  3：月底
