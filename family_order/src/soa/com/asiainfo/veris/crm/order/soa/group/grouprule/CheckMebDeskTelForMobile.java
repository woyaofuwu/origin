...
public class CheckMebDeskTelForMobile extends BreBase implements IBREScript
{
    ...
    private static Logger logger = Logger.getLogger(CheckMebDeskTelForMobile.class);

    public boolean run(IData databus, BreRuleParam ruleParam) throws Exception
    {
        if (logger.isDebugEnabled())
            logger.debug(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 进入 CheckMebDeskTelForMobile() >>>>>>>>>>>>>>>>>>");
        ...
        else
        {
            ...
            {
                BreTipsHelp.addNorTipsInfo(databus, BreFactory.TIPS_TYPE_ERROR, errCode, "只有IMS用户才能新增为桌面电话成员!");
                bResult = false;
            }
            else
            {
                ...
                {
                    BreTipsHelp.addNorTipsInfo(databus, BreFactory.TIPS_TYPE_ERROR, errCode, "固定终端IMS用户才能新增为桌面电话成员!");
                    bResult = false;
                }
            }
        }
        ...
        if (logger.isDebugEnabled())
            logger.debug(" <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 退出CheckMebDeskTelForMobile() " + bResult + "<<<<<<<<<<<<<<<<<<<");
