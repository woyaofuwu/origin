...
public class CttFeeRegBean extends CSBizBean
{
    public IDataset qryFeeRegCTT(IData param) throws Exception
    {
        IDataset feeRegType = FeeRegQry.queryFeeRegs();// 查询费用登记类型
        ...
        for (int i = 0; i < feeRegTypeSize; i++)
        {
            ...
            temp.put("PARENT_FEE_TYPE_CODE", feeTypeTemp.getString("PARENT_FEE_TYPE_CODE"));
            temp.put("PARENT_FEE_TYPE", feeTypeTemp.getString("PARENT_FEE_TYPE"));
    ...
    public IDataset queryFeeRegTotle(IData param) throws Exception
    {

        return FeeRegQry.queryFeeRegTotle(param.getString("LOG_ID"));
    }
     ...
     * @throws Exception
     */
    public String saveFeeReg(IData data) throws Exception
    {
        ...
        temp3.put("FEE_TYPE_CODE", "T1"); // 卡类合计费用
        ...
        if (dataset.size() > 0)
        {
            Dao.executeBatchByCodeCode("TF_F_FEEREG", "INS_FEEREG", dataset);
        }

        // 修改
        if (dataset2.size() > 0)
        {
            Dao.executeBatchByCodeCode("TF_F_FEEREG", "UPD_FEEREG", dataset2);
        }
        ...
        if ("1".equals(regFlag))
        {
            ...
            Dao.executeUpdateByCodeCode("TF_F_FEEREG", "UPD_FEEREG2", temp4);
        }
     ...
     * @throws Exception
     */
    public String saveFeeRegM(IData data) throws Exception
    {
        DecimalFormat df = new DecimalFormat("0");
        String logId = data.getString("LOG_ID", "-1");
        String a = data.getString("feeDataset");
