...
public class ChangeJKDTPastProductTrade extends ChangeUserElement {
    ...
    public void actTradePrdParams() throws Exception {
        ...
        if(!"6".equals(CSBizBean.getVisit().getInModeCode()))//如果是省boss发起则填写默认的暂停原因，平台要求必填
        {
        	 IData attrMap=new DataMap();
        	 attrMap.put("STATE", "ADD");
             attrMap.put("ATTR_CODE", "9001");
             attrMap.put("ATTR_VALUE", "暂停");
             attrMap.put("ATTR_GROUP", "");
             attrMap.put("ATTR_NAME", "暂停原因");
             params.add(attrMap);
        ...
        for (int i = 0; i < params.size(); i++) {
            ...
            if (GroupBaseConst.PRODUCT_ATTR_STATUS_DESC.ATTR_ADD.getValue().equals(state)) {
                ...
                instParam.put("MODIFY_TAG", TRADE_MODIFY_TAG.DEL.getValue());
    ...
    public void actTradeSub() throws Exception {
        ...
        // 修改用户服务状态表
        this.processSvcState(merchpOperType);
    ...
    public void infoRegDataEntireMerchP(String merchpOperType) throws Exception {
        ...
        grpMerchPInfo.put("MODIFY_TAG", TRADE_MODIFY_TAG.MODI.getValue());
...
//        }
    ...
    /**
     * @description 暂停时将集团资费截止掉
     * @author songxw
     * @date 2019-06-18
     */
    protected void infoRegDataDiscnt() throws Exception {
    	// 1-1 获取用户编号
        String userId = reqData.getUca().getUser().getUserId();

        String nextMonthDay = SysDateMgr.getFirstDayOfNextMonth();
        // 根据用户编号获取资费信息
        //由于暂停之前用户可能做过资费变更故暂停时只对生效日期为2050年的做处理
        IDataset userDiscnts = UserEcrecepProductInfoQry.qryEcrecepDiscntByUserId(userId, nextMonthDay);
        if(userDiscnts != null && userDiscnts.size() > 0){
	        IData userDiscnt = userDiscnts.getData(0);
	        userDiscnt.put("MODIFY_TAG", "1");
	        userDiscnt.put("END_DATE", SysDateMgr.getLastDateThisMonth());
	        userDiscnt.put("RSRV_NUM1", "1");//0-正常状态1-暂停状态
    ...
    protected void processSvcState(String merchpOperType) throws Exception {
        ...
        for (int i = 0; i < userSvcInfo.size(); i++) {
            ...
            for (int j = 0; j < userSvcStateInfo.size(); j++) {
                // 根据不同的产品操作类型来确定新的状态值
                String state = "0";
                if (GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE.getValue().equals(merchpOperType) || GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE_MEBFLUX.getValue().equals(merchpOperType)) {// 产品暂停
                    state = "3";
                } else if (GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PREDESTORY.getValue().equals(merchpOperType)) {// 产品预取消
                    state = "5";
                } else {// 产品恢复或恢复预取消
                    state = "0";
                }
                ...
                //集团V网暂停，下月生效。
                String islastmonth = UStaticInfoQry.getStaticValue("PAYMENTDAY", reqData.getUca().getProductId());
                if("1".equals(islastmonth) && GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE.getValue().equals(merchpOperType)){
                	userSvcState.put("END_DATE", SysDateMgr.getLastDateThisMonth());
                }else{
                ...
                if("1".equals(islastmonth) && GroupBaseConst.MERCH_PRODUCT_STATUS.PRODUCT_PASTE.getValue().equals(merchpOperType)){
                	userSvcState1.put("START_DATE", SysDateMgr.getFirstDayOfNextMonth());
                }else{
