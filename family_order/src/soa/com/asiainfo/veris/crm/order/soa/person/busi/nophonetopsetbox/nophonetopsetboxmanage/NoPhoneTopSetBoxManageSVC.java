...
public class NoPhoneTopSetBoxManageSVC extends CSBizService{
     ...
     * @version: v1.0.0
     * @author: zhengkai5
     * @date: 2017-9-10 下午8:39:41 Modification History: Date Author Version Description
     */
    ...
    public IData checkOperType(IData input) throws Exception{
    	...
    	String wSerialNumber = input.getString("SERIAL_NUMBER");
    	if(!"KD_".equals(wSerialNumber.substring(0, 3))){
    		wSerialNumber = "KD_"+wSerialNumber;
    	}
        ...
        IDataset validBoxInfos = UserResInfoQry.qrySetTopBoxByUserIdAndTag1(userIdA, "4", "J");
    	if("APPLY_TOPSETBOX".equals(operType)){//申领机顶盒
    		if(DataSetUtils.isNotBlank(validBoxInfos)){
    			...
    			if(IDataUtil.isEmpty(wideUserInfo)){
    				...
    				IDataset invalidBoxInfos = NoPhoneTopSetBoxManageBean.qryAllSetTopBoxByUserIdAndTag1(userIdA, "4", "J");
    				if(IDataUtil.isNotEmpty(invalidBoxInfos)){
    		        	...
    		        	if(basePackageInfo!=null&&!basePackageInfo.trim().equals("")){
    		            	...
    		            	if(basePackages!=null&&basePackages.length>0){
    		            		...
    		            		IDataset userPlaysvcInfos = BofQuery.getUserPlatSvc(userIdA,getVisit().getStaffEparchyCode());//查询魔百和业务是否取消
    ...
    /**
	 * 无手机魔百和机顶盒管理核对用户，并获取相关信息
	 * @param param
	 * @return
	 * @throws Exception
	 */
public IData checkUserValidForNoPhoneTopsetboxManage(IData param)throws Exception{
		
	NoPhoneTopSetBoxManageBean bean= BeanManager.createBean(NoPhoneTopSetBoxManageBean.class);
	IData result=new DataMap();
	
	String wSerialNumber=param.getString("AUTH_SERIAL_NUMBER");
	if(!"KD_".equals(wSerialNumber.substring(0, 3))){
		wSerialNumber = "KD_" + wSerialNumber;
	}
    IData userInfo = UcaInfoQry.qryUserInfoBySn(wSerialNumber);
    ...
    IData userInfoA = getRelaUUInfoByUserIdB(userInfo.getString("USER_ID"));
    String userIdA = userInfoA.getString("USER_ID_A");
    
    String serialNumberA = userInfoA.getString("SERIAL_NUMBER_A");
    userInfo.put("SERIAL_NUMBER_A",serialNumberA);
         
    //查询是否有魔百和开户的订单（tf_b_trade）
    ...
    if (IDataUtil.isNotEmpty(outDataset))
    {
    	...
    	//查询是否有魔百和相关业务的订单
    	IDataset out1Dataset = TradeInfoQry.getTradeInfosBySelTradeByUserIdCode("4909", userIdA, "0");
	...
	IDataset validBoxInfos = UserResInfoQry.qrySetTopBoxByUserIdAndTag1AllColumns(userIdA, "4", "J");
	
	if(IDataUtil.isEmpty(validBoxInfos)){
		IDataset invalidBoxInfos = bean.qryAllSetTopBoxByUserIdAndTag1(userIdA, "4", "J");
		if(IDataUtil.isNotEmpty(invalidBoxInfos)){
    ...
    if (DataSetUtils.isBlank(wideInfos))
    {
        ...
        if (IDataUtil.isNotEmpty(wUserInfo))
        {
        	...
        	IData wideNetInfo = NoPhoneTopSetBoxManageBean.getUserWidenetInfoBySerialNumber(wSerialNumber).first();
