...
/**
 * @program: hain_order
 * @description: 集客大厅删除成员
 * @author: zhangchengzhi
 * @create: 2018-09-30 11:26
 **/

public class DestroyReceptionHallMemTrade extends DestroyGroupMember{
    ...
    protected void actTradeRelationUU() throws Exception
    {
        ...
        relaData.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.DEL.getValue());
    ...
    public void actTradeSub() throws Exception
    {
        ...
        if (!reqData.isMerchInfo)
        {
            this.infoRegDataEceecrpMerchMeb();
...
//            infoRegDataSpecial();
     ...
     * @throws Exception
     */
    public void infoRegDataEceecrpMerchMeb() throws Exception {
        ...
        IDataset datas = UserEcrecepMebInfoQry.getSEL_BY_USERID_USERIDA(userId, ecUserId, eparchCode);
        ...
        data.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.DEL.getValue());
        data.put("STATUS", "Z");
        data.put("END_DATE", SysDateMgr.getSysDateYYYYMMDDHHMMSS());
        data.put("MEB_OPER_CODE", GroupBaseConst.BBOSS_MEB_STATUS.MEB_CANCLE.getValue()); // 删除
        this.addEcrecrpMerchMeb(data);

        //登记集客受理大厅流程信息
        IData epData = GrpCommonBean.actEcrecepProcedure(reqData.getUca().getUser().getUserId(), SeqMgr.getInstId(), reqData.getUca().getUser().getSerialNumber(), data.getString("PRODUCT_ORDER_ID"), data.getString("PRODUCT_OFFER_ID", ""), "0", "N", "0");
...
//        }
    ...
    protected void setTradeBase() throws Exception
    {
        ...
        if ("6".equals(CSBizBean.getVisit().getInModeCode())) {
           // IDataset dataset = CommparaInfoQry.getCommparaByAttrCode1("CSM", "9079", specProductId, CSBizBean.getTradeEparchyCode(), null);
       ...
       if(specProductId.equals("910401")){
             ...
             mainSvcInfo.put("MODIFY_TAG", "1");// 状态属性：0-增加，1-删除，2-变更
    ...
    private  void addTradePayrelaForBboss(IData payRelaInfo)throws Exception{
        ...
        if(IDataUtil.isEmpty(tradePayRelationInfoList)){
            payRelaInfo.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.DEL.getValue());
            payRelaInfo.put("END_CYCLE_ID", SysDateMgr.getNowCyc());
            super.addTradePayrelation(payRelaInfo);
            return;
        }
    ...
    public void infoRegOtherData() throws Exception{
        ...
        enumData.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.DEL.getValue());
     ...
     * @throws Exception
     */
    private void addTradeReceptionProduce(IData param) throws Exception{
        IData merchPInfo = new DataMap();


/*
	TRADE_ID	NUMBER(16)			Y
2		ACCEPT_MONTH	NUMBER(2)			Y		受理月份：受理时间的月份，可作为分区字段。正常情况下可从trade_id的第5、6位获得。
3	INST_ID		NUMBER(16)			Y
4		USER_ID	NUMBER(16)			Y
5		SERIAL_NUMBER	VARCHAR2(40)			Y
6	ECRECEP_ORDER_ID	ECRECEP_ORDER_ID	VARCHAR2(20)			Y
7		ECRECEP_OFFER_ID	VARCHAR2(20)			Y
8		OPERTYPE	CHAR(5)			Y
9		SVC_STATUS	CHAR(1)			Y
10		USER_STATUS	CHAR(1)			Y
11		START_DATE	DATE			Y
12		END_DATE	DATE			Y
13		MODIFY_TAG	CHAR(1)			Y		修改标志：0-增加  1-删除  2-修改
14		UPDATE_TIME	DATE			Y
15		UPDATE_STAFF_ID	CHAR(8)			Y
16		UPDATE_DEPART_ID	CHAR(5) */

        merchPInfo.put("TRADE_ID", bizData.getTrade().getString("TRADE_ID"));
        merchPInfo.put("USER_ID", reqData.getUca().getUser().getUserId());
        merchPInfo.put("SERIAL_NUMBER", reqData.getUca().getUser().getSerialNumber());
        merchPInfo.put("SVC_STATUS", "A");
        merchPInfo.put("START_DATE", getAcceptTime());
        merchPInfo.put("END_DATE", SysDateMgr.getTheLastTime());


        merchPInfo.put("ACCEPT_MONTH", reqData.getUca().getUser().getUserId());
        merchPInfo.put("INST_ID", reqData.getUca().getUser().getUserId());
        merchPInfo.put("ECRECEP_ORDER_ID", reqData.getUca().getUser().getUserId());
        merchPInfo.put("ECRECEP_OFFER_ID", reqData.getUca().getUser().getUserId());
        merchPInfo.put("OPERTYPE", reqData.getUca().getUser().getUserId());
        merchPInfo.put("USER_STATUS", reqData.getUca().getUser().getUserId());
        merchPInfo.put("MODIFY_TAG", reqData.getUca().getUser().getUserId());
        merchPInfo.put("UPDATE_TIME", reqData.getUca().getUser().getUserId());
        merchPInfo.put("UPDATE_STAFF_ID", reqData.getUca().getUser().getUserId());
        merchPInfo.put("UPDATE_DEPART_ID", reqData.getUca().getUser().getUserId());




//        merchPInfo.put("ECRECEP_ORDER_ID", reqData.getMerchp().getString("PRODUCT_ORDER_ID"));
//        merchPInfo.put("MODIFY_TAG", TRADE_MODIFY_TAG.Add.getValue());

        super.addTradeEcrecepProcedure(merchPInfo);
    }
