...
public class ScoreQueryTradeBean extends CSBizBean
{
    ...
    public IDataset queryScoreBizInfos(IData inparam, Pagination pagination) throws Exception
    {
        ...
        return QryScoreInfo.queryScoreBizInfos(inparam, pagination, Route.getJourDb(BizRoute.getTradeEparchyCode()));
    }
    ...
    public IDataset queryScoreDetail(IData inparam, Pagination pagination) throws Exception
    {
        ...
        score_type_code = StringUtils.isEmpty(score_type_code) ? "ZZ" : "0"+score_type_code; //如果为0-9则补0

        String startCycleId = inparam.getString("START_CYCLE_ID").replaceAll("-", "");
        String endCycleId = inparam.getString("END_CYCLE_ID").replaceAll("-", "");
        if (StringUtils.isNotBlank(startCycleId))
        {
            startCycleId = startCycleId.substring(0, 6);
        }
        ...
        IDataset dataset = AcctCall.queryScoreDetail(inparam.getString("USER_ID"), score_type_code, startCycleId, endCycleId);
    ...
    public IDataset queryYearSumScore(IData inparam) throws Exception
    {
        ...
        IDataset dataset = AcctCall.queryYearSumScore(inparam.getString("USER_ID"), startCycleId, endCycleId);
        return dataset;
    }
