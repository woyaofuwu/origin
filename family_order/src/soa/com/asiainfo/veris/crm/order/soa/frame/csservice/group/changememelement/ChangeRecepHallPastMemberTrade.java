 ...
 * @create: 2018-10-10 17:03
 **/

public class ChangeRecepHallPastMemberTrade extends ChangeRecepHallBaseMemberTrade{
    ...
    protected void modifyStateForRelaTabs(String memOpType) throws Exception
    {
        ...
        IDataset mebRelaUUInfos = RelaBBInfoQry.getBBByUserIdAB(grpUserId, memUserId, "1", relationTypeCode);
        if (null == mebRelaUUInfos || mebRelaUUInfos.size() < 0)
        {
            return;
        }
        for (int i = 0; i < mebRelaUUInfos.size(); i++)
        {
        	IData mebRelaUUInfo = mebRelaUUInfos.getData(i);
        	mebRelaUUInfo.put("RSRV_STR5", "N");
        	mebRelaUUInfo.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.MODI.getValue());
        	this.addTradeRelationBb(mebRelaUUInfo);//daidl
        }
