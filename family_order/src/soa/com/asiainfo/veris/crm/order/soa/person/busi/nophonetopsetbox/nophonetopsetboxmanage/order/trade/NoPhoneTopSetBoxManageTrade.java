...
package com.asiainfo.veris.crm.order.soa.person.busi.nophonetopsetbox.nophonetopsetboxmanage.order.trade;
...
import com.asiainfo.veris.crm.order.soa.person.busi.nophonetopsetbox.nophonetopsetboxmanage.NoPhoneTopSetBoxManageBean;
 ...
 * @ClassName: NoPhoneTopSetBoxTrade.java
 * @Description:
 * @version: v1.0.0
 * @author: zhengkai5
 * @date: 2017-9-10 
 */
public class NoPhoneTopSetBoxManageTrade extends BaseTrade implements ITrade
{
	private static Logger log=Logger.getLogger(NoPhoneTopSetBoxManageTrade.class);

    public void createBusiTradeData(BusiTradeData btd) throws Exception
    {
        NoPhoneTopSetBoxManageRequestData tsbReqData = (NoPhoneTopSetBoxManageRequestData) btd.getRD();
        ...
        if(IDataUtil.isNotEmpty(pageParam)){
        	...
        	if(operType!=null&&!operType.trim().equals("")){
        		if(operType.equals("APPLY_TOPSETBOX")){		//申领
        			...
        			NoPhoneTopSetBoxManageBean bean= BeanManager.createBean(NoPhoneTopSetBoxManageBean.class);
        			IDataset boxInfos = bean.qrySetTopBoxByUserIdAndTag1AllColumns(tsbReqData.getUca().getUserId(), "4", "J");
                    IDataset discntInfo = UserDiscntInfoQry.getAllDiscntByUser_2(tsbReqData.getUca().getUserId(), "84073843");//调测费没有此存折，不退  modify_by_duhj_kd
        			if(DataSetUtils.isNotBlank(boxInfos) && IDataUtil.isEmpty(discntInfo)){
        				...
        				if(Integer.parseInt(money) > 0){//有押金才退还押金
        					...
        					//调用接口，将【押金】——>【退费】
        					//无手机宽带魔百和业务调用退费接口
        					IDataset inAccts =  AcctCall.backFee(uca.getUserId(), tsbReqData.getTradeId(), "15000", "9016", "16001", money);
     ...
     * @date: 2017-9-10 
     */
    private void createResTradeData(BusiTradeData btd, NoPhoneTopSetBoxManageRequestData tsbReqData, String modifyTag) throws Exception
    {
     ...
     */
    private void createResTradeDataForChange(BusiTradeData btd, NoPhoneTopSetBoxManageRequestData tsbReqData, String modifyTag) throws Exception
    {
    	TopSetBoxManageBean bean= BeanManager.createBean(TopSetBoxManageBean.class);
    	IDataset boxInfos = bean.qrySetTopBoxByUserIdAndTag1AllColumns(tsbReqData.getUca().getUserId(), "4", "J");
     ...
     * @date: 2017-9-14 
     */
    private void updateMainTradeData(BusiTradeData btd, NoPhoneTopSetBoxManageRequestData tsbReqData) throws Exception
    {
        List mainList = btd.get(TradeTableEnum.TRADE_MAIN.getValue());
        MainTradeData mainTD = (MainTradeData) mainList.get(0);
