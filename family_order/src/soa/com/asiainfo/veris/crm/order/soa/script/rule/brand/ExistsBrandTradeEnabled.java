 ...
 * param_attr=135 and param_code=a.brand_code and para_code1=:TRADE_TYPE_CODE and (eparchy_code=:TRADE_EPARCHY_CODE or
 * eparchy_code='ZZZZ') and sysdate between start_date and end_date)
 */
public class ExistsBrandTradeEnabled extends BreBase implements IBREScript
{
    private static Logger logger = Logger.getLogger(ExistsBrandTradeEnabled.class);

    public boolean run(IData databus, BreRuleParam ruleParam) throws Exception
    {
        if (logger.isDebugEnabled())
            logger.debug(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 进入 ExistsBrandTradeEnabled() >>>>>>>>>>>>>>>>>>");
        ...
        IDataset ComparaInfo = BreQryForCommparaOrTag.getCommparaCode("CSM", 135, strTradeTypeCode, strTradeEparchyCode);
        ...
        for (int iListUser = 0, iASzie = listUser.size(); iListUser < iASzie; iListUser++)
        {
        ...
        if (logger.isDebugEnabled())
            logger.debug(" <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 退出 ExistsBrandTradeEnabled() " + bResult + "<<<<<<<<<<<<<<<<<<<");
