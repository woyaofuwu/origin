...
public class CheckMebJTCLBForMobile extends BreBase implements IBREScript
{

    /**
     * 移动号码不能新增为集团固话畅聊包产品成员
     */
    private static final long serialVersionUID = 1L;

    private static Logger logger = Logger.getLogger(CheckMebJTCLBForMobile.class);

    public boolean run(IData databus, BreRuleParam ruleParam) throws Exception
    {
        if (logger.isDebugEnabled())
            logger.debug(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 进入 CheckMebJTCLBForMobile() >>>>>>>>>>>>>>>>>>");
        ...
        else
        {
            ...
            {
                BreTipsHelp.addNorTipsInfo(databus, BreFactory.TIPS_TYPE_ERROR, errCode, "只有IMS用户才能新增为[集团固话畅聊包产品]成员!");
                bResult = false;
            }
            else
            {
                ...
                {
                    BreTipsHelp.addNorTipsInfo(databus, BreFactory.TIPS_TYPE_ERROR, errCode, "固定终端IMS用户才能新增为[集团固话畅聊包产品]成员!");
                    bResult = false;
                }
            }
        }
        ...
        if (logger.isDebugEnabled())
            logger.debug(" <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 退出CheckMebJTCLBForMobile() " + bResult + "<<<<<<<<<<<<<<<<<<<");
