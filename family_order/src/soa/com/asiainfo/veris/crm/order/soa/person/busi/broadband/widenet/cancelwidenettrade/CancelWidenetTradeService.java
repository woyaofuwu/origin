...
public class CancelWidenetTradeService extends CSBizService
{
	
	private Logger log=Logger.getLogger(CancelWidenetTradeService.class);
    ...
    public IDataset cancelTradeReg(IData input) throws Exception
    {
        ...
        if(tradeTypeCode.equals("3800")){	//如果是魔百和开户撤单
        	
        	/*
        	 * 形成退款的客户资源信息，插入一条魔百和的数据，用作退库使用
        	 */
        	String isOpenTopsetbox=tradeInfo.getString("RSRV_STR2","");
        ...
        }else if(tradeTypeCode.equals("600")||tradeTypeCode.equals("612")||
        		tradeTypeCode.equals("613")){
        	
        	//先装后付付款后不能撤单
        	String payMode = tradeInfo.getString("RSRV_STR1","");  //支付模式：P：立即支付  A：先装后付
        	String payState = tradeInfo.getString("RSRV_STR8",""); //付款判断
        	if( "A".equals(payMode) && ("Y").equals(payState))
        	{
        		CSAppException.appError("-1", "先装后付付款后不能撤单!");
        	}
            ...
            if (IDataUtil.isNotEmpty(imsInfos))
            {
                ...
                {
                	CSAppException.appError("2018000019", "该宽带同时办理的IMS固话业务已经出库开通，不能办理宽带撤单业务!");
                }
            }
            
            
            //start add by xuzh5 2018-9-25 17:34:43 REQ201807060009优化撤单和拆机权限管控模式
            //WIDE_CANCEL_ALL_CODE 权限可以撤掉全省的宽带数据 2019-1-10 10:38:24
            if(!(StaffPrivUtil.isFuncDataPriv(getVisit().getStaffId(), "WIDE_CANCEL_ALL_CODE")) ){
            	//HNKF和HNYD之间可以互撤 2019-1-17 10:15:36
            	if("HNKF".equals(cityCode) || "HNYD".equals(cityCode)){
            		if(!"HNKF".equals(tradeInfo.getString("TRADE_CITY_CODE", "")) && !"HNYD".equals(tradeInfo.getString("TRADE_CITY_CODE", ""))){
            			CSAppException.appError("-1","只能撤销HNKF或HNYD工号受理的宽带施工单！");
            		}
            	//别的只能撤归属地市
            	}else if(!cityCode.equals(tradeInfo.getString("TRADE_CITY_CODE", ""))){
	        		String res="只能撤销"+cityCode+"工号受理的宽带施工单！";
	        		CSAppException.appError("-1",res);
	        	}
            }
        	//如果存在生效的魔百和业务
        	String wnSerialNumber=tradeInfo.getString("SERIAL_NUMBER");
        	String serialNumber=wnSerialNumber.replaceAll("KD_", "");
        	...
        	if(IDataUtil.isNotEmpty(userinfo))
        	{
        		...
        		if(IDataUtil.isNotEmpty(boxInfos))
        		{
        			...
        			{		//如果是新用户的魔百和业务，在宽带撤单时，需要终止掉魔百和业务
                		 ...
                		 * 进行魔百和拆机
                		 */
                    	IDataset destroyResult=CSAppCall.call("SS.DestroyTopSetBoxRegSVC.tradeReg", param);
        ...
        /********************** IMS固话相关资料处理 **************************/
        //宽带开户同时开通IMS固话时，IMS固话信息放在other表中，没有生成IMS固话订单，如IMS没出库，IMS不需要做任何处理。
        /*String imsTradeId = tradeInfo.getString("RSRV_STR7","");
        if (StringUtils.isNotBlank(imsTradeId))
        {
        	dealImsTradeCancel(pubData, input, imsTradeId);
        }*/
        ...
        if("HNKF".equals(cityCode) && (recvObject != null && !"".equals(recvObject))) {
			...
			String noticeContent = "宽带账户：" + serialNumberbak + "; 用户名：" + custName + "; 装机地址：" + installAddr + "。 10086已撤单，如果该工单已派发至安装师傅，请及时通知装机师傅无须上门，请知晓! " + "撤单时间：" + SysDateMgr.getSysTime();
            ...
            if (!IDataUtil.isEmpty(Bdataset)){
            	for(int i = 0; i < Bdataset.size(); i++){
        			...
        			noticeContent = "宽带账户：" + serialNumberbak + "; 用户名：" + custName + "; 装机地址：" + installAddr + "。 10086已撤单，如果该工单已派发至安装师傅，请及时通知装机师傅无须上门，请知晓! " + "撤单时间：" + SysDateMgr.getSysTime();
...
//        String strFeeState = (lOperFee + lAdvancePay + lforegift == 0) ? "0" : "1";
     ...
     * @throws Exception
     */
    private void dealImsTradeCancel(IData pubData, IData pgData, String tradeId) throws Exception
    {
    	String acceptMonth = StrUtil.getAcceptMonthById(tradeId);
    	IDataset imstradeInfos = UTradeInfoQry.qryTradeByTradeIdMonth(tradeId, acceptMonth, "0");
    	
    	if (IDataUtil.isNotEmpty(imstradeInfos))
    	{
    		IData imsTradeInfo = imstradeInfos.getData(0);
            ...
            if (!Dao.delete("TF_B_TRADE", imsTradeInfo, keys, Route.getJourDb(BizRoute.getTradeEparchyCode())))
            {
                String msg = "删除IMS固话订单【" + imsTradeInfo.getString("TRADE_ID") + "】失败!";
                CSAppException.apperr(TradeException.CRM_TRADE_95, msg);
            }
    ...
    public IDataset queryUserCancelTradeMerge(IData inData) throws Exception
    {
        ...
        IDataset tradeInfos = WidenetTradeQuery.queryUserCancelTrade("KD_" + serialNumber, tradeTypeCode);
        
        //add by xuzh5 2018-9-25 17:34:43 REQ201807060009优化撤单和拆机权限管控模式
        if(StaffPrivUtil.isFuncDataPriv(getVisit().getStaffId(), "WIDE_CANCEL_VERIFY_CODE")){
