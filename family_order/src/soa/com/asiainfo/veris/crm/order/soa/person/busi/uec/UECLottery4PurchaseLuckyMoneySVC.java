...
public class UECLottery4PurchaseLuckyMoneySVC extends CSBizService {
    ...
    public IData lottery(IData data) throws Exception
    {
        ...
        if (prizeNumber1 != 0 || prizeNumber2 != 0 || prizeNumber3 != 0 || prizeNumber4 != 0 || prizeNumber5 != 0 || prizeNumber6 != 0)
        {
            ...
            {
            	String limitPrizeStr = checkResult.getString("LIMIT_PRIZE", "");
            	
            	int radix,prizeOdds0,prizeOdds1,prizeOdds2,prizeOdds3,prizeOdds4,prizeOdds5,prizeOdds6;
            	radix=prizeOdds0=prizeOdds1=prizeOdds2=prizeOdds3=prizeOdds4=prizeOdds5=prizeOdds6=0;
 
            	//不中奖几率
            	if(limitPrizeStr.indexOf("0") >= 0){
            		try {
						prizeOdds0 = Integer.parseInt(paramMap.getString("PRIZE_ODDS_0", "0"));
						radix = radix + prizeOdds0;
					} catch (Exception e) {
					}           		
            	}
            	//一等奖几率
            	if(limitPrizeStr.indexOf("1") >= 0){
            		try {
            	...
            	//六等奖几率
            	if(limitPrizeStr.indexOf("6") >= 0){
            		try {
						prizeOdds6 = Integer.parseInt(paramMap.getString("PRIZE_ODDS_6", "0"));
						radix = radix + prizeOdds6;
					} catch (Exception e) {
					}            		
            	}

                
                int prize1,prize2,prize3,prize4,prize5,prize6 = 0;
                ...
                prize6 = (prizeOdds6 != 0) ? prize5 + prizeOdds6:prize5;
                ...
                else if (!winFlag && usrRandom <= prize6 && prizeNumber6 > 0)
                {
                    data.put("PRIZE_TYPE_CODE", "6");
                    result.put("X_RESULTINFO", "抽中奖项六");
                    result.put("LEVEL", "6");
    ...
    private IData checkLottryRight(IData data) throws Exception
    {
        IData paramMap = getParamMap(data);
        ...
        data.put("LIMIT_PRIZE", param.get("ov_v_limit_prize"));
    ...
    private void recordHisMainTrade(IData data) throws Exception
    {
        ...
        Dao.executeUpdateByCodeCode("TF_BH_TRADE", "INS_ALL", iparam, Route.getJourDb(BizRoute.getTradeEparchyCode()));
    }

    private void recordSubTrade(IData data) throws Exception
    {
        ...
        Dao.executeUpdateByCodeCode("SMS", "INS_UEC_LOTTERY_TRADE", subTrade, Route.getJourDb(BizRoute.getTradeEparchyCode()));
    }
