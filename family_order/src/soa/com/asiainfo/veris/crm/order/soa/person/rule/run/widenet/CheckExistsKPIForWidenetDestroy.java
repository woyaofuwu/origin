...
public class CheckExistsKPIForWidenetDestroy extends BreBase implements IBREScript{
			...
			if(saleActivelist != null && saleActivelist.size() > 0){
				for (int i = 0; i < saleActivelist.size(); i++) {
					...
					if(IDataUtil.isNotEmpty(commParaInfo6600)){
		                ...
		                {
		                	BreTipsHelp.addNorTipsInfo(databus, BreFactory.TIPS_TYPE_ERROR, "190315", "客户存在生效的60元报停专项款，不能立即拆机！");
		                }
					}
					IDataset commParaInfo6601 = CommparaInfoQry.getCommparaByCodeCode1("CSM", "6601", productId, packageId);
					if(IDataUtil.isNotEmpty(commParaInfo6601)){
		                ...
		                if (date.compareTo(dateNow) > 0)
		                {
		                	BreTipsHelp.addNorTipsInfo(databus, BreFactory.TIPS_TYPE_ERROR, "190316", "客户存在生效的60元话费礼包，不能立即拆机！");
		                }
					}
				}
			}
