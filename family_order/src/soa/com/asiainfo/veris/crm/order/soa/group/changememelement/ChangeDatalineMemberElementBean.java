...
public class ChangeDatalineMemberElementBean extends GroupOrderBaseBean {
                ...
                if (null != dataline && dataline.size() > 0) {
                    for (int j = 0; j < dataline.size(); j++) {
                        ...
                        if (numberCode.equals(lineNumberCode)) {
                            ...
                            // GrpInvoker.ivkProduct(memberMaps, BizCtrlType.ChangeMemberDis, "ChangeMemberDis");
                        }
                    }
                    ...
                    // 变更成员
                    memberMaps = (IData) Clone.deepClone(maps);
                    String productId = memberMaps.getString("PRODUCT_ID");
                    if ("7012".equals(productId)||"70121".equals(productId)||"70122".equals(productId)) {
                        ...
                        memberMaps.put("PRODUCT_ID", mebOfferCode);
                        // 查询专线信息
                        ...
                        if (IDataUtil.isNotEmpty(datalineList)) {
                            ...
                            memberMaps.put("USER_ID", userIdB);
                        }
                        //根据53资费来的，不需要判断了
                       /* IData result  =  DatalineEspUtil.getOnceDiscntAttr(SysDateMgr.getSysDate(SysDateMgr.PATTERN_STAND), memberMaps.getString("USER_ID"), memberMaps.getString("PRODUCT_ID"));
                        Boolean onceDiscnt  = result.getBoolean("FLAG");
                        if(onceDiscnt){
                    		inter.put("pam_NOTIN_INSTALLATION_COST", "0");
                    	}*/
                        memberMaps.put("ELEMENT_LIST", DatalineUtil.getElementInfo(map.getDataset("ELEMENT_INFO_LIST"), inter, mebOfferCode));
                        GrpInvoker.ivkProduct(memberMaps, BizCtrlType.ChangeMemberDis, "ChangeMemberDis");
                    }
                }
    ...
    public void changeMemberElement(IData maps, IDataset commonData, IData datas, String mainTradeId, String lineNumberCode, IData mapInfos, IDataset internet, IData inter) throws Exception {
        ...
        if ("1".equals(flag)) {
            ...
            maps.put("ELEMENT_LIST", DatalineUtil.getElementInfo(maps.getDataset("ELEMENT_INFO_LIST"), datas, mebOfferCode));
