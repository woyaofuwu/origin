...
public class ChangeJKDTBaseParamOfferTrade extends ChangeUserElement
{
	private final static Logger logger = Logger.getLogger(ChangeJKDTBaseParamOfferTrade.class);
                ...
                ||GroupBaseConst.MERCH_STATUS.MERCH_PASTE_MEBFLUX.getValue().equals(merchOperType)||GroupBaseConst.MERCH_STATUS.MERCH_CONTINUE_MEBFLUX.getValue().equals(merchOperType))
        {
            ...
            else if (GroupBaseConst.MERCH_STATUS.MERCH_PREDESTORY.getValue().equals(merchOperType))// 商品操作类型为预取消
            {
                ...
                userExtenData.put("USER_STATE_CODESET", "5");
                    ...
                    ||GroupBaseConst.MERCH_STATUS.MERCH_CONTINUE_MEBFLUX.getValue().equals(merchOperType) && isCredit))
            {// 商品操作类型为冷冻期取消恢复商品订购
                userStatus = "A";
    ...
    public void infoRegDataEntireMerch(String merchOperType) throws Exception
    {
        ...
        if (merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_PASTE.getValue()) || merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_CONTINUE.getValue())
                || merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_CANCLEPREDESTORY.getValue()) || merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_PREDESTORY.getValue())
                ||merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_PASTE_MEBFLUX.getValue())||merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_CONTINUE_MEBFLUX.getValue()))
        {// 商品操作类型为暂停，预取消，恢复，冷冻期取消恢复商品订购
            if (merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_PASTE.getValue()) || merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_PASTE_MEBFLUX.getValue()))
            {
                merchInfo.put("STATUS", "N");
            }
            else if (merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_CONTINUE.getValue()) || merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_CANCLEPREDESTORY.getValue())
                    || merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_CONTINUE_MEBFLUX.getValue()))
            {
                merchInfo.put("STATUS", "A");
            }
            else if (merchOperType.equals(GroupBaseConst.MERCH_STATUS.MERCH_PREDESTORY.getValue()))
            {
                merchInfo.put("STATUS", "D");
            }
        }

      
        // 登记BBOSS侧商品表信息
        super.addTradeEcrecepOffer(IDataUtil.idToIds(merchInfo));//daidl

        IData epData = GrpCommonBean.actEcrecepProcedure(reqData.getUca().getUser().getUserId(), SeqMgr.getInstId(), reqData.getUca().getUser().getSerialNumber(), merchInfo.getString("MERCH_ORDER_ID"), merchInfo.getString("MERCH_OFFER_ID", ""), merchOperType, "A", "A");
    ...
    public void infoRegDataOther(String merchOperType) throws Exception
    {
        ...
        if (auditorInfosDS != null && auditorInfosDS.size() > 0)
        {
            for (int i = 0; i < auditorInfosDS.size(); i++)
            {
                ...
                if(!"6".equals(CSBizBean.getVisit().getInModeCode())){//正向发起
                	auditorInfos.put("RSRV_STR5", "898SUPERUSR");
                 }
        ...
        if (contactorInfosDS != null && contactorInfosDS.size() > 0)
        {
            for (int i = 0; i < contactorInfosDS.size(); i++)
            {
                ...
                if(!"6".equals(CSBizBean.getVisit().getInModeCode())){//正向发起
               	 contactorInfos.put("RSRV_STR5", "898SUPERUSR");
               }else{
        ...
        }else {//批量任务过来的没有联系人信息 导致发PF报错  这里容错处理
            addContactorInfo(dataset);
        }
     ...
     * @date 2017-03-14
     */
    protected void addContactorInfo(IDataset dataset) throws Exception
    {
        IData contactorInfo2 = new DataMap();
        IData contactorInfo5 = new DataMap();
        IData staffInfo=UStaffInfoQry.qryStaffInfoByPK(CSBizBean.getVisit().getStaffId());
        if(IDataUtil.isNotEmpty(staffInfo)){
            contactorInfo2.put("USER_ID", reqData.getUca().getUser().getUserId());
            contactorInfo2.put("RSRV_VALUE_CODE", "CONTACTOR_INFOS");
            contactorInfo2.put("RSRV_STR1", reqData.getUca().getProductId());
            contactorInfo2.put("RSRV_STR2", "2");
            contactorInfo2.put("RSRV_STR3", staffInfo.getString("STAFF_NAME","SUPERUSR"));
            contactorInfo2.put("RSRV_STR4", staffInfo.getString("SERIAL_NUMBER","13900000000"));
            contactorInfo2.put("RSRV_STR5", "898SUPERUSR");
            contactorInfo2.put("START_DATE", getAcceptTime());
            contactorInfo2.put("END_DATE", getAcceptTime());
            contactorInfo2.put("MODIFY_TAG", TRADE_MODIFY_TAG.Add.getValue());
            contactorInfo2.put("INST_ID", SeqMgr.getInstId());
            contactorInfo2.put("REMARK", "商品注销-联系人信息修正");
            dataset.add(contactorInfo2);

            contactorInfo5.put("USER_ID", reqData.getUca().getUser().getUserId());
            contactorInfo5.put("RSRV_VALUE_CODE", "CONTACTOR_INFOS");
            contactorInfo5.put("RSRV_STR1", reqData.getUca().getProductId());
            contactorInfo5.put("RSRV_STR2", "5");
            contactorInfo5.put("RSRV_STR3", staffInfo.getString("STAFF_NAME","SUPERUSR"));
            contactorInfo5.put("RSRV_STR4", staffInfo.getString("SERIAL_NUMBER","13900000000"));
            contactorInfo5.put("RSRV_STR5", "898SUPERUSR");
            contactorInfo5.put("START_DATE", getAcceptTime());
            contactorInfo5.put("END_DATE", getAcceptTime());
            contactorInfo5.put("MODIFY_TAG", TRADE_MODIFY_TAG.Add.getValue());
            contactorInfo5.put("INST_ID", SeqMgr.getInstId());
            contactorInfo5.put("REMARK", "商品注销-联系人信息修正");
            dataset.add(contactorInfo5);
        }
    }
    /*
    ...
    protected void setTradeBase() throws Exception
    {
        ...
        // 4- CREDIT00 信控云MAS业务，产品发PF，商品不发PF
        String merchSpecCode = GrpCommonBean.productJKDTToMerch(reqData.getUca().getProductId(), 0);
        if("0".equals(CSBizBean.getVisit().getInModeCode()) && BbossIAGWCloudMASDealBean.isIAGWCloudMASCredit(merchSpecCode)) {
            data.put("OLCOM_TAG", "0");
        }
...
//        }
    ...
    protected void init() throws Exception
    {
        ...
        }else if(GroupBaseConst.MERCH_STATUS.MERCH_MODIFY_DISCNT.getValue().equals(merchOperType)){
        	ctrlType = BizCtrlType.ChangeUserChangeDiscnt;
        }else if(GroupBaseConst.MERCH_STATUS.MERCH_PREDESTORY.getValue().equals(merchOperType)){
        	ctrlType = BizCtrlType.ChangeUserPreDel;
        }else if(GroupBaseConst.MERCH_STATUS.MERCH_CANCLEPREDESTORY.getValue().equals(merchOperType)){
        	ctrlType = BizCtrlType.ChangeUserCanclePreDel;
        }else if(GroupBaseConst.MERCH_STATUS.MERCH_PASTE_MEBFLUX.getValue().equals(merchOperType)){
        	ctrlType = BizCtrlType.ChangeUserPastMebFlux;
        }else if(GroupBaseConst.MERCH_STATUS.MERCH_CONTINUE_MEBFLUX.getValue().equals(merchOperType)){
        	ctrlType = BizCtrlType.ChangeUserContinueMebFlux;
        }else if(GroupBaseConst.MERCH_STATUS.MERCH_MODIFY_PARAM.getValue().equals(merchOperType)){
