...
public class FinancialBossCancel implements ITradeFinishAction
{

	protected static final Logger log = Logger.getLogger(FinancialBossCancel.class);
    private void addResLog(RegTradeData rtd) throws Exception
    {
        ...
        if (resTradeDatas != null && resTradeDatas.size() > 0)
        {
            for (ResTradeData resTradeData : resTradeDatas)
            {
                ...
                Dao.insert("TF_B_TRADE_RESOURCE_INTER", resourceTradeData.toData(),Route.getJourDb());
            }
        }
        ...
        insertData.put("SUB_LOG_ID", tradeId);
        insertData.put("CANCEL_TAG", cancelTag);
        Dao.executeUpdateByCodeCode("TF_R_RESOURCE_INTER", "INS_BY_TRADERESOURCE", insertData);
    }
    ...
    public void deal(RegTradeData rtd) throws Exception
    {
        ...
        this.dealWithSimCard(rtd, rtd.getTradeDatas(TradeTableEnum.TRADE_RES.getValue()));
        this.addResLog(rtd);
    }
    ...
    private void dealFeeTax(RegTradeData rtd) throws Exception
    {
        ...
        if (feeTaxTrades != null && feeTaxTrades.size() > 0)
        {
            for (FeeTaxTradeData feeTaxTrade : feeTaxTrades)
            {
                ...
                int oldFee = Integer.parseInt(feeTaxTrade.getSalePrice());
                int fee = Integer.parseInt(feeTaxTrade.getBusiFee());
                ...
                this.insertFinancialBossTrade("20", feeTaxTrade.getFeeTypeCode(), fee, oldFee, 0, "", "", "", 0, "", taxFee, rate, noTaxFee, taxTag1, presentFee, rtd);
            }
        }
    }
    ...
    private void dealWithFeeSub(RegTradeData rtd) throws Exception
    {
        ...
        if (feeTrades == null || feeTrades.size() == 0)
        {
            if ("234".equals(tradeTypeCode))
            {
                ...
                this.insertFinancialBossTrade("10", "10", fee, fee, 0, rtd.getMainTradeData().getRsrvStr5(), rtd.getMainTradeData().getRsrvStr8(), "", 0, "", "", "", "", "", "", rtd);
            }
            return;
        }
        int fee = 0;
        ...
        for (FeeTradeData feeData : feeTrades)
        {
            ...
            if ("10".equals(feeTypeCode) && "0".equals(feeMode) && !"146".equals(tradeTypeCode)&& !"141".equals(tradeTypeCode))
            {
                continue;
            }
    ...
    private void dealWithSimCard(RegTradeData rtd, List<ResTradeData> resTrades) throws Exception
    {
        ...
        if ("9721".equals(rtd.getMainTradeData().getTradeTypeCode()))
        {
            return;
        }
        ...
        for (ResTradeData resTrade : resTrades)
        {
            ...
            {
                this.insertFinancialBossTrade(feeType, feeItem, fee, oldFee, formFee, resTrade.getRsrvStr2(), resTrade.getRsrvStr1(), "", 0, "1", "", "", "", "", "", rtd);
                // 查询sim卡的资源相关信息
                if ("10".equals(rtd.getTradeTypeCode()) || "142".equals(rtd.getTradeTypeCode()))
                {
                    if ("A".equals(resTrade.getRsrvTag2()))
                    {
                        ...
                        this.insertFinancialBossTrade(feeType, feeItem, (-1) * formFee, (-1) * formFee, formFee, resTrade.getRsrvStr2(), resTrade.getRsrvStr1(), "", 0, "2", "", "", "", "", "", rtd);
                        break;
                    }
                }
            }
    ...
    public void executeAction(IData mainTrade) throws Exception
    {
        ...
        if ("502".equals(mainTrade.getString("TRADE_TYPE_CODE")))
        {
            return;
        }
            ...
            String noTaxFee, String taxTag1, String taxFee3, RegTradeData rtd) throws Exception
    {
        ...
        Dao.insert("TF_B_TRADE_BFAS_IN", financialData.toData(),Route.getJourDb());
        // 执行完工处理
        IData insertData = new DataMap();
        insertData.put("SUB_LOG_ID", financialData.getSubLogId());
        insertData.put("CANCEL_TAG", financialData.getCancelTag());
        Dao.executeUpdateByCodeCode("TF_A_BFAS_IN", "INS_BY_TRADEBFASIN", insertData);
    }
}
