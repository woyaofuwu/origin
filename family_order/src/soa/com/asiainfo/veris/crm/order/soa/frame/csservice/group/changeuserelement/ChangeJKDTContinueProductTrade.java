...
public class ChangeJKDTContinueProductTrade extends ChangeUserElement {
    ...
    public void actTradePrdParams() throws Exception {
        ...
        if(!"6".equals(CSBizBean.getVisit().getInModeCode()))//如果是省boss发起则填写默认的暂停原因，平台要求必填
        {
             ...
             attrMap.put("ATTR_CODE", "9003");
             attrMap.put("ATTR_VALUE", "恢复");
             attrMap.put("ATTR_GROUP", "");
             attrMap.put("ATTR_NAME", "恢复原因");
             params.add(attrMap);
...
//        }
    ...
    /**
     * @description 恢复时将集团资费截止日期改为2050
     * @author songxw
     * @date 2019-06-18
     */
    protected void infoRegDataDiscnt() throws Exception {
        ...
        IDataset userDiscnts = UserEcrecepProductInfoQry.qryEcrecepDiscntByUserIdAndRsrvNum1(userId, "1");
        if(userDiscnts != null && userDiscnts.size() > 0){
	        IData userDiscnt = userDiscnts.getData(0);
	        userDiscnt.put("MODIFY_TAG", "2");
	        userDiscnt.put("END_DATE", SysDateMgr.getTheLastTime());
	        userDiscnt.put("RSRV_NUM1", "0");//0-正常状态1-暂停状态
    ...
    {
        // 产品控制信息
        BizCtrlInfo ctrlInfo = BizCtrlBean.qryProductCtrlInfo(productId, BizCtrlType.ChangeUserContinue);
