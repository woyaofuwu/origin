 ...
 * @create: 2018-10-15 16:00
 **/

public class CreateJKDTOfferTrade extends CreateGroupUser{

	private final static Logger logger = Logger.getLogger(CreateJKDTOfferTrade.class);
    ...
    public void actTradeSub() throws Exception
    {
        ...
        //BBOSS商品业务台帐子表
        super.addTradeEcrecepOffer(merchInfo);
    ...
    public void infoRegDataEntireMerch() throws Exception
    {
        ...
        merchInfo.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
        ...
        IData epData = GrpCommonBean.actEcrecepProcedure(reqData.getUca().getUser().getUserId(),
                SeqMgr.getInstId(), reqData.getUca().getUser().getSerialNumber(), merchInfo.getString("MERCH_ORDER_ID"),
                merchInfo.getString("MERCH_OFFER_ID", ""), "1", "A", "A");
    ...
    public void infoRegDataOther() throws Exception
    {
        ...
        if (attInfosDS != null && attInfosDS.size() > 0)
        {
            ...
            for (int i = 0; i < attInfosDS.size(); i++)
            {
                ...
                attInfo.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
        ...
        if (auditorInfosDS != null && auditorInfosDS.size() > 0)
        {
            for (int i = 0; i < auditorInfosDS.size(); i++)
            {
                ...
                if(!"6".equals(CSBizBean.getVisit().getInModeCode())){//正向发起
                	auditorInfo.put("RSRV_STR5", "898SUPERUSR");
                 }
                auditorInfo.put("START_DATE", getAcceptTime());
                auditorInfo.put("END_DATE", SysDateMgr.getTheLastTime());
                auditorInfo.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
        ...
        if (contactorInfosDS != null && contactorInfosDS.size() > 0)
        {
            for (int i = 0; i < contactorInfosDS.size(); i++)
            {
                ...
                contactorInfos.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
        ...
        // 5- 渠道信息入表
        logger.info("reqData(hzl)=="+ reqData.toString());
        logger.info("MERCH_INFO(hzl)=="+ reqData.getMERCH_INFO().toString());
        IData channelInfos = reqData.getMERCH_INFO().getData("CHANNEL_INFO");
        ...
        {       
            ...
            if(!"".equals(channelInfos.getString("CHANNEL_ID",""))) {
            	...
            	channelInfo.put("RSRV_STR1", reqData.getUca().getProductId());
            	channelInfo.put("START_DATE", getAcceptTime());
            	channelInfo.put("END_DATE", SysDateMgr.getTheLastTime());
            	channelInfo.put("MODIFY_TAG", CSBaseConst.TRADE_MODIFY_TAG.Add.getValue());
            	channelInfo.put("INST_ID", SeqMgr.getInstId());
    ...
    protected void setTradeBase() throws Exception
    {
        ...
        if(specProductId.equals("010113001") && "6".equals(CSBizBean.getVisit().getInModeCode())){
        	//如果是企业视频彩铃商品，则执行时间不带时间戳
