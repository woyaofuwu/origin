Wade.nav.KeyEvent = {
	...
	onDocumentContextMenu:function(e){
		var target = e.target;
		if(target && target.nodeType && !target.disabled
			&& (
				$.nodeName(target,"input") 
				|| $.nodeName(e.target,"textarea")
			)){
			return true;
		}
		if(e.stopEvent){ //for wade3
			e.stopEvent();
		}
		return false;
	}
...
Wade.nav.createAdapter({
	init:function(win,doc){
		if(win && doc && win.Wade){
			if(!window.frame_isdebug){
				if($.browser.msie){ //屏蔽帮助按钮
					Wade.nav.KeyEvent.reHelp(win);
				}
				if(win.System){ //for wade3 
					win.System.fly(doc).on("keydown", Wade.nav.KeyEvent.onDocumentKeyDown);
					win.System.fly(doc).on("contextmenu", Wade.nav.KeyEvent.onDocumentContextMenu);
				}else{
	...
	reload:function(url, params){ //重载当前界面
