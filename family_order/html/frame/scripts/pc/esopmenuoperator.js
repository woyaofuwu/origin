...
(function(){
	...
	$.navmenu = {	
		...
		loadL1Menu: function(domainId){
			if(!domainId){
				MessageBox.alert("提示信息", "domainId 为空！");
				return;
			}
			...
			$.httphandler.get(menuClazz, "loadL1Menu", "&DOMAIN_ID=" + domainId, function(data){
				...
				if(data){
					if(data.indexOf("error:") == 0){
						MessageBox.alert("提示信息", "加载失败:"+data.replace(/error:/ig, ""));
					}else{
		...
		loadL2Menu: function(menuId,domainId){
			if(!menuId){
				MessageBox.alert("提示信息", "menuId为空");
				return;
			}
			//$.beginPageLoading('加载中...');
	...
	$.navsearch = {
		...
		onProductItemTap: function(el, offerId, offerType, idx){
			...
			if(parent.$ && parent.$.menuopener){
				...
				if(url) {
					parent.$.menuopener.openMenu("", title, url);
					return ;
				}else{
					title = "商品详情-" + k;
	...
	function welSwitch(e, idx){
		if(idx==3 && $(e.target).attr("id")=="welTab"){
			//parent.$.login.openCustCentre();
			openNav('跨省工单','provinceWork','init','','');
			return false;
		}
	}
