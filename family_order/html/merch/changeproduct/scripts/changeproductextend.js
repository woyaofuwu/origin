...
(function () {
    $.extend(ChangeProductExtend.prototype, {
        ...
        afterLoadChildInfo: function (data) {
            var oldVpmnDiscntTag = data.get("OLD_VPMN_DISCNT_TAG");
            var oldVpmnDiscnt = data.get("OLD_VPMN_DISCNT");
            var oldVpmnDiscntName = data.get("OLD_VPMN_DISCNT_NAME");
            var bookingProduct = data.get("BOOKING_PRODUCT");
            var vpmnUserIdA = data.get("VPMN_USER_ID_A");
            var vpmnProductId = data.get("VPMN_PRODUCT_ID");
            var vipClassId = data.get("VIP_CLASS_ID");
            var nextProductId = data.get("NEXT_PRODUCT_ID");
            var bookingDatePriv = data.get("BOOKING_DATE_PRIV");

            if (typeof oldVpmnDiscntTag !== "undefined"
                    && oldVpmnDiscntTag != null && oldVpmnDiscntTag !== "") {
            	$("#VPMN_TITLE").css("display", "");
...
//   gh         selectedElements.effectBookingDate();
        },
