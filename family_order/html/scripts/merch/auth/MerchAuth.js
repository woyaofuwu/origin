(function($){
	$.extend({auth:{
		...
		init:function(){
			...
			$.auth.ruleActionFlag = btnObj.attr("ruleActionFlag")
			...
			*/
			...
			if(top.document.getElementById("LOGIN_NUM_ID")){
				var loginVal = top.document.getElementById("LOGIN_NUM_ID").value;
				if(loginVal) {
                    if(loginVal.length='14' && loginVal.substr(0,2) == 'KD') {
                        $("#AUTH_SERIAL_NUMBER").val(loginVal.substr(3,loginVal.length));
					}
					else {
                        $("#AUTH_SERIAL_NUMBER").val(loginVal);
                    }
                }
			}
            else{
		...
		//商品订购
		reloadAuth : function(tradeTypeCode, beforeAction, tradeAction) {
			var oldTradeTypeCode = $("#TRADE_TYPE_CODE").val();
			if (oldTradeTypeCode != tradeTypeCode) {
				$("#TRADE_TYPE_CODE").val(tradeTypeCode);
				$("#TRADE_TYPE_CODE").attr("orderTypeCode", tradeTypeCode);
			}
			var ucaData = $.auth.cacheData["AUTH_DATA"];
			if (!$.auth || !ucaData) {
				return;
			}
			$("#AUTH_SUBMIT_BTN").attr("beforeAction", beforeAction ? beforeAction : "");
			$("#AUTH_SUBMIT_BTN").attr("tradeAction", tradeAction ? tradeAction : "");
		...
		authStart:function(){
			...
			$("#AUTH_AFTER_CONTENT").css("display","none");
			...
			ajaxSubmit(null, null, param, $.auth.componentId, 
				function(data){ 
			  		...
			  		if(data && data.get("RESULT_CODE")==0){
			  			...
			  			popupDialog("用户选择", "components.auth.UserListNew", "queryUserList", "&cond_SERIAL_NUMBER=" +$("#AUTH_SERIAL_NUMBER").val()+"&cond_USER_LIST_SVC="+$.auth.userListSvc+"&Date="+new Date(), null, "600px", "330px", true, $.auth.afterSelectMoreUser, null);
		...
		afterSelectMoreUser:function(){
			...
			if (selectedUserId != ""){
				...
				//多用户选择之后外框需要同步刷新，已销户信息(通过userId)
                $.auth.personLoginByUserId(selectedUserId);
			}else{
				$.auth.beforeAuthCheck();
			}
		},
		...
		popAuthCheck:function(){
			...
			if($.os.phone) {
                popupPage('用户身份校验',"components.auth.AuthCheckNew", "init", param,null,"c_popup c_popup-full", $.auth.onAuthCheck, null);
			}
			else {
                popupDialog("身份校验", "components.auth.AuthCheckNew", "init", param, null, "30em","250px",true, $.auth.onAuthCheck, null);
			}
...
//								return;
		...
		ruleAction:function(obj){
            ...
            $.auth.updateLoginInfo();
		},
		//更新外框信息
		updateLoginInfo:function() {
            //更新外框信息
			if(top.document.getElementById("LOGIN_NUM")){
                var loginVal = top.document.getElementById("LOGIN_NUM").value;
                if ($("#AUTH_SERIAL_NUMBER").val() != loginVal) {
                	$.auth.personLogin();
                }
                else if(top.document.getElementById("LOGIN_NUM_ID")){
                    var loginVal = top.document.getElementById("LOGIN_NUM_ID").value;
                    if(loginVal == "") {
                        $.auth.personLogin();
                    }
		...
		personLogin:function () {
            ...
            loginNum.value = $("#AUTH_SERIAL_NUMBER").val();
            if (top.loginConfirm) {
                top.$("#LOGIN_PRAMS").val("noNeedLog");
                top.loginConfirm();
            }
		},
		//通过userId刷新外框信息
        personLoginByUserId:function (userId) {
			if(top.document.getElementById("LOGIN_USER_ID")) {
                var loginNum = top.document.getElementById("LOGIN_NUM");
                loginNum.value = $("#AUTH_SERIAL_NUMBER").val();
                var loginUserId = top.document.getElementById("LOGIN_USER_ID");
		...
		fireAction:function(obj){
			...
			//新的三户展示区
            $("#AUTH_NAME").html(obj.get("CUST_INFO").get("CUST_NAME"));
            $.auth.fillAfterAuth(obj);
            $("#AUTH_AFTER_CONTENT").css("display","");
		...
		setRuleActionFlag : function(flag) {
			if (flag) {
				$("#AUTH_SUBMIT_BTN").attr("ruleActionFlag", flag);
				$.auth.ruleActionFlag = flag;
			}
		},
		//简单密码校验
		...
		showUca:function(o, id, align, width) {
			$.auth.setFloatLayer(o, id, align, width);
            if($.os.phone) {
				$("#AfterAuthPart").addClass('c_float-show');
			}
			else{
                toggleFloatLayer(id,'block');
            }
		},
		fillAfterAuth:function(ucaInfo){
			var userInfo = ucaInfo.get("USER_INFO");
			var custInfo = ucaInfo.get("CUST_INFO");
			var productName = userInfo.get("PRODUCT_NAME");
			var cityName = userInfo.get("CITY_NAME");
			var openDate = userInfo.get("OPEN_DATE");
			var userStateCodeSet = userInfo.get("USER_STATE_CODESET");
			var userStateCodeSetName = userInfo.get("USER_STATE_CODESET_NAME");
			if(openDate && openDate.length>0){
				openDate = openDate.substr(0,10);
			}
			$("#AUTH_CITY_NAME").html(cityName);
			$("#AUTH_PRODUCT_NAME").html(productName);
			$("#AUTH_OPEN_DATE").html(openDate);
			if(userStateCodeSet=="0"){
				$("#AUTH_SIDE").html('<span class="e_tag e_tag-green">'+userStateCodeSetName+'</span>');
			}else{
				$("#AUTH_SIDE").html('<span class="e_tag e_tag-red">'+userStateCodeSetName+'</span>');
			}
			
			
			var tempPsptCode = custInfo.get("PSPT_TYPE_CODE");
            var psptTypeName =$("#PSPT_TYPE_PASSPORTTYPE_float").find("li[val="+tempPsptCode+"]").find("div").text();
            var psptId = custInfo.get("PSPT_ID");
			var psptEndDate = custInfo.get("PSPT_END_DATE");
			if(psptEndDate && psptEndDate.length>0){
				psptEndDate = psptEndDate.substr(0,10);
			}
			$("#AUTH_PSPT_TYPE_NAME").html(psptTypeName);
			$("#AUTH_PSPT_ID").html(psptId);
			$("#AUTH_PSPT_END_DATE").html(psptEndDate);
		}
	}});
	//页面初始化
	$($.auth.init);
