...
function checkAdvChgNewSn()
{
	...
	var param =	'&MEM_SERIAL_NUMBER='+newsn+'&GROUP_ID='+group_id + '&GROUP_ACCT_CITY_CODE=' + grpCityCode;
	$.ajax.submit('mebArea','getAdvChgNewSnInfo', param ,'createMebArea',
			function(data)
		    {
		    
		    	var isGrp = data.get("IsGrpNum");
		    	$('#IsGrpNum').val(isGrp);
		    	if(isGrp != null && isGrp != "" && isGrp == "false"){
		    		//手机号码校验的规则
			    	var params = '&GRP_CUST_ID=' + $("#CUST_ID").val() + 
			    		'&SERIAL_NUMBER=' + newsn + 
			    		'&ACCT_ID=' + $("#mem_ACCT_ID").val() + '&USER_ID=' + $("#USER_ID").val();
...
function onSubmitBaseTradeCheck(){
	...
	var reg = /^(([1-9][0-9]*)|0)([.]\d{1,2})?$/;//费用金额格式0.00校验:
	...
	if(limittype == 1 || limittype == 2){
		...
		var checkFlag = reg.test(limit);
		if(!checkFlag){
			alert("限定值的格式必须是数字或格式为0.00的数字");
			return false;
		}
		
		var dotLength = limit.indexOf(".");
		if(dotLength>0){
			if ((parseFloat(limit)*100) < 0 && limit !=''){
				alert("限定值不能小于0，请重新输入！");
				return false;
			}
		}
		
		if(limittype == 1 && limit !=''){
			
			if(dotLength > 0){
				if(parseFloat(limit)*100 > 500000){
					alert("限定方式为金额时，限定值不能大于500000，请重新输入！");
					return false;
				}
			} else {
				if(parseInt(limit)*100 > 500000){
					alert("限定方式为金额时，限定值不能大于500000，请重新输入！");
					return false;
				}
			}
		}
		$('#NEW_LIMIT').val(limit * 100);
	}
