...
function clearImsForm(){
	...
	$("#OLD_KD_USER_ID").html('');
	$("#OLD_FTTH_ACOUNT").html('');
	$("#OLDKD_SERIAL_NUMBER").val("");
}
...
function onSubmitBaseTradeCheck(){
	...
	{
		alert("输入新宽带账号[" + condKdSerialNumber + "]与办理业务新宽带账号[" + kdSerialNumber + "]不一致, 请重新输入! ");
		return false;
	}
	
	var  oldkdSerialNumber = $("#OLDKD_SERIAL_NUMBER").val();
	if(condKdSerialNumber == oldkdSerialNumber)
	{
		alert("输入新宽带账号[" + condKdSerialNumber + "]与原来绑定的宽带账号[" + oldkdSerialNumber + "]一致, 请重新输入! ");
		return false;
	}
