...
/**
 * 基本接入号自动生成
 * @author yinxm 2017-08-09
 * @desc HXYD-YZ-XTYH-20170712-151-PR17013245基本接入号生成问题
 * @return
 */
function getBaseBizInCode() {

    $.httphandler.post("com.asiainfo.veris.crm.iorder.web.igroup.param.ProxyParam", "getBaseBizInCode",'',
        function(data){
    		console.log(data);
            var extendCode = data.get(0,"EXTEND_CODE");
            $("#pam_BIZ_IN_CODE").val(extendCode);
            $("#bt_get").attr("disabled", true);
            $.endPageLoading();
        },function(error_code,error_info,derror){
            $.endPageLoading();
        });
...
function checkAccessNumberErr(data,accessNumber){
	$.endPageLoading();
	MessageBox.alert("","基本接入号["+accessNumber+"]校验出错，请重新校验！");
...
function afterCheckAccessNumber(data,accessNumber){
	...
	if(flag == "true"){
		MessageBox.alert("","基本接入号可以使用！");
		$('#pam_BIZ_IN_CODE').val(accessNumber.replace(/(^\s*)|(\s*$)/g, ""));
	}else{
		MessageBox.alert("","基本接入号["+accessNumber+"]不能使用，请重新生成！");
		$('#pam_BIZ_IN_CODE').val('');
		$('#pam_SVRCODETAIL').val('');
	}
	$.endPageLoading();
...
function commSubmit() {
	...
	//svcparamdata.put("SVRCODETAIL", $("#pam_SVRCODETAIL").val());
