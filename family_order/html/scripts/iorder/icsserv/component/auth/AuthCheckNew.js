	...
	$.extend({authCheck:{
		...
		init:function(){
			...
			$("#mySegment").change(function () {
                $.authCheck.checkRadio(this.value);
            });
			...
			if(TRADE_TYPE_CODE=="142"){
                cond_CHECK_MODE.remove("2");
                cond_CHECK_MODE.append("SIM卡号(或白卡号)+验证码","5");
                cond_CHECK_MODE.append("服务密码+验证码","6")
                cond_CHECK_MODE.append("有效证件+验证码","9")
                $("#cond_CHECK_MODE").val('5');
		...
		setReturnDialogValue : function(){
			...
			//cxy 补换卡类型
			authParam +="&REMOTECARD_TYPE="+$("#mySegment").val();//$("input[name='SELECT_TYPE']:checked").val();
...
//			var returnObj = $.parseJSON("{\""+handler+"\": \""+authParam+"\"}");
			var returnObj = $.parseJSON("{\"" + handler + "\": \"" + authParam + "\",\"POP_FIELD_NAME\": \"" + handler + "\"}");
		...
		checkId:function(){
			...
			if(idType=="0" || idType == "1" || idType == "3"){
				...
				if(TRADE_TYPE_CODE=="141" || TRADE_TYPE_CODE == "142" || TRADE_TYPE_CODE=="71" || TRADE_TYPE_CODE=="73"){
					$("#cond_PSPT_ID").val("");
					var select_type =$("#mySegment").val();//$("input[name='SELECT_TYPE']:checked").val();
					if(select_type=="0"){
						$("#cond_PSPT_ID").attr("disabled",true); 
					}
					if(HIGH_PRIV=="1"){
		...
		checkRadio:function(value){
			var select_type =value;
			if(select_type=="0"){
				cond_CHECK_MODE.append("证件号码+服务密码","4");
        ...
        resetSize:function(){
			if(parent.$(".c_dialog").length==1){
               var okBtnTop = $("#auth_ok_btn").offset().top;
               var okBtnHeight = $("#auth_ok_btn").height();
...
function setDefaultFocus(){
	...
	if(psptTypeObj && psptTypeObj.length){
		...
		$("#cond_PSPT_TYPE_CODE").val("0");
	}
    $("#cond_PSPT_TYPE_CODE").trigger("change");
    setTimeout(function () {
        $("#cond_CHECK_MODE").trigger("change");
    },10);
	// $.authCheck.setAuthCheckArea();
}
...
function scanIDCard() {
    if ($.os.phone) {
        $.MBOP.getMsgByEForm("AUTH_CHECK");
    } else {
        getMsgByEForm("cond_PSPT_ID", "E_NAME", null, null, null, "E_ADDRESS", null, null);
    }
}