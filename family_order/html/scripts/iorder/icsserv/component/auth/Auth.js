(function($){
	$.extend({auth:{
			...
			if ($("#AUTH_SUBMIT_BTN").attr("moreUser") == "true"){
				...
				if("327" == tradeTypecode){
					if(null == groupPack || ""== groupPack){
						MessageBox.alert("提示","集团产品包不能为空，请选择集团产品包！");
					    return false;
					}
				}
		...
		afterAuthCheck:function(data){
			...
			var tradeTypeCode = $("#TRADE_TYPE_CODE").val();
			if(tradeTypeCode == "330")
			{
				$.getScript("scripts/iorder/icsserv/common/des/des.js",function(){
				var checkpasstag = data.get("CHECKPASSTAG");
				var checkpasskey = data.get("CHECKPASSKEY");

                var passtag=strEnc("0"+checkpasskey,"c","x","y");

				if ( checkpasstag != passtag){
