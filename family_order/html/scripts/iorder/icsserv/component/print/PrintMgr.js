            ...
            if ($.printMgr.userRecptSvcConf.hadGot == false) {
                ...
                if($.auth && $.auth.getAuthData()){
                    ...
                    if(user){
                        ...
                        if ($.cssubmit && $.cssubmit.tradeData && $.cssubmit.tradeData.containsKey("USER_ID")) {
...
//			alert("printWithTip_printInfo:"+printInfo);
        ...
        printNewElecAcceptBill:function(notePrintData){
            ...
            if('10' == tradetypecode || '100'== tradetypecode || '60'== tradetypecode || '40'== tradetypecode
                || '73'== tradetypecode || '142'== tradetypecode|| '310'== tradetypecode || '416'== tradetypecode||'494'==tradetypecode
            ...
            //跨区销户k3
            if('1990'==tradetypecode || '10'==tradetypecode || '40'==tradetypecode
            		|| '60'==tradetypecode || '100'==tradetypecode || '310'==tradetypecode||'494'==tradetypecode){            	  var params1990 = "&ACTION=PSPT_FRONTBACK_HEADPIC";
                  params1990 += "&TRADE_ID="+notePrintData.get("TRADE_ID", "");
                  params1990 += "&SERIAL_NUMBER="+notePrintData.get("SERIAL_NUMBER", "");
                  ajaxSubmit(null, null, params1990, $.cssubmit.componentId, function(data)
                  {		  
                          notePrintData.put("CARD_FRONT_BASE64",data.get("FRONT_BASE64",""));//身份证正面
                          notePrintData.put("CARD_BACK_BASE64",data.get("BACK_BASE64",""));//身份证正面
                          notePrintData.put("HEAD_BASE64",data.get("HEAD_BASE64",""));//身份证头像
                          notePrintData.put("CARD_BORN",data.get("BORN",""));//出生日期
                          notePrintData.put("EFF_DATE",data.get("EFF_DATE",""));//证件有效期
                          notePrintData.put("CARD_SEX",data.get("SEX",""));//性别
                          notePrintData.put("CARD_ADDRESS",data.get("ADDRESS",""));//家庭住址
                          notePrintData.put("CARD_ISSUED",data.get("ISSUED",""));//发行机构
            ...
            var isQingTui="0";
            var param290="&ACTION=FOREGIFT_TRADE";
            debugger;
            param290 += "&TRADE_ID="+notePrintData.get("TRADE_ID", "");
            if('290' == tradetypecode)
            {
                ajaxSubmit(null, null, param290,  $.cssubmit.componentId, function(data)
                {
                    isQingTui=data.get("IS_QING_TUI","0");
                    if("1"==isQingTui){
                        notePrintData.put("liushuihao",data.get("liushuihao",""));
                        notePrintData.put("tuikuanzhonglei",data.get("tuikuanzhonglei",""));
                        notePrintData.put("tuikuanjine",data.get("tuikuanjine",""));
                        notePrintData.put("jinedaxie",data.get("jinedaxie",""));
                    }
            ...
            //add by liangdg3 at 20191028 for REQ201910180018押金业务受理及清退电子化存储需求 end
            ...
            /*******************REQ201805090016_在线公司电子稽核报表优化需求_end***********************************/
            //REQ201902190015  关于吉祥号码开户全量纳入稽核的接口改造需求star    
            var beautiful_tag = notePrintData.get("BEAUTIFUL_TAG","0");//String对象
            edocXml.push('	<is_luckynumber>'+beautiful_tag+'</is_luckynumber>');
            //跨区销户k3
      
 if('1990'==tradetypecode || '10'==tradetypecode || '40'==tradetypecode
            		|| '60'==tradetypecode || '100'==tradetypecode || '310'==tradetypecode||'494'==tradetypecode){            	edocXml.push('	<idcard_list>');
                edocXml.push('		<idcard_info>');
                edocXml.push('			<card_name>'+notePrintData.get("CUST_NAME", "")+'</card_name>');
                edocXml.push('			<card_sex>'+notePrintData.get("CARD_SEX", "")+'</card_sex>');
                edocXml.push('			<card_nation>'+notePrintData.get("CARD_NATION", "")+'</card_nation>');
                edocXml.push('			<card_address>'+notePrintData.get("CARD_ADDRESS", "")+'</card_address>');
                edocXml.push('			<card_no>'+notePrintData.get("ID_CARD", "")+'</card_no>');
                edocXml.push('			<card_born>'+notePrintData.get("CARD_BORN", "")+'</card_born>');
                edocXml.push('			<card_validity>'+notePrintData.get("EFF_DATE", "")+'</card_validity>');
                edocXml.push('			<card_issued>'+notePrintData.get("CARD_ISSUED", "")+'</card_issued>');
                edocXml.push('			<card_head_base64>'+notePrintData.get("HEAD_BASE64", "")+'</card_head_base64>');
                edocXml.push('			<card_front_base64>'+notePrintData.get("CARD_FRONT_BASE64","")+'</card_front_base64>');
                edocXml.push('			<card_back_base64>'+notePrintData.get("CARD_BACK_BASE64","")+'</card_back_base64>');
                edocXml.push('		</idcard_info>');
            	edocXml.push('	</idcard_list>');
            }
            //REQ201902190015  关于吉祥号码开户全量纳入稽核的接口改造需求end
    //add by liangdg3 at 20191028 for REQ201910180018押金业务受理及清退电子化存储需求 start
            if('290'==tradetypecode){
                if('1'==isQingTui){
                    edocXml.push('<is_refund>1</is_refund>');
                    edocXml.push('<liushuihao>'+notePrintData.get("liushuihao", "")+'</liushuihao>');
                    edocXml.push('<daibiaohaoma></daibiaohaoma>');
                    edocXml.push('<hushu></hushu>');
                    edocXml.push('<kaihuyinhang></kaihuyinhang>');
                    edocXml.push('<yinhangzhanghao></yinhangzhanghao>');
                    edocXml.push('<tuikuanzhonglei>'+notePrintData.get("tuikuanzhonglei", "")+'</tuikuanzhonglei>');
                    edocXml.push('<jinedaxie>'+notePrintData.get("jinedaxie", "")+'</jinedaxie>');
                    edocXml.push('<tuikuanjine>'+notePrintData.get("tuikuanjine", "")+'</tuikuanjine>');
                }else{
                    edocXml.push('<is_refund></is_refund>');
                }
            }
            //add by liangdg3 at 20191028 for REQ201910180018押金业务受理及清退电子化存储需求 end
        ...
        printNewPhoneElecAcceptBill:function(notePrintData){
            ...
            needCertificatePhoto=false;
            needPersonPhoto=false;
            if(tradetypecode=='494'){
                needCertificatePhoto=true;
                needPersonPhoto=true;
                personPhotoTip = "实际使用人";
            }
            ...
            var params={
                "signContent": edocStr   ,
                "needCertificatePhoto":needCertificatePhoto,//是否需要二代证拍照 true、false(必填)
                "needPersonPhoto": needPersonPhoto,//是否需要人像照拍照 true、false(必填)
