		...
		events:{
			onChangePostTag:function(){
				...
				if(postTag=="0" || postTag=="") {
					...
					$("#PostInfoPart li").addClass("e_hide");
                    $("#PostInfoPart li:first").removeClass("e_hide");
					$("#PostTypeContentPart,#EmailTypeContentPart,#MmsTypeContentPart").css("display", "none");

					//去掉必填样式
					$("#span_POST_TYPESET,#span_POSTTYPE_CONTENT,#span_POST_ADDRESS,#span_POST_NAME,#span_POST_FAX_NBR,#span_POST_EMAIL").removeClass("required");
				...
				//选择邮寄
					...
					//显示邮寄信息区
                    $("#PostInfoPart li").removeClass("e_hide");
					...
					*/
					//必填样式
					$("#span_POST_TYPESET").addClass("required");
					//必填限制
			...
			onClickPostTypeset:function(){
				...
				if($(this).attr("checked")) {
					if($(this).val()==0) {	//邮政投递
						$("#PostTypeContentPart").css("display", "");
						//必填样式
					    $("#span_POSTTYPE_CONTENT,#span_POST_ADDRESS,#span_POST_NAME").addClass("required");
			            ...
			            /********取消EMAIL********/
			            $("#span_POST_EMAIL").removeClass("required");
					    $("#POST_EMAIL").attr("nullable","yes");

					}else if($(this).val()==2) {
						$("#EmailTypeContentPart").css("display", "");
						//必填样式
						$("#span_EMAILTYPE_CONTENT,#span_EMAIL").addClass("required");
						...
						$("#MmsTypeContentPart").css("display", "");
						$("#span_MMSTYPE_CONTENT").addClass("required");
				...
				//取消邮寄方式
					...
					if($(this).val()==0) {
						...
						//去掉必填样式
					    $("#span_POSTTYPE_CONTENT,#span_POST_ADDRESS,#span_POST_NAME").removeClass("required");
						...
						//必填样式
						$("#span_EMAILTYPE_CONTENT,#span_EMAIL").removeClass("required");
						$("#POST_EMAIL").attr("nullable", "yes");

						$("input[name=EMAILTYPE_CONTENT]").attr("checked", false).attr("disabled", false);
					}else if($(this).val()==3) {
						$("#MmsTypeContentPart").css("display", "none");
						$("#span_MMSTYPE_CONTENT").removeClass("required");
