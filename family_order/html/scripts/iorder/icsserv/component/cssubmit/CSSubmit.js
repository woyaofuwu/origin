(function($){
	...
	cssubmit:{
		...
		*/
        ...
        // 权益弹窗条件查询
        getWelfarePop: function (orderId, tradeId, eparchyCode, orderTypeCode) {
            var param = "&ACTION=QRY_WELFARE";
            param += "&ORDER_ID=" + orderId;
            param += "&TRADE_ID=" + tradeId;
            param += "&EPARCHY_CODE=" + eparchyCode;
            param += "&ORDER_TYPE_CODE=" + orderTypeCode;
            $.beginPageLoading("加载是否存在关联权益商品。。。");
            ajaxSubmit(null, null, param, $.cssubmit.componentId, function (data) {
                $.endPageLoading();
                if (data) {
                    var action = data.get("ACTION");
                    var bookingTag = data.get("BOOKING_TAG");
                    var addWelfareOfferStr = data.get("ADD_WELFARE_OFFER_STR");
                    var delWelfareOfferStr = data.get("DEL_WELFARE_OFFER_STR");
                    var addProductId = data.get("ADD_PRODUCT_ID");
                    var addStartDate = data.get("ADD_START_DATE");
                    var delProductId = data.get("DEL_PRODUCT_ID");
                    var delEndDate = data.get("DEL_END_DATE");
                    var advanceOrderId = data.get("ADVANCE_ORDER_ID");
                    var advanceTradeId = data.get("ADVANCE_TRADE_ID");
                    var tipMsg = data.get("TIP_MSG");
                    var serialNumber = data.get("SERIAL_NUMBER");
					var userId = data.get("USER_ID");
					var addOfferId = data.get("ADD_OFFERS_ID");
                    if (action) $.cssubmit.commData.put("ACTION", action);
                    if (bookingTag) $.cssubmit.commData.put("BOOKING_TAG", bookingTag);
                    if (addWelfareOfferStr) $.cssubmit.commData.put("ADD_OFFER_STR", addWelfareOfferStr);
                    if (delWelfareOfferStr) $.cssubmit.commData.put("DEL_OFFER_STR", delWelfareOfferStr);
                    if (addProductId) $.cssubmit.commData.put("ADD_PRODUCT_ID", addProductId);
                    if (addStartDate) $.cssubmit.commData.put("ADD_START_DATE", addStartDate);
                    if (delProductId) $.cssubmit.commData.put("DEL_PRODUCT_ID", delProductId);
                    if (delEndDate) $.cssubmit.commData.put("DEL_END_DATE", delEndDate);
                    if (advanceOrderId) $.cssubmit.commData.put("ADVANCE_ORDER_ID", advanceOrderId);
                    if (advanceOrderId) $.cssubmit.commData.put("ADVANCE_TRADE_ID", advanceTradeId);
                    if (tipMsg) $.cssubmit.commData.put("TIP_MSG", tipMsg);
                    if (serialNumber) $.cssubmit.commData.put("SERIAL_NUMBER", serialNumber);
					if (userId) $.cssubmit.commData.put("USER_ID", userId);
					if (addOfferId) $.cssubmit.commData.put("ADD_OFFERS_ID", addOfferId);
                }
            }, function (code, info, detail) {
                $.endPageLoading();
                MessageBox.error("错误提示", "加载业务权益受理数据报错！", null, null, info, detail);
            }, {
                "async": false
            });
        },
        // 权益弹窗
        openWelfarePop: function () {
            var serialNumber = $.cssubmit.commData.get("SERIAL_NUMBER", "");
            var action = $.cssubmit.commData.get("ACTION", "");
            var bookingTag = $.cssubmit.commData.get("BOOKING_TAG", "");
            var addWelfareOfferStr = $.cssubmit.commData.get("ADD_OFFER_STR", "");
            var delWelfareOfferStr = $.cssubmit.commData.get("DEL_OFFER_STR", "");
            var addStartDate = $.cssubmit.commData.get("ADD_START_DATE", "");
            var delEndDate = $.cssubmit.commData.get("DEL_END_DATE", "");
            var advanceOrderId = $.cssubmit.commData.get("ADVANCE_ORDER_ID", "");
            var advanceTradeId = $.cssubmit.commData.get("ADVANCE_TRADE_ID", "");
            var addProductId = $.cssubmit.commData.get("ADD_PRODUCT_ID", "");
            var delProductId = $.cssubmit.commData.get("DEL_PRODUCT_ID", "");
			var addOfferId = $.cssubmit.commData.get("ADD_OFFERS_ID", "");
			var userId = $.cssubmit.commData.get("USER_ID", "");
            var param = "&SERIAL_NUMBER=" + serialNumber;
            param += "&ACTION=" + action;
            param += "&BOOKING_TAG=" + bookingTag;
            param += "&ADD_OFFER_STR=" + addWelfareOfferStr;
            param += "&ADD_PRODUCT_ID=" + addProductId;
            param += "&ADD_START_DATE=" + addStartDate;
            param += "&DEL_OFFER_STR=" + delWelfareOfferStr;
            param += "&DEL_PRODUCT_ID=" + delProductId;
            param += "&DEL_END_DATE=" + delEndDate;
            param += "&ADVANCE_ORDER_ID=" + advanceOrderId;
            param += "&ADVANCE_TRADE_ID=" + advanceTradeId;
			param += "&ADD_OFFERS_ID=" + addOfferId;
			param += "&USER_ID=" + userId;
            popupPage("权益商品受理", "welfare.WelfareTrade", "onInit", param, null, "c_popup c_popup-full c_popup-full-hasBg", null, null);
        },
		...
		*/
		...
		showMessage : function(result, title, content, isPrint) {
			...
			if (!msgPanel.length) {
                ...
                // 权益弹窗add
                var orderId = $.cssubmit.tradeData.get("ORDER_ID");
                var tradeId = $.cssubmit.tradeData.get("TRADE_ID");
                var eparchyCode = $.cssubmit.tradeData.get("EPARCHY_CODE");
                var orderTypeCode = $.cssubmit.tradeData.get("ORDER_TYPE_CODE");
                if (orderId && tradeId && eparchyCode && orderTypeCode) {
                    $.cssubmit.getWelfarePop(orderId, tradeId, eparchyCode, orderTypeCode);
                    if($.cssubmit.commData.get("ACTION")){
                        html.push('<button type="button" class="e_button-r e_button-red" ontap="$.cssubmit.openWelfarePop();" ><span class="e_ico-browse"></span><span>权益商品受理</span></button>');
                    }
                }
                // 权益弹窗end
				html.push('</div>');
