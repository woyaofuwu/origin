(function($){
	$.extend({tradeCheck:{
		...
		showRuleResult:function(data, tag, func){
			...
			var tradeTypeCode = data.get("TRADE_TYPE_CODE");
			//错误提示
			if(tag==0 && errorSet && errorSet.length>0){
				//中断错误提示，直接返回
				if('192'==tradeTypeCode || 192 == tradeTypeCode){
					//甩单这里要区分拒绝类规则和下沉规则
					$.ajax.submit('','tpRulePageCheck','','',function (param) {
						var showTag = param.get("SHOW_TAG");
						if("1" == showTag){
                            var custInfo = $.auth.getAuthData().get("CUST_INFO");
                            var param =  '&ERROR_SET='+encodeURI(encodeURI(errorSet))+'&ACCESS_NUMBER='+$("#AUTH_SERIAL_NUMBER").val()+
                                '&CUST_NAME='+encodeURI(encodeURI(custInfo.get("ORIGIN_CUST_NAME")))+'&TRADE_TYPE_CODE=192';
                            dialog = popupDialog('甩单规则列表', 'tpOrder.TpRuleList', 'init',param, '/order/iorder', '80em', '500px', true, null, null);
                            $(dialog.titleFnClose).css("display", "none");//隐藏弹出框的X
						}else {
		...
		showConfirm:function(confirmSet, warnSet, tag, func){
			...
			MessageBox.confirm("业务规则提示！", content, 
				function(btn){
					if(btn == "ok"){
						$.tradeCheck.showConfirm(confirmSet, warnSet, tag, func);
					}else{
