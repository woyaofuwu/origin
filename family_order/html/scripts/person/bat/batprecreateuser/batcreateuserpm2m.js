if(typeof(BatCreateUserM2M)=="undefined"){
	window["BatCreateUserM2M"]=function(){
		 
	};
	var batcreateuserpm2m = new BatCreateUserM2M();
}

(function(){
	$.extend(BatCreateUserM2M.prototype,{
		...
		PasswdbeforeAction:function() {
		    ...
		    var serialNumber = '11111111111';//$("#PHONE").val();
		...
		chkPsptTypeCode:function(fieldName){
			...
			//0-本地身份证 1-外地身份证
			psptObj.attr("datatype", (psptTypeCode==0 || psptTypeCode==1 || psptTypeCode==2)? "pspt" : "");						
			...
			if(fieldName == "USE_PSPT_TYPE_CODE"){
				...
				var useValue = useObj.val();
				
				if(useValue=="D" || useValue=="E" || useValue=="G" || useValue=="L" || useValue=="M")
				{							
					alert("使用人只能选择个人证件，请重新选择！");
					useObj.find("option[index=0]").attr("selected", true);
					$("#USES_PSPT_ID").attr("datatype", "");
					return;					
				}				
		...
		chkPsptId:function(fieldName){
			...
			if(psptTypeCode=="H"){
			   ...
			   //台湾居民回乡:证件号码为11位时，前10位为阿拉伯数字，最后一位为校验码，括号内为英文字母或阿拉伯数字；证件号码为8位时，均为阿拉伯数字。
				...
				//营业执照：证件号码长度需满足15位、13位、18位、20位、22位、24位  REQ201608090019_关于开户等界面证件核验优化的需求
		...
		identification: function(picid,picstream){
			...
			if(picid == "custInfo_RSRV_STR4_PIC_ID"){
				//责任人
				custName = $("#RSRV_STR2").val();
				psptId = $("#RSRV_STR4").val();
				psptType = $("#RSRV_STR3").val();
				fornt = $("#custInfo_RSRV_STR4_PIC_STREAM").val();
			}else{
				//经办人
