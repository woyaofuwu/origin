...
function queryProduct(node)
{
    ...
    $.beginPageLoading(); 
    var ifcheck =true;
    Wade.httphandler.submit('','com.asiainfo.veris.crm.order.web.frame.csview.group.changeuserelement.BaseInfoHttpHandler', 'queryProductInfo','&PRODUCT_ID='+node.value+'&CUST_ID='+$('#CUST_ID').val()+'&BUSI_TYPE='+$('#BUSI_TYPE').val(),
		function(data){
			if(data == null)
				return;
