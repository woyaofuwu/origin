var BaseInfoLib = {
	...
	//是否展示隐藏必填项
	toggleNeedInput:function(infoPart){
		var infoPart = $("#"+infoPart);
		if (!infoPart.hasClass("e_hide-x")){
			infoPart.addClass("e_hide-x");
			$(this).find("i").removeClass().addClass("e_ico-show");
			$(this).find("span").html("显示非必填项");
		}else {
			infoPart.removeClass("e_hide-x");
			$(this).find("i").removeClass().addClass("e_ico-hide");
			$(this).find("span").html("隐藏非必填项");
		}				
	},
	...
	setRequiredField:function(fieldId, tag){
		...
		$("#"+this.infoPart).find("#"+fieldId).attr("nullable", nullable);
		if(tag){
			$("#"+this.infoPart).find("#span_"+fieldId).addClass("e_required");
		}else{
			$("#"+this.infoPart).find("#span_"+fieldId).removeClass("e_required");
		}
