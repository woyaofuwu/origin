var SimCardOcx = {
	...
	getOcxVersion:function(){
		...
		if(this.PRE_SIM){
			...
			if(code !=0){
				alert(this.getErrMsg(strOcxVersion.substr(2)));
				return false;
			}
	...
	getICCID:function(){
		...
		if(this.PRE_SIM){
			...
			if(code!=0){
				alert(this.getErrMsg(strICCID.substr(2)));
				return false;
			}
			//0E0A为白卡号
			...
			if(strICCID == ""){
				alert("获取ICCID错误："+ this.getErrMsg());
				return false;
			}
			//获取卡号数
	...
	getPhoneNums:function(){
		...
		if(phoneNum==0){
			alert("获单号/多号卡错误："+ this.getErrMsg());
			return false;	
	...
	writeCard:function(str){
		...
		if(this.PRE_SIM){
			...
			if(result.substr(0,1)!="0"){
				alert("写卡错误："+ this.getErrMsg(result.substr(2)));
				return false;
			}
	...
	getCardInfo:function(){
		...
		if(code!=0){
			alert("读取卡片信息错误："+ this.getErrMsg(result.substr(2)));
			return false;
		}
