...
//			alert("printWithTip_printInfo:"+printInfo);
			if(printInfo){
				...
				MessageBox.alert("提示","即将打印"+printInfo.get("NAME")+"，请做好准备！",function(btn){				
					if(btn=="ok")
					{
						var printDoc = printInfo.get("PRINT_DATA");					
						$.printMgr.switchBeforeStartupPrint(printInfo,printDoc);
	...
	staffNote:{
		...
		createStaffNote:function(info, name, fee){
			...
			if (!msgPanel.length) {
				...
				msgArr.push('		<button type="button" onclick="$.staffNote.submitPrint(true)" id="TicketPrtBtn" class="e_button-page"><i class="e_ico-print"></i><span>打印</span></button>	');
				msgArr.push('		<button type="button" onclick="$.staffNote.submitPrint(false)" id="TicketNonPrtBtn"  class="e_button-page-cancel"><i class="e_ico-cancel"></i><span>取消</span></button>	');
				msgArr.push('	</div>	');
