	...
	cssubmit:{
		...
		*/
		...
		openKJConf:function(){
			...
			var param = '&USER_ID=' +userId+eparchyCodeparamStr+'&TRADE_TYPE_CODE='+tradetypecode;
		...
		openGrpKJConf:function(){
			...
			popupPage('group.creategroupacct.SetGrpElecInvoice', 'initial',param , '电子发票打印设置', 870,1380);
		...
		*/
		...
		showMessage : function(result, title, content, isPrint) {
			...
			if (!msgPanel.length) {
				...
				//------购物车
				if(isPrint){
					msgArr.push('<button  onclick=\"javascript:$.cssubmit.printTrade();\" class=\"e_button-page\"><i class=\"e_ico-print\"></i><span>打印纸质单据</span></button>');
				}
				...
				if (tradeTypeCode) {
					...
					if ($.cssubmit.commData.get("CAN_SET_EPRINTCEPT_CONF") && "TRUE"==$.cssubmit.commData.get("CAN_SET_EPRINTCEPT_CONF")){
						...
						if($.printMgr.userRecptSvcConf.custType != "GRP"){
							msgArr.push('<button  onclick=\"javascript:$.cssubmit.openKJConf();\" class=\"e_button-page\"><i class=\"e_ico-print\"></i><span>电子发票打印设置</span></button>');
						}else{							
							msgArr.push('<button  onclick=\"javascript:$.cssubmit.openGrpKJConf();\" class=\"e_button-page\"><i class=\"e_ico-print\"></i><span>电子发票打印设置</span></button>');
						}
					}						
				}
				}catch(e){alert(e);}
