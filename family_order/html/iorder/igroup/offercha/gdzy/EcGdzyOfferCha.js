...
function initPageParam_110000007348() {
	...
	//设置oldfee
	$("#pam_NOTIN_FEE_COST").attr("oldfee", "0");

	initCrtUs();
	
	if($("#pam_NOTIN_OLD_AttrGdzy").val()!="" && table.length==0)
	{
		$.each(new Wade.DatasetList($("#pam_NOTIN_OLD_AttrGdzy").val()),function(index,data) { 
			BroadbandTable.addRow($.parseJSON(data.toString()));
		});
	}
...
function initCrtUs() {
	var METHOD_NAME = $('#cond_OPER_TYPE').val();//$('#pam_NOTIN_METHOD_NAME').val();
...
function createData() {
	...
	//if(data != null && data.length > 0){
		...
		//$.validate.alerter.one($("#pam_NOTIN_PROJECT_NAME")[0], "只能新增一条费项参数,不可再新增!");
		//return false;
	//}
    ...
    var feeNameV = $("#pam_NOTIN_FEE_NAME").text();
    //收费金额(元)
	...
	{
		//alert("请填写项目名称!");
		$.validate.alerter.one($("#pam_NOTIN_PROJECT_NAME")[0], "请填写项目名称!");
		return false;
	}
	...
	if (currLength > 100 && currLength != 0) 
    {
    	...
    	$.validate.alerter.one($("#pam_NOTIN_PROJECT_NAME")[0], "项目名称最大长度不能超过100字节,当前输入字符的总字节数为" + currLength + "字节!\n【提示：汉字为两个字节，字母、数字、标点符号等其它为一个字节】");
    	return false;
    }
	...
	{
		//alert("请填写收费名称!");
		$.validate.alerter.one($("#pam_NOTIN_FEE_NAME")[0], "请填写收费名称!");
		return false;
	}
	...
	{
		//alert ("请填写收费金额(元)!");
		$.validate.alerter.one($("#pam_NOTIN_FEE_COST")[0], "请填写收费金额(元)!");
		return false;
	}
	else
	{
		...
		if(!flag)
		{
			...
			$.validate.alerter.one($("#pam_NOTIN_FEE_COST")[0], "收费金额(元)必须是数字或格式为0.00的数字!");
			return false;
		}
		...
		{
			//alert("收费金额(元)不能为负数!");
			$.validate.alerter.one($("#pam_NOTIN_FEE_COST")[0], "收费金额(元)不能为负数!");
			return false;
		}
	}
	...
	if(feeName == "4")
	{
		if(feeEndDate != "")
		{
			...
			$.validate.alerter.one($("#pam_NOTIN_FEE_END_DATE")[0], "收费名称为其他一次性费用,收费截止时间不需要填写!");
			return false;
		}
	}
	else 
	{
		...
		{
			//alert("请选择收费截止时间!");
			$.validate.alerter.one($("#pam_NOTIN_FEE_END_DATE")[0], "请选择收费截止时间!");
			return false;
		}
		...
		if(comFlag == false)
		{
			...
			$.validate.alerter.one($("#pam_NOTIN_FEE_END_DATE")[0], "收费截止时间不可早于当前时间!");
			return false;
		}
	}
	...
	var data = new Wade.DataMap(gdzyData);
	BroadbandTable.addRow($.parseJSON(data.toString()));
...
function deleteData(){
	...
	if(METHOD_NAME=='ChgUs')
	{
		...
		var rowData = BroadbandTable.getSelectedRowData();
...
function updateData() {
	...
	{
		$.validate.alerter.one($("#pam_NOTIN_PROJECT_NAME")[0], "请重新选择要修改的行数据后,再点击修改按钮!");
		return false;
	}
	...
	if(updateFlag)
	{
		var message = "项目名称:" + projectName + ",收费名称:" + feeNameV + "已经存在,请重新修改!";
		$.validate.alerter.one($("#pam_NOTIN_FEE_NAME")[0], message);
		return false;
	}
...
debugger;
	...
	var oldPamAttr = $("#pam_NOTIN_OLD_AttrGdzy").val();
	if (pamAttr == "" || pamAttr == "[]"){
		$.validate.alerter.one($("#pam_NOTIN_PROJECT_NAME")[0], "请增加一条费项信息!");
		return false;
	}
	var attrList = new Wade.DatasetList();
	var attrList = compareDataset(pamAttr,oldPamAttr);
	$("#pam_NOTIN_AttrGdzy").val(attrList.toString());
    ...
    for (var i=0;i<newValueSet.length;i++){
	    ...
	    for (var j=0;j<oldValueSet.length;j++) {
	        ...
	        if(oldValueColumn.get('pam_NOTIN_OPER_TAG') == newValueColumn.get('pam_NOTIN_OPER_TAG')){//可能不变，可能变更
	        	...
	        	if(oldValueColumn.get('pam_NOTIN_PROJECT_NAME') == newValueColumn.get('pam_NOTIN_PROJECT_NAME')&&
        			oldValueColumn.get('pam_NOTIN_FEE_NAME') == newValueColumn.get('pam_NOTIN_FEE_NAME')&&
        			oldValueColumn.get('pam_NOTIN_FEE_COST') == newValueColumn.get('pam_NOTIN_FEE_COST')&&
        			oldValueColumn.get('pam_NOTIN_FEE_END_DATE') == newValueColumn.get('pam_NOTIN_FEE_END_DATE')&&
        			oldValueColumn.get('pam_NOTIN_REMARK') == newValueColumn.get('pam_NOTIN_REMARK')){
...
function updateDataCommpare(notinOperTag,newProjectName,newFeeName)
{
	...
	var dataSet = BroadbandTable.getData(true);
	if (dataSet == ""){
        dataSet == "[]";
    }
    var isfound = false;
