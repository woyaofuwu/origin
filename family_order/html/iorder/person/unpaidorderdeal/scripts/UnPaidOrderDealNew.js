...
function qryUnpaidTrade() {
	...
	// 用户可返销订单查询
	ajaxSubmit('QueryCondPart', 'queryUnpaidTrade', null, 'TradeInfoPart',
			function(data) {
				$.endPageLoading();
...
function doPayTrade(obj) {
	...
	gloabObj = obj;
	ajaxSubmit('', 'createPaySubmit', param, '', function(paydata) {
		$(obj).blur();
		...
		var popupId = popupDialog('支付收银', "pay.order.PayMain",
				"queryOrderInfo", param + '&PARENT_EVENT_ID=_UN_PAY_FEE_ID',
				subsys_cfg.payment, 60, 30, true, '_UN_PAY_FEE_ID');
