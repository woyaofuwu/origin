...
{ 	 
	...
	if(acctTag==0){
		MessageBox.error("错误提示", "用户已经激活无需激活", $.auth.reflushPage, null, null, "");
	}
...
//								popupPage('beforecheck.BeforeCheck','init',param,'业务检查','600','300',null,null,null,null,false);
		...
		var usePsptTypeCode=$("#custInfo_USE_PSPT_TYPE_CODE").val();
		/**
		 * 以上界面户主使用身份证、户口本等证件办理入网业务时，营业员可通过手工调整生日信息，导致一经上传客户资料被拦截被判非实名，
		 * 请进行屏蔽，生日信息全部通过身份证号码进行截取，且不可更改，涉及证件类型为本地身份证、外地身份证、军人身份证、户口本，
		 * 其余个人证件不限制，需人工选择。
		 * */
		if(usePsptTypeCode==0 || usePsptTypeCode==1||usePsptTypeCode==2)
		{
			//$("#custInfo_BIRTHDAY").attr("disabled",true);
			//$("#popup").css("display", "");
			// window["myDialog"] = new Wade.Dialog("myDialog");
			MessageBox.alert("提示", "请使用人进行人像比对");
			$("#ReadCardButton4").css("display", "");
			$("#USE_SHOT_IMG").css("display", "");
		}else{
			MessageBox.alert("提示", "请对使用人证件号码做一致性比对");
			$("#ReadCardButton4").css("display", "none");
			$("#USE_SHOT_IMG").css("display", "none");
			$("#custInfo_USE_PSPT_ID2").attr('readonly', false);

		}
		$("#custInfo_REMARK").attr('readonly', false);
		//如果有手动输入权限
		if(inputPermision==1){
			$("#custInfo_USE_PSPT_ID2").attr('readonly', false);
		}
...
function checkPsptTypeCode(objId){
	...
	if("custInfo_RSRV_STR3"==objId){//责任人信息
		custName = $.trim($("#custInfo_RSRV_STR2").val());
		custNameObj = $("#custInfo_RSRV_STR2");
		this.checkCustName("custInfo_RSRV_STR2");
	}
	...
	if(custName!="" && psptTypeCode != "A"){
    	if(this.includeChineseCount(custName)<2){
    		$.TipBox.show(document.getElementById('custInfo_USE'), obj.attr('desc')+"不是护照，"+custNameObj.attr('desc')+"不能少于2个中文字符！", "red");
	...
	if('custInfo_AGENT_PSPT_TYPE_CODE'==objId){
		...
		var objValue = $("#custInfo_PSPT_TYPE_CODE").val();		
			...
			if(agentValue=="D" || agentValue=="E" || agentValue=="G" || agentValue=="L" || agentValue=="M")
			{
				...
				$.TipBox.show(document.getElementById('custInfo_AGENT_PSPT_TYPE_CODE'), "经办人只能选择个人证件，请重新选择！", "red");
//				alert("经办人只能选择个人证件，请重新选择！");
		...
		if(objValue=="D" || objValue=="E" || objValue=="G" || objValue=="L" || objValue=="M")
		{
			...
			if(useObjValue=="D" || useObjValue=="E" || useObjValue=="G" || useObjValue=="L" || useObjValue=="M")
			{
				...
				$.TipBox.show(document.getElementById('custInfo_USE_PSPT_TYPE_CODE'), "证件类型选择集团证件，使用人的证件类型只能选择个人证件，请重新选择！", "red");
...
function checkPsptId(objId){	
	...
	}else if("custInfo_USE_PSPT_ID"==objId||"custInfo_USE_PSPT_ID2"==objId){
...
function clickScanPspt3(){
	...
	if ($.os.phone) {
		$.MBOP.getMsgByEForm("custInfo_USE_PSPT_ID2"); //
	} else {
		getMsgByEForm("custInfo_USE_PSPT_ID2","custInfo_USE",null,null,null,"custInfo_USE_PSPT_ADDR",null,null);
...
//使用人信息
	var usepsptId = $("#custInfo_USE_PSPT_ID").val();
	if($.trim(usepsptId)==""){
		$.TipBox.show(document.getElementById('custInfo_USE_PSPT_ID'), "使用人证件号码不能为空！", "red");
		return false;
	}
	//责任人信息
	...
	var usePsptId2 = $("#custInfo_USE_PSPT_ID2").val();
	if (usePsptId2==""){
		MessageBox.error("请先扫描使用人身份证！");
		return false;
	}

	$("#custInfo_USE_PSPT_ID").val(usePsptId2);
		...
		$.ajax.submit(null,'isCmpPic','','',
			...
			if(flag=="0"){
					...
					//使用人信息
					var userpicid = $("#custInfo_USE_PIC_ID").val();
					var useTypeCode = $("#custInfo_USE_PSPT_TYPE_CODE").val();
					var compSuceese = $("#custInfo_PIC_COMP_SUCSSES").val();
					if((useTypeCode == "0" || useTypeCode == "1" ) && compSuceese == "0" ){
						MessageBox.error("告警提示","请进行使用人摄像!",null, null, null, null);
						return false;
					}
			...
			$.endPageLoading();
					...
					if(email && email.indexOf('*')>=0)//被模糊化了 则设置email为text类型
					{
...
function identification(picid,picstream){
	...
	fornt = $("#custInfo_USE_FRONTBASE64").val();
...
//			alert("年龄小于11或大于等于120岁的 ,不需要进行客户摄像");
	...
	if ($.os.phone) { // 手机移动端人像比对
		$.MBOP.shotImage(picid + "_494"); // 494：单位证件开户激活
		if($.MBOP.shotImageSuccess){
			$("#custInfo_PIC_COMP_SUCSSES").val("1");
		}
...
function verifyIdCard(fieldName){	
	...
	}else if(fieldName == "custInfo_USE_PSPT_ID"||fieldName=="custInfo_USE"||fieldName == "custInfo_USE_PSPT_ID2"){
		custNameObj = $("#custInfo_USE");
...
function verifyIdCardName(fieldName){
	...
	if(fieldName == "custInfo_PSPT_ID"||fieldName=="custInfo_CUST_NAME"){
		...
		custNameObj = $("#custInfo_RSRV_STR2");
		psptObj = $("#custInfo_RSRV_STR4");
		psptcodeObj = $("#custInfo_RSRV_STR3");
	}
	var psptId = $.trim(psptObj.val());
	var custName = $.trim(custNameObj.val());
	...
	$.ajax.submit(null, 'checkGlobalMorePsptId', param, '', function(data){
		$.endPageLoading();
			...
			if(data && data.get("CODE2")=="0"){
				$.MessageBox.confirm("确认提示",data.get("MSG"),function(re){
					if(re=="ok"){
					}else{
						$(clearPsptId).val("");
						return false;
