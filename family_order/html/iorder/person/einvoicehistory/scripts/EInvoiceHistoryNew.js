...
function queryEInvoiceTrade(data) {
	...
	ajaxSubmit("queryPart", "queryEInvoiceTrade", param, "RefreshTable",
			function(data) {
				$.endPageLoading();
...
function toDetail() {
	...
	if ($("#seleced_trade_id").val() == "") {
		MessageBox.alert("请选择一条电子发票数据");
		return false;
	}
	//	$.redirect.toPage('subsys_cfg.acctmanm','amnote.postbill.QueryElecInvoice','',"SERIAL_NUMBER="+serialNumber+"&REQUEST_ID="+printId)
	openNav('电子发票统一查询', 'elecnote.QueryElecInvoice', 'myInitialize',
			'&REQUEST_ID=' + printId + '&SERIAL_NUMBER=' + serialNumber,
			subsys_cfg.acctmanm);
}
...
function toCredit() {
	...
	if ($("#seleced_print_flag").val() == "冲红发票") {
		MessageBox.alert("请选择正常发票进行冲红");
		return false;
	}
	MessageBox.confirm('确认提示', '确定要冲红该电子发票吗？', function(btn) {
		if (btn == "ok") {
			var param = "&PRINT_ID=" + printId + "&SERIAL_NUMBER=" + serialNumber + "&TRADE_ID=" + tradeId;
			$.beginPageLoading("正在冲红....");
			ajaxSubmit("", "toCreditNote", param, "", function(data) {
				$.endPageLoading();
