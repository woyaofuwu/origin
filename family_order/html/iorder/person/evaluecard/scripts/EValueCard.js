...
function refreshPartAtferAuth2(data)
{
	$.ajax.submit('AuthPart', 'loadChildInfo', "&USER_INFO="+data.get("USER_INFO").toString()+"&CUST_INFO="+data.get("CUST_INFO").toString(), 'freshPart,QueryCondPart', function(data){
		 ...
		 disabledArea('SelOperTypePart', false);
...
function queryBatchReqInfo(){
	...
	if(dateDiff(startData, endData)>day){
		MessageBox.alert("提示","开始时间和结束时间跨度不能超过"+day+"天");
		return false;
	}
...
function createSellReq() {	
	...
	if(channelType=="")
	{
		$.TipBox.show(document.getElementById('CHANNEL_TYPE'), "请选择受理渠道！", "red");
//		alert("请选择受理渠道！");
		return false;
	}
	...
	if(publicKeyID=="")
	{
		$.TipBox.show(document.getElementById('PUBLIC_KEY_ID'), "请输入公钥ID！", "red");
//		alert("请输入公钥ID！");
		return false;
	}
	...
	if(cardType=="")
	{
		$.TipBox.show(document.getElementById('CARD_TYPE'), "请选择卡类型！", "red");
//		alert("请选择卡类型！");
		return false;
	}
	...
	if(cardMoney=="")
	{
		$.TipBox.show(document.getElementById('CARD_MONEY'), "请输入卡面值！", "red");
//		alert("请输入卡面值！");
		return false;
	}
	if(!$.isNumeric(cardMoney)){
		$.TipBox.show(document.getElementById('CARD_MONEY'), "卡面值输入不合法！", "red");
//		alert("卡面值输入不合法！");
		return false;
    }
	
	if(!$.isNumeric(buyCount)){
		$.TipBox.show(document.getElementById('BUY_COUNT'), "购卡数量输入不合法！", "red");
//		alert("购卡数量输入不合法！");
		return false;
	}
	
	if(buyCount<=10){
		$.TipBox.show(document.getElementById('BUY_COUNT'), "购卡数量必须大于10张！", "red");
//		alert("购卡数量必须大于10张！");		
		return false;
	}
	if(buyCount>1000000){
		$.TipBox.show(document.getElementById('BUY_COUNT'), "购卡数量不能大于一百万！", "red");
//		alert("购卡数量不能大于一百万！");		
		return false;
	}	
...
//		custEdit["CARD_TYPE"] = $("#CARD_TYPE")[0].options($("#CARD_TYPE")[0].selectedIndex).value;//卡类型   值
		custEdit["CHANNEL_TYPE_TEXT"] = $("#CHANNEL_TYPE").text();
		custEdit["CARD_TYPE_TEXT"] = $("#CARD_TYPE").text();
		custEdit["CHANNEL_TYPE"] = $("#CHANNEL_TYPE").val();//受理渠道    值 
		custEdit["CARD_TYPE"] = $("#CARD_TYPE").val();//卡类型   值
...
function deleteSellReq(){	
	var rowData = SellReqTable.getRowData(SellReqTable.selected);
	if(rowData == 0 || rowData == null || rowData == undefined)
	{
		MessageBox.alert("提示", "请您选择记录后再进行删除操作！");
		return false;
	}
//	$.table.get("SellReqTable").deleteRow();	
	SellReqTable.deleteRow(SellReqTable.selected);
cleanSellReq();
...
//	var sellReqTable=$.table.get("SellReqTable").getTableData(null,true);
	var sellReqTable=SellReqTable.getData();
...
function editRsaInfo(){
	var rowData = infosTable.getRowData(infosTable.selected);//$.table.get("infosTable").getRowData();
	if(rowData.length == 0)
	{
...
function deletRsaInfo(obj){
	var rowData = infosTable.getRowData(infosTable.selected);
	if(rowData.length == 0)
	{
...
function changeSelect(){
	...
	if(cardtype == '00'){
		$("#CARD_BUSIPROP").attr("disabled","true");
	}else{
		$("#CARD_BUSIPROP").attr("disabled",null);
	}
}