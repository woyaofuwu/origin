...
//	   alert("请先加载用户信息");
	...
	$.ajax.submit('sellTopSetBoxInfoPart', 'checkTerminal', "&RES_ID="+resID + "&SERIAL_NUMBER=" + serialNumber + "&OLD_RES_ID=" + old_res_id, 'terminalPart',
			function(data) {
				$.endPageLoading();
				//清理费用
...
function submitBeforeCheck()
{
	if($("#RES_ID").val()!=$("#RES_NO").val()){
		MessageBox.alert("提示", "终端串不一致，请重新校验！");
		return false;
	}
	...
	if(!artitificialService||artitificialService==""){
		$.TipBox.show(document.getElementById('Artificial_services'), "请选择“需要上门安装”选项！", "red");
//		alert("请选择“需要上门安装”选项！");
		return false;
	}
