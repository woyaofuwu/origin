$(function () {
    window["baseinfo_radio"] = new Wade.Segment("baseinfo_radio", {});
    $("#baseinfo_radio").val("a");
    $("#QueryCondPart").addClass("e_hide-x");
});
...
function checkRadio() {
    ...
    if ($("#baseinfo_radio").val() == "a") {
        $("#QueryCondPart").removeClass("e_hide-x");
        $("#xCodingStr").val("");
        CHG_TAG = true;
    } else {
        $("#QueryCondPart").addClass("e_hide-x");
...
function checkBeforeSubmit() {
    ...
    if ($("#TRADE_TYPE_CODE").val() == "416") {
        ...
        for (var i = 0; i < $(objTable).find("tbody > tr").length; i++) {
			...
			totalPrice =Number(totalPrice) + Number($.trim($(objTable).find("tbody > tr").eq(i).find("td").eq(5).text()));
		 ...
		 * @date 20170717
		 */		
		var isTotalPriceflag="0";
		var errerMsg="";
			var param = "";
			param += "&TOTAL_PRICE=" + totalPrice;
			$.ajax.submit(null,'isTotalPrice',param,'',
					function(data){ 
				$.endPageLoading();
				isTotalPriceflag=data.get("tag");
				errerMsg=data.get("msg");				
			},function(error_code,error_info){
		...
		if(isTotalPriceflag == "1"){
			//扫描证件信息
			var psptId=$("#custInfo_PSPT_ID").val();
			if(psptId == ""){
				MessageBox.error("告警提示","请进行证件扫描",null, null, null, null);
				return false;
			}
			//需要进行人像采集
			var picid = $("#custInfo_PIC_ID").val();
			if(picid == ""){
				MessageBox.error("告警提示",errerMsg,null, null, null, null);
				return false;
			}
		}else if(isTotalPriceflag == "2"){
			//纳入人像采集,有价卡约定额度没有配置
			MessageBox.error("告警提示","纳入人像采集,有价卡约定额度没有配置",null, null, null, null);
			return false;
		}		
    }
...
function identification(picid,picstream){
	...
	var objTable = document.getElementById('table2');
	for(var i = 0; i < $(objTable).find("tbody > tr").length; i++) {
		//totalPrice =Number(totalPrice) + Number(objTable.rows[i].cells[5].innerHTML);
		totalPrice = Number(totalPrice) + Number($.trim($(objTable).find("tbody > tr").eq(i).find("td").eq(5).text()));
	...
	if(isTotalPriceflag == "0"||isTotalPriceflag == "2"){
		//不需要进行摄像
		MessageBox.error("告警提示","额度达不到约定数,不需要进行客户摄像",null, null, null, null);
		return false;
	}
