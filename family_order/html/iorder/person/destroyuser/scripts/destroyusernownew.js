...
function refreshPartAtferAuth(data) {
	...
	$.ajax.submit('AuthPart', 'setPageCustInfo', param, 'BusiInfoPart,HiddenPart,DestroyInfoPart', function(data) {
		...
		$(".REMOVE_REASON_LIST_ITEM").removeClass("checked");
		$(".REMOVE_REASON_LIST_ITEM").eq(0).addClass("checked");
		$("#REMOVE_REASON_CODE").val($(".REMOVE_REASON_LIST_ITEM").eq(0).attr("valueCode"));
...
function submitBeforeCheck() {
	...
	if ($("#REMOVE_REASON_CODE").val() == "") {
		MessageBox.alert("请选择销户原因");
		return false;
	}
    ...
    var param = '&USER_ID=' + userId + '&SERIAL_NUMBER=' + serialNumber + '&EPARCHY_CODE=' + eparchyCode;
	$.ajax.submit('DestroyInfoPart', 'checkScore', param, '', function(data) {
...
// 点击销户原因选择
function getRemoveReasonCode(obj) {
	$(".REMOVE_REASON_LIST_ITEM").removeClass("checked");
	$(obj).addClass("checked");
	$("#REMOVE_REASON_CODE").val($(obj).attr("valueCode"));
}