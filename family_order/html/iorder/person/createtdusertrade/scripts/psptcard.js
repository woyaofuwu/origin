...
function checkDriver() {
	try {
		...
		MessageBox.alert("提示",'系统检测到您没有安装写卡客户端，请到系统首页进行下载安装！');
		return false;
	}
}
...
{ 
	...
	if(document.getElementById("custinfo_ReadCardFlag").value==0)
	{
		...
		if( obj == null )
		{
			MessageBox.alert("提示","加载身份证阅读控件失败!");
		}
		...
		if( obj != null )
		{
			...
			{
				MessageBox.alert("提示", "读取身份证信息失败! 内容: " + ReadTxt );
				return;
			}
			...
			if( arrNum < 8 )
			{
				MessageBox.alert("提示", "读取身份证信息格式不正确! 字段数量:" + arrNum + ", 内容: " + ReadTxt );
				return;
			}
