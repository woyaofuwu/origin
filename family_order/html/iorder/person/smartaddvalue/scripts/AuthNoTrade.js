        ...
        authNoTrade: {
                ...
                MessageBox.confirm("温馨提示", "系统将提交受理数据信息，您确认要继续吗？", function (re) {
                    if (re == "ok") {
                        ...
                        var param = "&TRADE_ID=" + tradeInfo.get("TRADE_ID") + "&TRADE_TYPE_CODE=" + tradeInfo.get("TRADE_TYPE_CODE")
                            //&OPER_FEE=0&FOREGIFT=0&ADVANCE_PAY=0老界面这三个值传的就是空
                            + "&ACCEPT_MONTH=" + tradeInfo.get("ACCEPT_MONTH") + "&OPER_FEE=0&FOREGIFT=0&ADVANCE_PAY=0" +
            ...
            afterSubmit: function (data) {
                ...
                MessageBox.success("成功提示", "业务取消成功!", function (btn) {
                    $.redirect.toPage("iorder", "smartaddvalue.CancelSmartNetworkNew", '', '');
                });
            }
        }
    });
