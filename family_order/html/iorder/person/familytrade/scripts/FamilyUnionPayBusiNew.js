...
function refreshPartAtferAuth(data) {
	...
	ajaxSubmit('', 'loadChildInfo', params, 'EditPart,EditPartPayitemCode,assignPayitemCodePart,memberSnPart,curMemberCountPart,HiddenPart', function() {
				...
				$("#EditPartPayitemCode").attr("disabled", false);
				$.endPageLoading();
...
function addOrModifyMember(){
	...
	var memberSn = $("#MEMBER_SN").val(); // 新增成员号码
	
	if (memberSn.length <= 0) {
		$.TipBox.show(document.getElementById("MEMBER_SN"), "请输入新增副卡号码（多个号码以空格分隔）", "red");
		return false;
	}

	memberSn = memberSn.trim();

	if (memberSn.length > 1) {
		memberSn = memberSn.split(" ");
	}
	...
	// 删除新增副卡数组中的空字符
	for (var i = 0; i < memberSn.length; i++) {
		var sn = memberSn[i];
		if (sn == "") {
			memberSn.splice(i,1);
			i--;
	...
	var fmyCurCount=$('#FMY_CUR_COUNT').html(); // 当前绑定成员数
	var mebLimValue = $('#MEB_LIM').val(); // 成员数量上限
	
	if (memberSn.length + parseFloat(fmyCurCount) > parseFloat(mebLimValue)) {
		$.TipBox.show(document.getElementById("MEMBER_SN"), '您的统付成员号码数量已经达到' +mebLimValue+ '个的上限，不能再增加成员号码！', "red");
		return false;
	}
	...
	for (var i = 0; i < memberSn.length; i++) {
		...
		var mebSn = memberSn[i];
	    // 校验批量输入的号码，不能重复
	    for ( var k = i + 1; k < memberSn.length; k++) {
	    	var tempMebSn = memberSn[k];
	    	if (mebSn == tempMebSn) {
	    		$.TipBox.show(document.getElementById("MEMBER_SN"), "对不起，您输入的手机号码[" + mebSn + "]重复，请勿重复输入！", "red");
		        return false;
	    	}
	    }
		// 不能与主号一致
		if (mebSn == mainSn) {
			$.TipBox.show(document.getElementById("MEMBER_SN"), "对不起，成员号码不能和主卡号码一样，请重新输入！", "red");
			return false;
		}
	
		for ( var j = 0, size = memberSnDataset.length; j < size; j++) {
			var tmp = memberSnDataset.get(j);
			var sn = tmp.get('SERIAL_NUMBER_B');
			var orgEndDate = tmp.get('ORG_END_DATE');
			var modifyTag = tmp.get('MODIFY_TAG');
			
			if(orgEndDate != lastTimeThisMonthValue && sn == mebSn && (modifyTag == undefined || modifyTag == "" || modifyTag =="0")) {
				$.TipBox.show(document.getElementById("MEMBER_SN"), "号码" + mebSn + "已经存在,请重新输入！", "red");	
				return false;
			}
			
			if(orgEndDate != lastTimeThisMonthValue && sn == mebSn && modifyTag == "1") {
				$.TipBox.show(document.getElementById("MEMBER_SN"), "号码" + mebSn + "为本次删除的号码不能再新增！", "red");
				return false;
			}
		}
	}
	...
	memberSn = modifyMember(memberSn); // 先对需要修改的成员进行操作，过滤出新增的号码

	if (memberSn.length > 0) { // 对新增成员进行操作
		addMember(memberSn);
	}
}
...
function payitemCodeSelect() {
	var payitemCode = $("#PAYITEM_CODE_SEL").val();
	$("#PAYITEM_CODE").val(payitemCode);
}