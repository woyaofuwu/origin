    ...
    $.beginPageLoading("宽带资料查询。。。");
	 $.ajax.submit('AuthPart,hiddenPart', 'loadChildInfo', param, 'wideInfoPart', function(data){
	     ...
	     if('' !== wideSerial && null != wideSerial && undefined !== wideSerial) {
	         MessageBox.alert("告警提示", "客户已办理魔百和业务，魔百和需过户!");
	     }
...
function submitBeforeCheck()
{
	...
	MessageBox.confirm("告警提示","提交成功后，您的宽带将过户。确认继续吗?",function(re){
		if(re=="ok"){
...
function initModifyInfo()	
{
  ...
  if($("#REAL_REG").val()=="1")
  {
	MessageBox.alert("提示","用户已经办理了实名制预受理业务，自动默认其资料");
...
{   
    ...
    {     
       MessageBox.alert("提示","办理实名制用户，证件类型不能为其它！");
...
function checkPsptTypeCode(objId){
	...
	if(if6070=="TRUE"){
		if(oldPsptTypeCode=="C" && psptTypeCode!="C" && psptTypeCode!="0" && psptTypeCode!="1"){
			MessageBox.alert("提示","吉祥号码军官证只允许变更为身份证");
		...
		if(oldPsptTypeCode=="2"){
			if(psptTypeCode=="0" || psptTypeCode=="1"||psptTypeCode=="2"){
				if(oldPsptId!=$("#custInfo_PSPT_ID").val()){
					MessageBox.alert("提示","吉祥号码户口本变更为身份证后证件号码不能改变。");
					$("#custInfo_PSPT_ID").val(oldPsptId);
					return;
				} 
			}else{
				MessageBox.alert("提示","吉祥号码户口本只允许变更为身份证");
				$("#custInfo_PSPT_TYPE_CODE").val(oldPsptTypeCode);
				return;
			}
		}
		...
		if(oldPsptTypeCode=="0" || oldPsptTypeCode=="1"){
			if(psptTypeCode=="2" ){
				if(oldPsptId!=$("#custInfo_PSPT_ID").val()){
					MessageBox.alert("提示","吉祥号码身份证变更为户口本后证件号码不能改变。");
					$("#custInfo_PSPT_ID").val(oldPsptId);
					return;
				} 
			}else{
				MessageBox.alert("提示","吉祥号码身份证只允许变更为户口本。");
				$("#custInfo_PSPT_TYPE_CODE").val(oldPsptTypeCode);
				return;
			}
		}
		...
		if(oldPsptTypeCode!="C" &&oldPsptTypeCode!="0" && oldPsptTypeCode!="1" && oldPsptTypeCode!="2" ){
			MessageBox.alert("提示","吉祥号码变更证件限制：身份证和户口本允许更换类型不允许改号码；军官证可变更为身份证；其余的不允许办理。");
			$("#custInfo_PSPT_TYPE_CODE").val(oldPsptTypeCode);
			return;
		}
	}
	...
	if(custName!="" && psptTypeCode != "A"){
    	if(this.includeChineseCount(custName)<2){
    		MessageBox.alert("提示",obj.attr("desc")+"不是护照，"+custNameObj.attr("desc")+"不能少于2个中文字符！");
    		custNameObj.val("");
			return ;
    	}
	}
	...
	if('custInfo_AGENT_PSPT_TYPE_CODE'==objId){
		...
		var objValue = $("#custInfo_PSPT_TYPE_CODE").val();		
			...
			if(agentValue=="D" || agentValue=="E" || agentValue=="G" || agentValue=="L" || agentValue=="M")
			{
				...
				MessageBox.alert("提示","经办人只能选择个人证件，请重新选择！");
		...
		if(objValue=="D" || objValue=="E" || objValue=="G" || objValue=="L" || objValue=="M")
		{
			...
			if(useObjValue=="D" || useObjValue=="E" || useObjValue=="G" || useObjValue=="L" || useObjValue=="M")
			{
				...
				MessageBox.alert("提示","证件类型选择集团证件，使用人的证件类型只能选择个人证件，请重新选择！");
...
function checkUsePsptIdForReal(){
    if($("#custInfo_USE_PSPT_TYPE_CODE").val()=="Z") {
        MessageBox.alert("提示","使用人证件类型不能为其他，请重新选择！");
        return false;
    }
    var psptId = $("#custInfo_USE_PSPT_ID").val();
    if($.toollib.isRepeatCode(psptId))
    {
        MessageBox.alert("提示","使用人证件号码过于简单，请重新输入！");
        return false;
    }
    if($("#custInfo_USE").val().indexOf("海南通")>-1)
    {
        MessageBox.alert("提示","使用人姓名不能为【海南通】，请重新输入！");
        $("#custInfo_USE").val("");
        return false;
    }
    if(!$.validate.verifyField("custInfo_USE_PSPT_ID")){
...
function checkPsptId(objId){	
	...
	if(if6070=="TRUE"){//对于满足吉祥号码这种条件的，只有原来是军官证变更成身份证这种情况不拦截
		if(oldPsptId != psptId){
			if(oldPsptTypeCode=="C" && (psptTypeCode=="C" || psptTypeCode=="0" || psptTypeCode=="1")){
				...
				MessageBox.alert("提示","吉祥号码变更证件限制：修改号码只允许办理军官证变更身份证，其余的均不允许变更号码。");
				$("#custInfo_PSPT_ID").val(oldPsptId);
				return false;
			}
		}
	...
	if($.toollib.isRepeatCode(psptId)){
		MessageBox.alert("提示",desc+"不能全为同一个数字，请重新输入！");
		obj.val("");
		obj.focus();
	...
	if($.toollib.isSerialCode(psptId)){
		MessageBox.alert("提示","连续数字不能作为"+desc+"，请重新输入！");
		obj.val("");
		obj.focus();
	...
	if(psptId.length>=4 && serialNumber.length>=psptId.length && ( serialNumber.indexOf(psptId) ==0 
		...
		MessageBox.alert("提示","电话号码的前面四位（或以上）与电话号码后面四位（或以上）不能作为"+desc+"！");
		obj.val("");
		obj.focus();
	...
	if(psptTypeCode=="H"){
		if(psptId.length!=9 && psptId.length!=11){
			MessageBox.alert("提示","港澳居民回乡证校验："+desc+"必须为9位或11位！");
			obj.val("");
			obj.focus();
		...
		if( !(psptId.charAt(0)=="H" || psptId.charAt(0)=="M") || !$.toollib.isNumber(psptId.substr(1)) ){
			MessageBox.alert("提示","港澳居民回乡证校验："+desc+"首位必须为英文字母\"H\"或\"M\",其余位均为阿拉伯数字！");
			obj.val("");
			obj.focus();
		...
		if(psptIdtemp!=psptId){
				MessageBox.alert("提示","港澳居民来往内地通行证校验：证件号码中间不能有空格。");
				obj.val("");
				obj.focus();
		...
		if(psptId.length!=9 && psptId.length!=11){
			MessageBox.alert("提示","港澳居民来往内地通行证校验："+desc+"必须为9位或11位。");
			obj.val("");
			obj.focus();
		...
		if( !(psptId.charAt(0)=="H" || psptId.charAt(0)=="M") || !$.toollib.isNumber(psptId.substr(1)) ){
			MessageBox.alert("提示","港澳居民来往内地通行证校验："+desc+"首位必须为英文字母\"H\"或\"M\",其余位均为阿拉伯数字。");
			obj.val("");
			obj.focus();
	   ...
	   //台湾居民回乡:证件号码为11位时，前10位为阿拉伯数字，最后一位为校验码，括号内为英文字母或阿拉伯数字；证件号码为8位时，均为阿拉伯数字。
		if(psptId.length!=8 && psptId.length!=11){
			MessageBox.alert("提示","台湾居民回乡校验："+desc+"必须为8位或11位！");
			obj.val("");
			obj.focus();
		...
		if(psptId.length==8){
			if(!$.toollib.isNumber(psptId)){
				MessageBox.alert("提示","台湾居民回乡校验："+desc+"为8位时，必须均为阿拉伯数字！");
				obj.val("");
				obj.focus();
		...
		if(psptId.length==11){
			if(!$.toollib.isNumber(psptId.substring(0,10))){
				MessageBox.alert("提示","台湾居民回乡校验：："+desc+"为11位时，前10位必须均为阿拉伯数字。");
				obj.val("");
				obj.focus();
		...
		var psptIdtemp = psptId.replace(/\s/g,''); 		
		if(psptIdtemp!=psptId){
			MessageBox.alert("提示","台湾居民来往大陆通行证校验：证件号码中间不能有空格。");
			obj.val("");
			obj.focus();
		...
		if(psptId.substr(0, 2)!="TW"&&psptId.substr(0, 4)!="LXZH"){	 				
			...
			if(psptId.length==11 ||psptId.length==12){
				if(!$.toollib.isNumber(psptId.substring(0,10))){
					MessageBox.alert("提示","台湾居民来往大陆通行证校验："+desc+"为11或12位时，前10位必须均为阿拉伯数字。");
					obj.val("");
					obj.focus();
				...
				if(!$.toollib.isNumber(psptId)){
					MessageBox.alert("提示","台湾居民来往大陆通行证校验："+desc+"为8位时，必须均为阿拉伯数字。");
					obj.val("");
					obj.focus();
				...
				if(!$.toollib.isNumber(psptId)){
					MessageBox.alert("提示","台湾居民来往大陆通行证校验："+desc+"为7位时，必须均为阿拉伯数字。");
					obj.val("");
					obj.focus();
				...
				MessageBox.alert("提示","台湾居民来往大陆通行证校验："+desc+"格式错误");
				obj.val("");
				obj.focus();
			...
			if(re1.test(psptIdsub)||!re.test(psptIdsub)||!pattern1.test(psptIdsub)||!pattern2.test(psptIdsub)||!pattern3.test(psptIdsub)||!pattern4.test(psptIdsub)){
				MessageBox.alert("提示","台湾居民来往大陆通行证校验："+desc+"前2位“TW”或 “LXZH”字符时，后面是阿拉伯数字、英文大写字母与半角“()”的组合");
				obj.val("");
				obj.focus();
		...
		var psptIdtemp = psptId.replace(/\s/g,''); 				 				
		if(psptIdtemp!=psptId){
				MessageBox.alert("提示","证件号码中间不能有空格。");
				obj.val("");
				obj.focus();
		...
		if(psptId.length < 6){
			...
			MessageBox.alert("提示",tmpName+desc+"须大于等于6位字符！");
			obj.val("");
			obj.focus();
		...
		if(psptId.length != 13 && psptId.length != 15 && psptId.length != 18 && psptId.length != 20 && psptId.length != 22 && psptId.length != 24){
			MessageBox.alert("提示","营业执照类型校验："+desc+"长度需满足13位、15位、18位、20位、22位或24位！当前："+psptId.length+"位。");
			obj.val("");
			obj.focus();
		...
		if(psptId.length != 10 && psptId.length != 18){
			MessageBox.alert("提示","组织机构代码证类型校验："+desc+"长度需满足10位或18位。");
			obj.val("");
			obj.focus();
		...
		if(psptId.length == 10 &&psptId.charAt(8) != "-"){
			MessageBox.alert("提示","组织机构代码证类型校验："+desc+"规则为\"XXXXXXXX-X\"，倒数第2位是\"-\"。");
			obj.val("");
			obj.focus();
		...
		if(psptId.length != 12 && psptId.length != 18){
			MessageBox.alert("提示","事业单位法人登记证书类型校验："+desc+"长度需满足12位或者18位！");
			obj.val("");
			obj.focus();
		...
		if("custInfo_AGENT_PSPT_ID"==objId||"custInfo_USE_PSPT_ID"==objId){
		   if(trim(psptId)!=""&&!this.checkAge(psptId)){
		     	MessageBox.alert("提示",'年龄范围必须在10-120岁之间'); 
				obj.val("");
				obj.focus();
		...
		if (area[parseInt(psptId.substr(0, 2))] == null) {
			MessageBox.alert("提示","\u8eab\u4efd\u8bc1\u53f7\u7801\u4e0d\u6b63\u786e\u0028\u5730\u533a\u975e\u6cd5\u0029\uff01");
			obj.val("");
			obj.focus();
		...
		if (!(/(^\d{15}$)|(^\d{17}([0-9]|X)$)/.test(psptId))) {
			MessageBox.alert("提示","\u8f93\u5165\u7684\u8eab\u4efd\u8bc1\u53f7\u957f\u5ea6\u4e0d\u5bf9\uff0c\u6216\u8005\u53f7\u7801\u4e0d\u7b26\u5408\u89c4\u5b9a\uff01\n15\u4f4d\u53f7\u7801\u5e94\u5168\u4e3a\u6570\u5b57\uff0c18\u4f4d\u53f7\u7801\u672b\u4f4d\u53ef\u4ee5\u4e3a\u6570\u5b57\u6216X\u3002 ");
			obj.val("");
			obj.focus();
		...
		if (!bGoodDay) {
			MessageBox.alert("提示","\u8f93\u5165\u7684\u8eab\u4efd\u8bc1\u53f7\u91cc\u51fa\u751f\u65e5\u671f\u4e0d\u5bf9\uff01");
			obj.val("");
			obj.focus();
		...
		if(bit11!="0"&&bit11!="1")
		{
			MessageBox.alert("提示",'18位身份证号码11位必须为0或者1');
			obj.val("");
			obj.focus();
		...
		if(parseInt(bit13)>3)
		{
			MessageBox.alert("提示",'18位身份证号码13位必须小于等于3'); 
			obj.val("");
			obj.focus();
...
function getBirthdayByPsptId(psptId) {
	...
	if((psptId.length!=15) &&(psptId.length!=18)) {
	    MessageBox.alert("提示","告警提示", "输入的证件号码位数错误！");
		return tmpStr;
	}
	if(psptId.length==15) {
...
function checkCustName(objId){
    ...
    if(mainPsptTypeCode == "E"||mainPsptTypeCode == "G"||mainPsptTypeCode == "D" ||mainPsptTypeCode == "M" ||mainPsptTypeCode == "L"){
    	...
    	if(custnametemp!=""&&agentnametemp!=""&&custnametemp==agentnametemp){
			MessageBox.alert("提示","单位名称和经办人名称不能相同！");
			obj.val("");
			obj.focus();
			return;
    	}
    	if(custnametemp!=""&&usenametemp!=""&&custnametemp==usenametemp){
			MessageBox.alert("提示","单位名称和使用人名称不能相同！");
			obj.val("");
			obj.focus();
			return;
    	}
	   ...
	   for(i=0;i<specialStr.length;i++){
		   if (custName.indexOf(specialStr.charAt(i)) > -1){
			   MessageBox.alert("提示",obj.attr("desc")+"包含特殊字符，请检查！");
			   obj.val("");
			   obj.focus();
			   return false;
		   }
	...
	if(re2.test(custName)){
		MessageBox.alert("提示",desc+"包含非法关键字！");
		obj.val("");
		obj.focus();
		return false;
	}
	...
	if(psptTypeCode=="0" || psptTypeCode=="1" || psptTypeCode=="2" || psptTypeCode=="3" || psptTypeCode=="C"|| psptTypeCode=="H"|| psptTypeCode=="I"|| psptTypeCode=="N"|| psptTypeCode=="O"){
		var re=/^[•··.．·\d\u4e00-\u9fa5]+$/;
		if(!re.test(custName)){
			MessageBox.alert("提示",desc+"包含特殊字符，请检查！");
			obj.val("");
			obj.focus();
			return;
		}
    ...
    if(psptTypeCode != "A" && psptTypeCode != "D"){
	     ...
	     {//	E:营业执照,G:事业单位法人证书,D:单位证明,M:组织机构代码证,L:社会团体法人登记证书	
    		var pattern =/[a-zA-Z0-9]/;
	         if(pattern.test(custName)){
	        	MessageBox.alert("提示",psptTypeDesc+"不是护照, "+desc+"不能包含数字和字母！");
	        	obj.val("");
	        	obj.focus();
	        	return false;
	         }		
		...
		if(this.includeChineseCount(custName)<2){
			MessageBox.alert("提示",psptTypeDesc+"不是护照,"+desc+"不能少于2个中文字符！");
			obj.val("");
			obj.focus();
			return;
		}
		...
		if(custName.length<3 || $.toollib.isNumber(custName)){
			MessageBox.alert("提示",psptTypeDesc+"是护照,"+desc+"须大于三个字符，且不能全为阿拉伯数字！");
			obj.val("");
			obj.focus();
			return false;
		}
    ...
    if(psptTypeCode == "E"||psptTypeCode == "G"||psptTypeCode == "M"){//	营业执照、组织机构代码证、事业单位法人登记证书
    	if(this.includeChineseCount(custName)<4){
			MessageBox.alert("提示",psptTypeDesc+"不是护照,"+desc+"不能少于4个中文字符！");
			obj.val("");
			obj.focus();
			return;
		}
	...
	if(psptTypeCode=="0" || psptTypeCode=="1" || psptTypeCode=="2" || psptTypeCode=="3" || psptTypeCode=="C"|| psptTypeCode=="H"|| psptTypeCode=="I"|| psptTypeCode=="N"|| psptTypeCode=="O"){
		...
		if(custName.length<2){
			MessageBox.alert("提示",psptTypeDesc+"是护照,"+desc+"须两个字符及以上");
			obj.val("");
			obj.focus();
			return;
		}
    ...
    if(custName.indexOf("校园")>-1 || custName.indexOf("海南通")>-1 || custName.indexOf("神州行")>-1
        ...
        MessageBox.alert("提示",desc+"不能包含【校园、海南通、神州行、动感地带、套餐】，请重新输入！");
        obj.val("");
        obj.focus();
        return false;
    } 
    if("custInfo_CUST_NAME" == objId){
		...
		if ($("#STAFF_SPECIAL_RIGTH").val() != "true"
			...
			MessageBox.alert("提示","该用户还处在营销活动期限内，不能够修改"+desc+"！");
...
function checkContactName(objId){
	...
	if(re4.test(contactName)){
		MessageBox.alert("提示",desc+"不能为纯数字！");
		obj.val("");
		obj.focus();
		return false;
	}
...
function checkBirthday()
{
    ...
    {    
          ...
          {
             MessageBox.confirm("告警提示","身份证号码与生日不吻合，是否将生日改为与身份证号码一致？",function(re){
        		if(re=="ok"){
...
function checkPhone(objId){
	...
	if(!$.toollib.isNumber(phone) || phone.length<6){
		MessageBox.alert("提示",desc+"必须大于等于6阿拉伯数字！");
		phoneObj.val("");
		phoneObj.focus();
		return false;
	}
...
//	var psptAddr = this.trimAll(custAddrObj.val());	
	var zjNum = this.trimAll(custAddrObj.val()).replace(/[^\x00-\xff]/g, "**").length ; //字节数
	if(custAddrObj.val()=="" || zjNum*1<12){
		 MessageBox.alert("提示",custAddrObj.attr("desc")+'栏录入文字需不少于6个汉字或12个字节！');
		 custAddrObj.val('');
		 custAddrObj.focus();
		 return false;
	}
//	if(psptAddr.indexOf('省')== -1 && psptAddr.indexOf('市')== -1 && psptAddr.indexOf('自治区')== -1){
//		MessageBox.alert("提示","身份证地址（含身份证、户口本、军人身份证）信息必须包含“省”、“市”、“自治区”之一！");
//		custAddrObj.val('');
//		custAddrObj.focus();
//		return false;
//	}
	 if(!isNaN(custAddrObj.val())&& custAddrObj.val() != ''){
		 MessageBox.alert("提示",custAddrObj.attr("desc")+'栏不能全部为数字');
		 custAddrObj.val('');
		 custAddrObj.focus();
		 return false;
	 }
...
function submitCheck(obj) {     
	...
	if (oldIsRealName!=1) { //修改之前的客户不是实名制，代表本次是第一次做实名制办理
		MessageBox.confirm("告警提示","您正在办理实名制，一旦提交资料将不能修改。请确认输入的资料无误！",function(re){
			if(re=="ok"){
