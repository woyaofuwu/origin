(function($){ 
	$.extend({authNoTrade:{
		...
		submitCancelTrade:function()
		{
	    	...
	    	MessageBox.confirm("温馨提示","系统将提交受理数据信息，您确认要继续吗？",function(re){
				if(re=="ok"){
					var tradeInfo = tradeInfos.get(0);
					
					if (null != tradeInfo.get("RSRV_STR10") && '' != tradeInfo.get("RSRV_STR10") && tradeInfo.get("RSRV_STR10").length > 2)
			    	{
			    		MessageBox.confirm("温馨提示","该用户宽带开户同时办理了和目业务，请确认是否已经退还和目终端？",function(re){
			    			if(re=="ok"){
	    ...
	    afterSubmit:function(data){
	    	...
	    	MessageBox.success("成功提示","业务取消成功!",function(btn){
	    		$.redirect.toPage("iorder","broadband.widenet.CancelMergeWideUserCreateTradeNew","onInitTrade",'');
	    	});
	    }
	}});
	//页面初始化
