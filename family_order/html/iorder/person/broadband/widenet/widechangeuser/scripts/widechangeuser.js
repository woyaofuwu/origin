...
function qureyMphone(){  
	...
	{
	  MessageBox.alert("提示","请输入需要过户的原号码点击【查询】!");
	  $("#AUTH_SERIAL_NUMBER").focus();
	  return false;
	}
	if(inputSn==null||inputSn=="")
	{
	  MessageBox.alert("提示","请输入变更的服务号码!");
	  return false;
	}
	if(inputSn==oldSerialNum){
		MessageBox.alert("提示","不允许过户给自己！");
		$("#IS_CHECK").val("0");
		$("#CHECK_DESC").html("不允许过户给自己！");
		$("#CHECK_DESC").attr("value","不允许过户给自己！");
		return false;
	}
	...
	$.ajax.submit(this, "qureyMphone", inparms, '',function(data){
		 $.endPageLoading();
			...
			$("#IS_CHECK_DEC").html(result.get("IS_CHECK_DEC"));
			$("#CHECK_DESC").html(result.get("CHECK_DESC")); 
			$("#CHECK_DESC").attr("value",result.get("CHECK_DESC"));
...
function onTradeSubmit(){
	...
	if(isCheck!="1"){
		MessageBox.alert("提示","新号码校验未通过，无法办理宽带过户。原因："+$("#CHECK_DESC").attr("value"));
		return false;
	}

	MessageBox.confirm("告警提示","系统将提交宽带过户受理数据信息，你确认要继续吗？",function(re){
		if(re=="ok"){
