...
(function(){
	$.extend(ChangeWidenetProduct.prototype,{
		...
		afterChangeProduct: function(productId,productName,brandCode,brandName){
			...
			$("#PRODUCT_ID_RATE").val(productId);
			var new_rate = PRODUCT_ID_RATE.selectedText;
            ...
            {
                MessageBox.alert("提示","您已经办理过度假宽带月、季、半年套餐（海南）营销活动，不能选择速率低于50M套餐，请重新选择更高档的套餐！");
                $("#CHG_PRODUCT_ID").val("");
                return false;
            }
            //add by zhangxing3 for 候鸟月、季、半年套餐（海南）
            //add by zhangxing3 for BUS201907300031新增度假宽带季度半年套餐开发需求
            if(saleProductId != "" && saleProductId != null && saleProductId == "66004809" && parseInt(new_rate) < 51200)
            {
                MessageBox.alert("提示","您已经办理过度假宽带营销活动，不能选择速率低于50M套餐，请重新选择更高档的套餐！");
                $("#CHG_PRODUCT_ID").val("");
                return false;
            }
            //add by zhangxing3 for BUS201907300031新增度假宽带季度半年套餐开发需求
			...
			if((hasEffActive=="1"||hasEffYear=="1"||(hasYearActive=="1"&&hasEndYearActive=="1"))&&(old_rate!=null&&old_rate!="")&&(new_rate!=null&&new_rate!="")&&(parseInt(old_rate)>parseInt(new_rate))){
				MessageBox.alert("提示","您已经办理过包年套餐或者未到期的营销活动，不能选择低档套餐，请重新选择同档或更高档的套餐！");
				$("#CHG_PRODUCT_ID").val("");
			...
			{
				MessageBox.alert("提示","同制式、同速率的宽带产品不允许移机！");
				$("#CHG_PRODUCT_ID").val("");
				return false;
			}
			
			//REQ201910240002 融合套餐与宽带速率适配系统提醒功能—BOSS侧 start
            var params = "&NEW_PRODUCT_ID=" + $("#CHG_PRODUCT_ID").val()
...
//					$("#MODEL_MODE").append("<option value='3'>自备</option>");
					...
					//add by zhangxing3 for REQ201810080022同市县宽带移机关联和家固话移机
					var isIMSDiffCity = data.get(0).get("IS_IMS_DIFF_CITY");
					if(isIMSDiffCity=="1"){
						alert("业务限制：该用户存在IMS固话，请先办理IMS固话拆机！");
						$("#STAND_ADDRESS").val("");
						return false;
					}
					//add by zhangxing3 for REQ201810080022同市县宽带移机关联和家固话移机
					...
					if(isSameDeviceId=="1"){
						MessageBox.alert("提示","业务限制：您选择的标准地址和现在使用的一致，请重新选择标准地址！");
						$("#STAND_ADDRESS").val("");
						return false;
					}
					...
					if(isChgOther=="1"){
						if(isNeedChgProd=="1"){
							...
							$("#STAND_ADDRESS").val("");
						}else MessageBox.alert("提示","您当前有预约的主产品变更，只能办理移机业务，不能变更产品!");
						return false;
					}
					...
					if(isNeedChgProd=="1")
					{
						if(hasMonthDiscnt=="1")
						{
							MessageBox.alert("提示","您订购的宽带包月优惠不能进行产品变更，只能办理同类型宽带移机！");
							$("#STAND_ADDRESS").val("");
						...
						{
							MessageBox.alert("提示","您的宽带产品已经发生变化，请重新选择您需要的宽带产品");
						}
					}
					else
					{
						if(hasMonthDiscnt=="1")
						{
							MessageBox.alert("提示","您订购的宽带包月优惠不能进行产品变更操作！");
					...
					$("#USER_WIDENET_RATE").val(PRODUCT_ID_RATE.selectedText);
