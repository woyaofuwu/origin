...
function refreshPartAtferAuth(data)
{
	...
	if(tradeTypeCode == '71' || tradeTypeCode == '3810' ){//71用户密码变更，3810无线固话用户密码变更
		...
		//效验用户是否为携入号码
		var param ="&SERIAL_NUMBER="+$("#AUTH_SERIAL_NUMBER").val()+"&EPARCHY_CODE=0898"+"&TRADE_TYPE_CODE=73";
		$.ajax.submit(null,'checkNpQryOrWX',param,'',
				function(data){ 
					//把携入标识  传给界面
		...
		if(npTag == 1 || wxTag == 1){
			...
			if(cmpTag == "0"){
				...
				if(authCheckPsptTypeCode == "0" ||authCheckPsptTypeCode == "1"||authCheckPsptTypeCode == "2"){
					...
					if(custInfoCustName != loadCustName 
							|| authCheckPsptId != loadPsptId
							|| authCheckPsptTypeCode != loadPsptTypeCode){
				...
				if((authCheckPsptTypeCode == "0" ||authCheckPsptTypeCode == "1"||authCheckPsptTypeCode == "2"
					||agentPsptTypeCode== "0"|| agentPsptTypeCode== "1"|| agentPsptTypeCode== "2")&& authCheckMode == "0" ){
...
function clickScanPspt(){
	...
	getMsgByEForm("custInfo_PSPT_ID","custInfo_CUST_NAME",null,null,null,"custInfo_PSPT_ADDR,custInfo_POST_ADDRESS",null,"custInfo_PSPT_END_DATE");
...
function changePsptTypeCode(tag){
	...
	if(tag == "PSPT_TYPE_CODE"){
		...
		//经办人
		var agentPsptTypeCode=$("#custInfo_AGENT_PSPT_TYPE_CODE").val();
		if(agentPsptTypeCode=="0"||agentPsptTypeCode=="1"||agentPsptTypeCode=="2"){
			//显示扫描和摄像按钮
