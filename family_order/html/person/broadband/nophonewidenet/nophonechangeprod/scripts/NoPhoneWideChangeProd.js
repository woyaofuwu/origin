...
(function(){
	$.extend(ChangeWidenetProduct.prototype,{
		...
		afterSubmitSerialNumber: function(data){
			...
			$.ajax.submit(null,"qryUserWideInfos",para_str,'productTypePart,changetimePart,productHiddenPart',
				function(data)
				{
					$.endPageLoading();
		...
		afterLoadChildInfo: function(data){
			...
			$("#PRODUCT_ID_NOPHONE").val(userProductId);
			...
			if( bookTag == "1")
			{
				$("#PACKAGE_VALID").attr("disabled",false);
			}
		},
		...
		getProductRateById:function(newProductId){
			...
			if(productList != null && productList != '')
			{
				...
				{
					if (newProductId == productList.items[i].get('PRODUCT_ID'))
					{
						new_rate = productList.items[i].get('WIDE_RATE');
					}
				}
			}
			return new_rate;
		...
		//选择产品后
		afterChangeProductNew:function(productId,productName,brandCode,brandName){
			var newProductId = $("#PRODUCT_ID_NOPHONE").val();
			...
			//计算到期开始时间
			var endD=agreementEndDate.substr(0,10); 
			endD=endD.replace(/-/g,"/"); 
			var startD=new Date(new Date(endD).valueOf()+1*24*60*60*1000);
			startD.setMonth(startD.getMonth()+1);
			var year=startD.getFullYear();
			var mon=String(startD.getMonth());
			var day=String(startD.getDate());
			if(mon.length==1){
				mon="0"+mon;
			}
			if(day.length==1){
			...
			var nextStartDate=year+"-"+mon+"-"+day; 
			if(Number(new_rate) <  Number(old_rate))
			{
				...
				if("0" == bookTag)
				{
					if("1" == isYearProduct)
					{
						$("#PRODUCT_ID_NOPHONE").val(userCurProductId); //清空所选择的产品
...
//							
//							$("#PRODUCT_ID_NOPHONE").val(userCurProductId);	
//							MessageBox.alert("提示",tipMsg);
//							return false ;
//						}
//					} 
					rateDownUpFlag = "2";
					$("#UP_RATE_STARTDATE").val("");//升档开始日期置空
					$("#PACKAGE_VALID").val(nextStartDate);
					$("#DOWN_RATE_STARTDATE").val(nextStartDate);
					//$("#PACKAGE_VALID").attr("disabled",true);
				}
			}
			else if(Number(new_rate) >  Number(old_rate))
			{
				...
				$("#DOWN_RATE_STARTDATE").val("");//降档开始日期置空
				$("#UP_RATE_STARTDATE").val(nextStartDate);
				var nextMon=$("#NEXT_MON_FISRTDAY").val();
		   		$("#PACKAGE_VALID").attr("disabled",false);
		   		$("#PACKAGE_VALID").val(nextMon);
				rateDownUpFlag = "1";
			}
		   	else if(Number(new_rate) ==  Number(old_rate))
		   	{
...
//						}
		...
		getActiveRuleSvcCheck:function(){
			...
			uls.each(function(i,li){
				...
				if (li_class!="e_del")
				{
					var svc_val = $(li).children("label").children("span").children("input").val();
					if (sel_svcid=="")
					{
						sel_svcid = svc_val;
					}else
		...
		getChangeType:function(){
			...
			if(saleActiveList != null && saleActiveList != '')
			{
				...
				{
					if (newSalePackageId == saleActiveList.items[i].get('PARA_CODE5'))
					{
						newSelectActiveType = saleActiveList.items[i].get('PARA_CODE7');
						newSaleProductId = saleActiveList.items[i].get('PARA_CODE4')
					}
				}
			}
			...
			if(changeUpDownTag == '0')
			{
				...
				else if(oldSalePackageId != '')
				{
					...
					if(newSalePackageId != '' && oldSalePackageId == newSalePackageId)
					{
						//原营销活动非本月底结束
			...
			else
			{
				...
				alert("UP_DOWN_TAG=" + changeUpDownTag + ";BOOK_TAG=" + bookTag + ";IS_YEAR_PRODUCT=" + isYearProduct + ";END_FALG=" + endFlag);
				return false ;
			}
		},
		//校验用户选择的优惠中是否有包年优惠
		...
		{
			var changeType = "1";//changeWidenetProduct.getChangeType();
		...
		submit: function(num)
		{
			...
			if("0" != changeUpDownTag) 
			{
				...
				if(!window.confirm(tipMsgForMove))
				{
					return false ;
				}
			}
		...
		getSaleProductIdByPackageId: function(packageId){
			...
			if(saleActiveList != null && saleActiveList != '')
			{
				for (var i = 0; i < saleActiveList.length; i++)
				{
					if (packageId == saleActiveList.items[i].get('PARA_CODE5'))
					{
						saleProductId = saleActiveList.items[i].get('PARA_CODE4');
					}
				}
			}
		...
		initActivePackage: function(productId){
			...
			$.ajax.submit(null,"initActivePackage",para_str,'',
				function(data){
					//预约时间处理
					$.endPageLoading();
					/*
...
//			alert("para_str="+para_str);
		...
		afterQueryPkgInfo:function(data){
			if(data){
				var temp = data.get(0);
				var ext_desc = temp.get("EXT_DESC");
		...
		{//选择预约日期之后的处理
			...
			var downStartDate=$("#DOWN_RATE_STARTDATE").val();//降档允许开始日期 
			if(downStartDate!="" && downStartDate!=null){
				var d1=new Date(v_date.replace(/-/g,"/"));
				var d2=new Date(downStartDate.replace(/-/g,"/"));
				if(d1 != d2){
					var tipMsg = "您当前有宽带到期后的次月开始时间为：【"+downStartDate+"】，【降档】操作只允许选择该日期，已自动调整，请确认!";
					alert(tipMsg);
					$("#PACKAGE_VALID").val(downStartDate);
					return;
				}
			}
			var upStartDate=$("#UP_RATE_STARTDATE").val();//升档档允许开始日期 
			if(upStartDate!="" && upStartDate!=null){
				var d1=new Date(v_date.replace(/-/g,"/"));
				var d2=new Date(upStartDate.replace(/-/g,"/"));
				if(d1 > d2){
					var tipMsg = "您当前有宽带到期后的次月开始时间为：【"+upStartDate+"】，【升档】操作不允许大于该日期，已自动调整，请确认!";
					alert(tipMsg);
					$("#PACKAGE_VALID").val(upStartDate);
					return;
				}
			}
		...
		getNextStartDate:function(agreementEndDate){
			var endD=agreementEndDate.substr(0,10); 
			endD=endD.replace(/-/g,"/"); 
			var startD=new Date(new Date(endD).valueOf()+1*24*60*60*1000);
			startD.setMonth(startD.getMonth()+1);
