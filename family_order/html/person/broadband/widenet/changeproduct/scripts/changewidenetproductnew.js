		...
		getProductRateSpcById:function(newProductId){
			...
			if(productList != null && productList != '')
			{
				for (var i = 0; i < productList.length; i++)
				{
					if (newProductId == productList.items[i].get('PRODUCT_ID'))
					{
						new_rate_spc = productList.items[i].get('WIDE_RATE_SPECIAL');
					}
				}
			}
			return new_rate_spc;
		...
		getChangeType:function(){
			...
			if(saleActiveList != null && saleActiveList != '')
			{
				for (var i = 0; i < saleActiveList.length; i++)
				{
					if (newSalePackageId == saleActiveList.items[i].get('PARA_CODE5'))
					{
						...
						salePackageIdFlag = saleActiveList.items[i].get('PARA_CODE8'); //20170511不受活动结束时间限制，可以变活动。
						salePackageIdFlagSpc = saleActiveList.items[i].get('PARA_CODE9');
					}
				}
			}
			...
			if(changeUpDownTag == '0')
			{
				...
				else if(oldSalePackageId != '')
				{
					...
					if(newSalePackageId != '' && oldSalePackageId == newSalePackageId && newSaleProductId !="66002202")
					{
						//原营销活动非本月底结束
					...
					else if(newSalePackageId != '' && oldSalePackageId != newSalePackageId)
					{
						...
						if(bookTag == '1')
						{
							...
							else if(newSaleProductId == '66002202')
							{
								changeType = "2";    
								return changeType;
							}
					...
					{
						if(bookTag == '1' && newSaleProductId == '66002202')
						{
							//如果用户选择的时间小于协议结束时间，则不允许提交						
							if(!changeWidenetProduct.dateCompare(effectiveDate,agreementEndDate))
							{
