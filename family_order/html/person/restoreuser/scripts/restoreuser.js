...
function refreshPartAtferAuth(data) {
	...
	$.ajax.submit('AuthPart', 'loadChildInfo', param,'ProductTypePart,userResInfosPart,userOldDiscntPart,hiddenPart',
			function(data) {
...
function tableRowClick(obj) {
    ...
    var imsi = json.get("col_IMSI");
    if(imsi!=null && imsi!="" && imsi!='undefined'){
...
function checkRes() 
{
	var resNo =  $('#RES_CODE').val();//输入的资源编码
	if(resNo==null || resNo=="")
	{
		alert('请输入新资源号码再进行校验！');
		return false;
	...
	var resTypeCode = $('#RES_TYPE_CODE').val();//当前表格选择行的资源类型编码
	if(resTypeCode == "")
    {
...
function submitBeforeAction()
{
	...
	if(!canSubmit){
		...
		if(npTag == 1 || wxTag == 1){
			...
			if((authCheckPsptTypeCode == "0" ||authCheckPsptTypeCode == "1") && authCheckMode == "0"){
