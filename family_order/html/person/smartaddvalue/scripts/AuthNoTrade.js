(function($){
	$.extend({authNoTrade:{
		...
		queryUserCancelTrade:function(){
			//$.BmFrame.setAcctType(); //获取并设置宽带账号类型(主账号、子账号、普通账号)
		...
		submitCancelTrade:function()
		{
	    	...
	    	if (null != tradeInfo.get("RSRV_STR10") && '' != tradeInfo.get("RSRV_STR10")
	    			&& tradeInfo.get("RSRV_STR10").length > 2)
	    	{
	    		if(!window.confirm("该用户宽带开户同时办理了和目业务，请确认是否已经退还和目终端？"))
		    	{
			    	return false;
			    }
	    	}
	    	...
	    	+"&ACCEPT_MONTH="+tradeInfo.get("ACCEPT_MONTH")+"&OPER_FEE="+$("#backOperFee").val()
	    	+"&FOREGIFT="+$("#backForeGift").val()+"&ADVANCE_PAY="+$("#backAdvancePay").val()
	    ...
	    afterSubmit:function(data){
	    	...
	    	MessageBox.success("成功提示","业务取消成功!",function(btn){
	    		$.redirect.toPage("order","smartaddvalue.cancelSmartNetwork",'','');
	    		debugger;
	    	});
	    }
	}});
