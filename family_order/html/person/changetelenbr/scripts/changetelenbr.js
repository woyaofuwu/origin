...
{
	//测试时不弹出选号框,直接进入选号后处理,手动造测试数据
	var staffId = top.$("#staffId").val();
	var deptId = top.$("#deptId").val();
	var loginEpachyId = top.$("#loginEpachyId").val();
	
	//TODO:先注释掉资源的号码选择界面,自己造测试数据
	$.popupPage('brm.officeNumberSelect', 'init', '&refresh=true&cond_FOFFICE_ID='+$("#OLD_SIGN_PATH").val()+'&cond_STAFF_IF_IN='+staffId+'&cond_OPER_DEPART_ID='+deptId+'&cond_EPARCHY_CODE='+loginEpachyId, '选择固话号码', '620', '400','NEW_SERIAL_NUMBER','',subsys_cfg.pbossintf);
...
function selectHintMonth(){	
	...
	if(viewFee && viewFee.get("FEE_LIST")){
		...
		feeList.each(function(item, index, totalCount){
			if(item.get("CODE") == "9004"){
				$.feeMgr.removeFee("9710","0","9004");//移除fee_type_code:9004的费用
			}
			return true;
		});
	}
	...
	if(month>0){
		...
		monthFeeObj.put("CODE","9004");
		monthFeeObj.put("FEE",month*600);
		monthFeeObj.put("TRADE_TYPE_CODE","9710");
		$.feeMgr.insertFee(monthFeeObj);
	}
	$("#MONTH").val(month);
...
{
	//设置测试数据
	$("#POP_cond_REGION_ID").val("新测试标准地址");
	$("#cond_REGION_ID").val("1");
	$("#foffice_id").val("新测试局向地址");
...
function afterSubmitSerialNumber(data){
	$.ajax.submit('AuthPart', 'loadTradeInfo', "&USER_ID="+data.get("USER_INFO").get("USER_ID"), 'installInfoPart', function(data){
		var feeData = $.DataMap();	
		feeData.put("MODE","0");
		feeData.put("CODE","9001");
		feeData.put("FEE","800");
		feeData.put("TRADE_TYPE_CODE","9710");
		$.feeMgr.removeFee("9710","0","9001");//移除fee_type_code:9004的费用
		$.feeMgr.removeFee("9710","0","9004");//移除fee_type_code:9004的费用
