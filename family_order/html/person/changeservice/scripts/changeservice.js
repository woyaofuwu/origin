...
function changeState(value)
{
	if('ADD_SVC' == value)
	{
	 	...
	 	if ($('#ADD_SVC').val() == "19")
	    {
	 		$("#NEW_END_DATE_SPAN").removeClass("e_input");
		 	$("#NEW_END_DATE_SPAN").addClass("e_select");
		 	
		 	$("#NEW_END_DATE_SEL").attr("style","display:block");
			$("#NEW_END_DATE").attr("style","display:none");
			$('#NEW_END_DATE_SEL').empty(); 
			var html = "<option value=''>请选择</option><option value='0'>30天</option><option value='1'>180天</option><option value='2'>长期</option>" 
			 
			$('#NEW_END_DATE_SEL').html(html); 
				 ...
				 dataset.each
					(
						function(info)
						{
							...
							alert("用户话费余额不足，请先交纳"+parseInt(fee)/100+"元差额话费后再重新办理国漫开通功能");
	 	...
	 	else if ($('#ADD_SVC').val() == "15")
	    {
			...
			$('#NEW_END_DATE_SEL').empty(); 
			$('#NEW_END_DATE_SEL').html("<option value=''>请选择</option><option value='2'>长期</option>"); 
	 	}
	 	else
	    {
	 		$("#NEW_END_DATE_SPAN").removeClass("e_select");
		 	$("#NEW_END_DATE_SPAN").addClass("e_input");
		 	
			$("#NEW_END_DATE_SEL").attr("style","display:none");
			$("#NEW_END_DATE").attr("style","display:block");
	 	}
...
function addSvc()
{
	...
	var svcName = $("#ADD_SVC").find("option:selected").text();
	...
	//判断是否重复新增服务
	var userSvcs = $.table.get("newUserSvc").getTableData();
 	//alert(userSvcs);
 	...
 	if(result)
 	{
		...
		info["MODIFY_TAG"] = '0';
		info["MODIFY_NAME"] = '新增';
		$.table.get("newUserSvc").addRow(info);
...
function delSvc()
{
	...
	var svcName = $("#DEL_SVC").find("option:selected").text();
...
function resetSvc()
{
	$.table.get("newUserSvc").cleanRows();
...
function submitBeforeAction() 
{
	...
	*/
	...
	var userId = $.auth.getAuthData().get("USER_INFO").get("USER_ID"); 
	var param = "&SERIAL_NUMBER=" + serialNumber + "&X_CODING_STR=" + userSvcs + "&REMARK=" + remark + "&USER_ID=" + userId;
	//alert(param);
