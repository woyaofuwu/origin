...
function changeOperaType()
{
   ...
   function controlRedirect(order_num,tenant_num,bill_time){	
   	
   	var phone_num = $('#cond_PHONE_NUM').val();
     var param = 'ORDER_NUM='+order_num+'&TENANT_NUM='+tenant_num+'&BILL_TIME='+bill_time+'&PHONE_NUM='+phone_num;
	 $.ajax.submit('recordNavDetail', 'queryBaseDetail', param, 'HiddenPartDetail,SerialNumPartDetail,QueryListPartDetail', function(data){
			$.endPageLoading();
		},
		function(error_code,error_info){
			$.endPageLoading();
