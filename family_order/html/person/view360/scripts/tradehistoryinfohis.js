...
function chgDateFile() {
	$.ajax.submit('QueryCondPart', 'chgDateFile', null, 'SelectDatePart',
			function(data) {
				$.endPageLoading();
...
function thQuery() {
	...
	$.ajax.submit('QueryCondPart,QueryCondPart1,ThInfonav', 'queryInfo', null,
...
function contorlRedirect(cust_id, user_id, trade_flag, trade_id, trade_type) {
	...
	var param = 'TRADE_ID=' + trade_id + '&TRADE_FLAG=' + trade_flag
			+ '&CUST_ID=' + cust_id + '&USER_ID=' + user_id
			+ '&TRADE_TYPE_CODE=' + trade_type + '&SERIAL_NUMBER='
			+ serial_number + '&QUERY_YEAR='+$("#QUERY_YEAR").val() + '&HISTORY_QUERY_TYPE='+$("#HISTORY_QUERY_TYPE").val();
...
function popupPagePlat(obj) {
	var param = 'SERVICE_ID=' + $(obj).attr('service_id') + '&TRADE_ID='
			+ $(obj).attr('trade_id');
	$.ajax.submit('commInfo', 'queryPlatAttrInfo', param,
			'popupPart,popupResult', function(data) {
