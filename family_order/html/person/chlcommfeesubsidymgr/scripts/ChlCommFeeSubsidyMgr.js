...
function refreshPartAtferAuth(data){
	...
	$.ajax.submit('AuthPart', 'loadChildInfo', "&USER_INFO="
			...
			'busiInfoPart,busiInfoPart2', function(data) {
				chlcommfeeInit();
				if(data && "" != data.get("DEPART_KIND_CODE")){
					$("#chlInfo_CHL_TYPE").val(data.get("DEPART_KIND_CODE"));
				}
				$.endPageLoading();
...
function chlcommfeeInit(){
	...
	var para_code5 = $("#chlInfo_PARA_CODE5").val();//渠道星级
	var para_code6 = $("#chlInfo_PARA_CODE6").val();//渠道名称
	var para_code7 = $("#chlInfo_PARA_CODE7").val();//店员编码
	...
	if(para_code2 != ""){
		custType.val(para_code2);
		 $("#chlInfo_CUST_TYPE").attr('disabled',true);
	}
	if(para_code3 != ""){
		chlType.val(para_code3);
		$("#chlInfo_CHL_TYPE").attr("disabled",true);
	}
	if(para_code4 != ""){
		chlCode.val(para_code4);
		chlCode.attr("disabled",true);
	}
	if(para_code5 != ""){
		chlLevel.val(para_code5);
		chlLevel.attr("disabled",true);
	}
	if(para_code6 != ""){
		chlName.val(para_code6);
		chlName.attr("disabled",true);
	}
	if(para_code7 != ""){
		staffId.val(para_code7);
...
function dealChlCodeOrStaffHead(){
	...
	if(custType.val() == "1"||custType.val() == "3"){//店员  modify by qinbin 增加店员(高级及以上级别)
...
function beforeCommitCheck(){
	...
	/* 校验所有的输入框 */
	if(!$.validate.verifyAll("busiInfoPart2")) {
		return false;
	}
