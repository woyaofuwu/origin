...
function queryUncheckInfos() {
	...
	$.ajax.submit("QueryCondPart", "queryUncheckInfos", null, "MonitorInfoPart", function (data) {
		$.endPageLoading();
...
function verifyData(type) {
	if(type == "1") {
		...
		if(startDate > endDate) {
			alert("开始日期不能比结束日期大！");	
			return false;
		}else if((endDate - startDate)/86400000 > 29) {
			alert('【起始、终止】日期时间段不能超过30天!');
			return false;
		}
	}
	return true;
...
function checkMonitoreInfo() {
	...
	$.ajax.submit("QueryCondPart", "checkMonitoreInfo", param, "MonitorInfoPart", function (data) {
		$.endPageLoading();
		$.showSucMessage("审核成功!");
	}, function (error_code, error_info, detail) {
		$.endPageLoading();
