(function($){
	$.extend($.BmFrame,{
		checkedTradeId : "",//票据检查通过流水列表 
		inCheckTradeId : "",//票据待检查流水列表 
		initPage:function(){
        ...
        queryUserCancelTrade:function(){
 	    ...
 	    selTrade:function(rowIndex,isSelect){ //勾选或去勾选可取消业务时触发
 	    	...
 	    	if(isSelect){
 		    	...
 		    	var tradeId = tradeInfo.get("TRADE_ID");
 		    	var params = "&TRADE_INFO="+tradeInfo+"&SERIAL_NUMBER="+$("#SERIAL_NUMBER").val()+"&TRADE_ID="+tradeId;
	    ...
	    gBmFrame_SuccessFunc:function(data){
	    	...
	    	MessageBox.success("成功提示","业务取消成功!",function(btn){
	    		$.redirect.toPage(null,"canceltrade.cancelTrade","onInitTrade",'');
	    		debugger;
	    	});
	    },
