...
c.res_type para_code2,
decode(a.modify_tag,'0','新增','1','删除','2','修改','其他') para_code3,
...
'' start_date,'' end_date,'' eparchy_code,'' remark,'' update_staff_id,'' update_depart_id,'' update_time,'' subsys_code,0 param_attr,'' param_code,'' param_name
  FROM tf_b_trade_relation a,tf_b_trade_res b,td_s_restype c
 WHERE a.trade_id=to_number(:PARA_CODE1)
   AND a.accept_month = TO_NUMBER(SUBSTR(:PARA_CODE1,5,2))
   AND b.trade_id=a.trade_id
   AND b.accept_month = TO_NUMBER(SUBSTR(:PARA_CODE1,5,2))
   AND b.res_code = a.short_code
   AND c.res_type_code = b.res_type_code
   AND a.role_code_b = '2'
