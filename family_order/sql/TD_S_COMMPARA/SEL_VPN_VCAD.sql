select '' SUBSYS_CODE, 0 PARAM_ATTR, '' PARAM_CODE, '' PARAM_NAME,
c.cust_name para_code1, c.group_id para_code2, c.cust_name para_code3, 
a.serial_number para_code4, b.rsrv_str3 para_code5,
b.rsrv_str1 para_code6, 
b.rsrv_str2 para_code7, c.cust_manager_id para_code8, '' PARA_CODE9 , 
'' PARA_CODE10 ,'' PARA_CODE11 ,'' PARA_CODE12 , '' PARA_CODE13 ,
'' PARA_CODE14 , '' PARA_CODE15 ,
'' PARA_CODE16 , '' PARA_CODE17 , '' PARA_CODE18 , '' PARA_CODE19 ,
'' PARA_CODE20 , '' PARA_CODE21 , '' PARA_CODE22 , '' PARA_CODE23 ,
'' PARA_CODE24 , '' PARA_CODE25 , '' PARA_CODE26 , '' PARA_CODE27 ,
'' PARA_CODE28 , '' PARA_CODE29 , '' PARA_CODE30 , '' START_DATE , 
'' END_DATE , '' EPARCHY_CODE , '' REMARK , '' UPDATE_STAFF_ID , 
'' UPDATE_DEPART_ID , '' UPDATE_TIME
from tf_f_user a,tf_f_user_other b,tf_f_cust_group c
where a.user_id=b.user_id
and a.cust_id=c.cust_id
and b.rsrv_value_code='VCAD'
and a.remove_tag='0'
and ((:PARA_CODE1 = '' OR :PARA_CODE1 IS NULL) or a.serial_number=:PARA_CODE1)
and ((:PARA_CODE2 = '' OR :PARA_CODE2 IS NULL) or b.rsrv_value=:PARA_CODE2)
and ((:PARA_CODE3 = '' OR :PARA_CODE3 IS NULL) or a.city_code=:PARA_CODE3) 
and (:PARA_CODE4 = '' OR :PARA_CODE4 IS NULL) 
and (:PARA_CODE5 = '' OR :PARA_CODE5 IS NULL) 
and (:PARA_CODE6 = '' OR :PARA_CODE6 IS NULL) 
and (:PARA_CODE7 = '' OR :PARA_CODE7 IS NULL) 
and (:PARA_CODE8 = '' OR :PARA_CODE8 IS NULL) 
and (:PARA_CODE9 = '' OR :PARA_CODE9 IS NULL)
and (:PARA_CODE10 = '' OR :PARA_CODE10 IS NULL)