...
'' start_date,'' end_date,'' eparchy_code,'' remark,'' update_staff_id,'' update_depart_id,'' update_time,'' subsys_code,0 param_attr,'' param_code,'' param_name
 ...
 WHERE a.cust_id = b.cust_id
    ...
    AND a.serial_number >= :PARA_CODE6
    AND a.serial_number <= :PARA_CODE7
    AND a.remove_tag = '0'
    AND a.score_value >= :PARA_CODE1     
    AND a.score_value <= :PARA_CODE2
    AND a.open_date >= to_date(:PARA_CODE3,'yyyy-mm-dd hh24:mi:ss')
    AND a.open_date <= to_date(:PARA_CODE4,'yyyy-mm-dd hh24:mi:ss')
    AND (:PARA_CODE5 IS NULL OR a.city_code = :PARA_CODE5)
