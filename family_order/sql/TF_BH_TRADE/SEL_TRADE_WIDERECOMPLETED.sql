select replace(t.serial_number, 'KD_', '') || '|||' || t.user_id || '|||' ||
       ...
       w.detail_address || '|||' || decode(e.TRADE_TYPE_CODE,600,'新装',6800,'新装',6900,'新装',606,'移机','拆机') || '|||' || decode(e.TRADE_TYPE_CODE,600,'宽带',606,'宽带',605,'宽带',615,'宽带','固话') as FILEVALUE
  from tf_bh_trade                 t,
 ...
 where t.accept_month = to_number(to_char(sysdate,'mm'))
   and t.trade_type_code in (600,6800,6900，606,605,615,6805)
...
select replace(t.serial_number, 'KD_', '') || '|||' || t.user_id || '|||' ||
       ...
       to_char(t.finish_date, 'yyyymmddhh24miss') || '|||' ||
       to_char(t.accept_date, 'yyyymmddhh24miss') || '|||' ||
       w.detail_address || '|||' || decode(e.TRADE_TYPE_CODE,3800,'新装',4800,'新装','拆机') || '|||' || '魔百盒' as FILEVALUE
  from tf_bh_trade                 t,
       tf_f_user_widenet           w,
       tf_f_user                   u,
       td_m_depart                 d,
       ...
       td_s_tradetype              e
 where t.accept_month = to_number(to_char(sysdate,'mm'))
   and t.trade_type_code in (3800,4800,3806)
   and d.depart_id = t.trade_depart_id
