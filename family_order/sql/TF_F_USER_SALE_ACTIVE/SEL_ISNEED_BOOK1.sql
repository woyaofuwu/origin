SELECT /*+ ordered use_nl(s,a)*/nvl(to_char(max(s.rsrv_date2), 'yyyy-mm-dd hh24:mi:ss'),
...
FROM (SELECT * FROM (SELECT * FROM tf_f_user_sale_active a WHERE a.user_id = to_number(:USER_ID)
      ...
      ORDER BY nvl(a.rsrv_date2,a.end_date) DESC) WHERE ROWNUM = 1) s,td_s_commpara a 
