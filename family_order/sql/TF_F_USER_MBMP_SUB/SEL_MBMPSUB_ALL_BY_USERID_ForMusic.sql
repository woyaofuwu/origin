SELECT partition_id,
       ...
       '' serial_number
FROM
(SELECT /*+ index(a IDX_TF_F_USER_MBMP_SUB_USER_ID) */ a.partition_id,
...
UNION ALL
SELECT /*+ index(a IDX_TF_F_USER_MBMP_USER_ID) */ a.partition_id,
       to_char(a.user_id) user_id,
       c.sp_id,
       a.biz_type_code,
       a.org_domain,
...
WHERE a.user_id = TO_NUMBER(:USER_ID)
   ...
   AND ((a.end_date > SYSDATE AND a.biz_state_code <> 'E' AND a.biz_state_code <> 'P')
        or(a.end_date > SYSDATE -3 AND (a.biz_state_code = 'E' or a.biz_state_code = 'P')))
  AND d.param_attr='3'
  AND d.param_code=a.biz_type_code
