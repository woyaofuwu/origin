SELECT T.info_recv_id,
       ...
       case when length(coalesce(E.serv_request_type,t.serv_request_type)) >= 10 then
       	SUBSTR(coalesce(E.serv_request_type,t.serv_request_type) , 9, 2)
       else
        ''
       end as fourth_type_code,
       case when length(coalesce(E.serv_request_type,t.serv_request_type)) >= 12 then
       	SUBSTR(coalesce(E.serv_request_type,t.serv_request_type) , 9, 4)
       else
        ''
       end as fifth_type_code,
       case when length(coalesce(E.serv_request_type,t.serv_request_type)) >= 14 then
       	SUBSTR(coalesce(E.serv_request_type,t.serv_request_type) , 9, 6)
       else
        ''
       end as serv_request_type,
       case when length(coalesce(E.serv_request_type,t.serv_request_type)) >= 16 then
       	SUBSTR(coalesce(E.serv_request_type,t.serv_request_type) , 9, 6)
       else
        ''
       end as seven_type_code,
       T.recv_province,
  ...
  FROM tf_f_badness_info t, TI_B_BADNESS_EXTENDS E
   ...
   AND ((:BADNESS_INFO_PROVINCE='898' AND (E.BADNESS_INFO_PROVINCE ='000'OR E.BADNESS_INFO_PROVINCE ='898'))
   OR (:BADNESS_INFO_PROVINCE='999' AND E.BADNESS_INFO_PROVINCE ='999')
   OR (:BADNESS_INFO_PROVINCE IS NULL AND (E.BADNESS_INFO_PROVINCE ='898' OR E.BADNESS_INFO_PROVINCE ='000')))  
