/* TF_B_TRADE_BLACKWHITE SEL_PLATSVC_SVCATTR*/
SELECT t.service_id,
...
AND t.oper_state IN (
SELECT s.param_code FROM td_s_commpara s WHERE s.param_attr = '737' AND s.para_code1 = '3' AND s.subsys_code = 'CSM'
))
OR(t.USER_TYPE_CODE in ('B','W','S','XW') 
AND t.oper_state IN (
SELECT s.param_code FROM td_s_commpara s WHERE s.param_attr = '747' AND s.para_code1 = '3' AND s.subsys_code = 'CSM'
)))
