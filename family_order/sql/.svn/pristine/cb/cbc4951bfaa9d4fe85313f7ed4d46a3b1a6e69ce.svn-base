...
SELECT to_number(:TRADE_ID), to_number(:ACCEPT_MONTH), to_number(:USER_ID), -1, -1, -1, to_number(:SERVICE_ID), '0',
    to_number(:INST_ID),null,TO_DATE(:START_DATE,'YYYY-MM-DD HH24:MI:SS'),TO_DATE(:END_DATE,'YYYY-MM-DD HH24:MI:SS'),
    '0',sysdate,:TRADE_STAFF_ID,:TRADE_DEPART_ID,'优惠变化绑定服务变化'
FROM dual
WHERE NOT EXISTS(SELECT 1 FROM tf_f_user_svc
