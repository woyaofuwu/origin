...
WHERE a.group_id = c.group_id
 ...
 and ( decode(rsrv_1,'1','0','0','1','') = :RSRV_STR1 or :RSRV_STR1 is null)
 AND
  ((exists
   ( select 1
     ...
     AND (serial_number = :VPMN_GROUP_ID OR :VPMN_GROUP_ID IS NULL)
    )   )  or :VPMN_GROUP_ID is null )
 and
   ((exists     ( select 1
