select adjust_fee, a_discnt, b_discnt,fee,balance,integrate_item,integrate_item_code,adeb_fee,aimp_fee,
aspay_fee,city_code,pay_name,serial_number,bill_id,user_id,bcyc_id,can_paytag,rsrv_info2,rsrv_info1
  from (
  	select to_char(nvl(c.adjust_fee,0)) adjust_fee,to_char(nvl(c.a_discnt,0)) a_discnt,
  	 ...
  	 (
  	   select adeb_fee,aimp_fee,d.aspay_fee aspay_fee,d.city_code city_code,d.pay_name pay_name,d.serial_number serial_number,bill_id,user_id	
          ,d.bcyc_id bcyc_id,can_paytag, d.recv_acyc_id recv_acyc_id,e.remark rsrv_info2,e.return_tag rsrv_info1 
  	    from tf_a_consigninfolog d,tf_a_consignlog e
  	    where e.acct_id=to_number(:ACCT_ID)  and e.acyc_id >=  :START_ACYC_ID
