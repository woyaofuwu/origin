--IS_CACHE=Y
SELECT staff_id,data_code,data_type,right_attr,right_class,oper_special,right_tag,rsvalue1,rsvalue2,remark,accredit_time,accredit_staff_id 
  FROM (
        ...
        UNION
         SELECT s.staff_id staff_id,NVL(r.data_code,'') data_code,NVL(s.data_type,'') data_type,
        	...
        	NVL(s.remark,' ') remark
        	FROM tf_m_staffdataright s,td_m_dataright d,tf_m_roledataright r
