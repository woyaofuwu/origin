SELECT city_code,depart_id,card_type_code,rsrv_tag1,rsrv_tag2,to_char(sum(para_value9)) para_value9 
  FROM tf_b_resdaystat_log
 WHERE res_type_code=:RES_TYPE_CODE
   AND eparchy_code=:EPARCHY_CODE
   AND stat_type=:STAT_TYPE
   AND (:OPER_FLAG is null or oper_flag=:OPER_FLAG)--库存级别
   ...
   group by city_code,depart_id,card_type_code,rsrv_tag1,rsrv_tag2