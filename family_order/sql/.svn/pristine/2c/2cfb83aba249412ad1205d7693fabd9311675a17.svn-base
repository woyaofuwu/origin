SELECT to_char(charge_id) charge_id,
 ...
 WHERE partition_id >= :PARTITION_ID1 and partition_id <= :PARTITION_ID2 AND
       eparchy_code = :EPARCHY_CODE AND recv_staff_id = :RECV_STAFF_ID AND
...
SELECT to_char(charge_id) charge_id,
       ...
       recv_city_code,
       recv_depart_id,
       recv_staff_id,to_char(cancel_time,'yyyy-mm-dd hh24:mi:ss') cancel_time,cancel_eparchy_code,cancel_city_code,cancel_depart_id,cancel_staff_id  from 
tf_a_paylog_emer where deal_tag='0' and cancel_tag='0'
AND eparchy_code = :EPARCHY_CODE  AND recv_staff_id = :RECV_STAFF_ID AND
       recv_time <= TO_DATE(:RECV_TIME_E, 'YYYY-MM-DD HH24:MI:SS') AND
       recv_time >=
       decode(:X_TAG,
              1,
