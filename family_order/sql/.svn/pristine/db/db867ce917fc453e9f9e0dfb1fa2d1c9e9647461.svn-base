SELECT T.IBSYSID,
  ...
  AND T.ATTACH_TYPE = :ATTACH_TYPE
   AND T.SUB_IBSYSID = (SELECT MAX(A.SUB_IBSYSID)
                          FROM TF_B_EOP_ATTACH A
                         WHERE A.IBSYSID = :IBSYSID
                         AND A.ATTACH_TYPE = :ATTACH_TYPE)
   UNION
        SELECT T.IBSYSID,
       T.SEQ,
       T.GROUP_SEQ,
       ...
       T.VALID_TAG,
       T.RECORD_NUM
  FROM TF_BH_EOP_ATTACH T
  WHERE T.IBSYSID = :IBSYSID
  AND T.ATTACH_TYPE = :ATTACH_TYPE
   AND T.SUB_IBSYSID = (SELECT MAX(A.SUB_IBSYSID)
                          FROM TF_BH_EOP_ATTACH A
                         WHERE A.IBSYSID = :IBSYSID
                         AND A.ATTACH_TYPE = :ATTACH_TYPE)