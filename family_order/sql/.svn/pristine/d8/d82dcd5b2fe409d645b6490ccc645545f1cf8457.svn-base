...
SELECT TO_NUMBER(:TRADE_ID),:ACCEPT_MONTH,TO_NUMBER(:USER_ID),user_id_a,package_id,product_id,TO_NUMBER(:DISCNT_CODE),
     spec_tag,relation_type_code,inst_id,campn_id,nvl(TO_DATE(:START_DATE,'YYYY-MM-DD HH24:MI:SS'),start_date),
     nvl(TO_DATE(:END_DATE,'YYYY-MM-DD HH24:MI:SS'),f_csm_getdiscntenddate(:DISCNT_CODE,:MODIFY_TAG)),'1',sysdate,
     :UPDATE_STAFF_ID,:UPDATE_DEPART_ID,:REMARK
