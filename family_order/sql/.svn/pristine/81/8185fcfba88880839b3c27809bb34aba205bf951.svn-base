SELECT distinct(TO_CHAR(A.TRADE_ID)) TRADE_ID,
  ...
  FROM TF_BH_TRADE A,
       (SELECT MAX(ACCEPT_DATE) ACCEPT_DATE
         ...
         WHERE USER_ID=TO_NUMBER(:USER_ID)
           ...
           AND CANCEL_TAG = '0'
           AND (SUBSTR(PROCESS_TAG_SET,1,1) = :PROCESS_TAG_SET_1 OR :PROCESS_TAG_SET_1 IS NULL)
 ...
 WHERE C.USER_ID=TO_NUMBER(:USER_ID)
   AND C.TRADE_TYPE_CODE = TO_NUMBER(:TRADE_TYPE_CODE)
   AND (C.TRADE_EPARCHY_CODE = :TRADE_EPARCHY_CODE OR :TRADE_EPARCHY_CODE ='-1')
   AND C.CANCEL_TAG = '0'
   AND C.ACCEPT_DATE+0 = B.ACCEPT_DATE
   AND A.ORDER_ID=C.ORDER_ID
    AND (SUBSTR(C.PROCESS_TAG_SET,1,1) = :PROCESS_TAG_SET_1 OR :PROCESS_TAG_SET_1 IS NULL)