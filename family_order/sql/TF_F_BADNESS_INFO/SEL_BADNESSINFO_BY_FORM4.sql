SELECT A.REPORT_TYPE_CODE,
       ...
       (SUM(A.UNDO_SUM) + SUM(A.DONE_SUM)) BADNESSINFO_SUM,
       SUM(A.DONE_SUM) DONE_SUM,
       SUM(A.IN_TIME_SUM) IN_TIME_SUM,
       SUM(A.OUT_TIME_SUM) OUT_TIME_SUM,
       DECODE(SUM(A.DONE_SUM),
              '0',
              '0.00%',
              TRIM(TO_CHAR(SUM(A.IN_TIME_SUM) / (SUM(A.DONE_SUM)) * 100,
                           '9990.99')) || '%') IN_TIME_LV
  FROM (SELECT NVL(SUBSTR(T.SORT_RESULT_TYPE, 5, 2), '07') REPORT_TYPE_CODE,
               ...
               (CASE
                 WHEN T.STATE IN ('02', '03', '0A', '0B') AND
                      T.DEAL_DATE - (T.RECV_TIME + 2) < 0 THEN
                  1
                 ELSE
                  0
               END) IN_TIME_SUM,
