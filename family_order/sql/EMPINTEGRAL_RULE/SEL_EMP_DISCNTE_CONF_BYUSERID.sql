--IS_CACHE=N
SELECT TO_NUMBER(C.PARA_CODE4) 套餐价值,TO_CHAR(D.END_DATE, 'YYYYMM') 结束月份,C.PARA_CODE1 套餐,C.PARA_CODE2 业务编码,C.PARA_CODE6 是否自选, C.PARAM_NAME, T.DEPART_KIND_CODE 部门类型, TO_CHAR(D.UPDATE_TIME, 'YYYYMM') 变更月份,TO_CHAR(D.UPDATE_TIME,'YYYY-MM-DD HH24:MI:SS') 变更时间
...
WHERE C.SUBSYS_CODE = 'CSM'
 ...
 AND TRUNC(D.END_DATE) <> TO_DATE('2050-12-31', 'YYYY-MM-DD') ----失效了
  ...
  AND trunc(D.UPDATE_TIME) = trunc(to_date(:UPDATE_TIME,'YYYY-MM-DD HH24:MI:SS'))
