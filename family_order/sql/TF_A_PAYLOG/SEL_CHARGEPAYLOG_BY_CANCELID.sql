SELECT to_char(charge_id) charge_id,
 ...
 WHERE partition_id >= :PARTITION_ID1 and partition_id <= :PARTITION_ID2 AND
       eparchy_code = :EPARCHY_CODE  AND cancel_staff_id =:CANCEL_STAFF_ID AND cancel_tag = '1' AND
...
tf_a_paylog_emer where deal_tag='0' and cancel_tag='1'and
       eparchy_code = :EPARCHY_CODE   
      AND partition_id >= :PARTITION_ID1 
      and partition_id <= :PARTITION_ID2
      AND cancel_staff_id =:CANCEL_STAFF_ID AND
       cancel_time <= TO_DATE(:RECV_TIME_E, 'YYYY-MM-DD HH24:MI:SS') AND
       cancel_time  >=
       decode(:X_TAG,
              1,
