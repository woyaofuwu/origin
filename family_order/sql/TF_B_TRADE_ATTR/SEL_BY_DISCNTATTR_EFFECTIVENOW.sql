SELECT 
    ...
    T2.IS_NEED_PF,
    T2.ELEMENT_ID
FROM TF_B_TRADE_DISCNT T1,TF_B_TRADE_ATTR T2
WHERE T1.INST_ID=T2.RELA_INST_ID
    AND T1.USER_ID=T2.USER_ID
    AND T1.TRADE_ID=T2.TRADE_ID
    AND SYSDATE BETWEEN T1.START_DATE AND T1.END_DATE
    AND T2.END_DATE < trunc(add_months(sysdate,1),'mm')
    AND T1.DISCNT_CODE=:DISCNT_CODE 
    AND T2.TRADE_ID=:TRADE_ID
    and( T2.ELEMENT_ID=:DISCNT_CODE or T2.ELEMENT_ID='')
